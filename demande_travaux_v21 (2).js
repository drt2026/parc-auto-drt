...
(function () {
  ...
  function buildCarburantSheet(workbook, sheetName, list) {
    ...
    list.forEach(function(e, idx) {
      ...
      var row = ws.addRow([
        ...
        prixLitre,
        { formula: 'IF(AND(ISNUMBER(L' + r + '),ISNUMBER(O' + r + '),O' + r + '<>0),L' + r + '/O' + r + ',0)-(IF(ISNUMBER(J' + r + '),J' + r + ',0)-IF(ISNUMBER(H' + r + '),H' + r + ',0))' },
  ...
  css.textContent = `
    ...
    /* BLOC ADDITIF — Colonne Actions toujours visible (fixée à droite, sans besoin de scroller) */
    .tr2-table th.tr2-sticky-actions { position:sticky; right:0; z-index:3; background:#1A1A2E;
      box-shadow:-4px 0 6px -2px rgba(0,0,0,0.18); }
    .tr2-table td.tr2-sticky-actions { position:sticky; right:0; z-index:2; background:#fff;
      box-shadow:-4px 0 6px -2px rgba(0,0,0,0.12); white-space:nowrap; }
    .tr2-table tr:hover td.tr2-sticky-actions { background:#fafbfc; }
    /* FIN BLOC ADDITIF */
  ...
  function initSuiviCarburantModal() {
    ...
    css.textContent = `
      ...
      #tr3-modal { background:#fff; border-radius:20px; width:min(1700px,99vw);
        max-height:98vh; overflow-y:auto;
        box-shadow:0 24px 64px rgba(0,0,0,0.22);
        animation:tr2Up .28s cubic-bezier(.22,1,.36,1); }
      #tr3-table-wrap table { width:100%; border-collapse:collapse; font-size:16.5px; margin:10px 0; }
      #tr3-table-wrap thead tr { background:#1F3864; color:#fff; }
      #tr3-table-wrap th { padding:13px 12px; text-align:left; font-weight:600; }
      #tr3-table-wrap td { padding:11px 10px; border-bottom:1px solid #e2e8f0; }
      #tr3-table-wrap td input[type=number] { width:130px; padding:11px 12px; border:1.5px solid #e2e8f0;
        border-radius:8px; font-size:16.5px; font-family:inherit; }
      #tr3-table-wrap td input[type=text] { width:200px; padding:11px 12px; border:1.5px solid #e2e8f0;
        border-radius:8px; font-size:16.5px; font-family:inherit; }
    ...
    function renderTable(nom, mois) {
      ...
      var rowsHtml = vehicles.map(function(mat) {
        ...
        var alreadySaisi = !!entry;
        // Index Début toujours automatique = index fin du mois précédent (jamais modifiable
        // manuellement dès qu'un historique existe ; libre uniquement pour la toute 1ère saisie).
        var debVal = alreadySaisi ? entry.indexDebut : (lastIdx != null ? lastIdx : '');
        var debReadonly = alreadySaisi || (lastIdx != null);
        var finVal = alreadySaisi ? entry.indexFin : '';
        var qDebVal = alreadySaisi ? entry.qteDebut : (lastQte != null ? lastQte : '');
        var qDebReadonly = alreadySaisi || (lastQte != null);
        ...
        return '<tr data-mat="' + mat + '"' + (alreadySaisi ? ' class="tr3-row-locked"' : '') + '>' +
          ...
          '<td><input type="text" class="tr3-obs" value="' + obsVal + '" placeholder="Optionnel" style="width:170px"' + lockAttr + '></td>' +
          '<td class="tr3-actions" style="white-space:nowrap">' + actionsCell + '</td>' +
          '</tr>';
      }).join('');
      wrap.innerHTML = '<div style="overflow-x:auto"><table style="min-width:1560px"><thead><tr>' +
          ...
          <table class="tr2-table" style="min-width:1220px">
            <thead>
              <tr>
                ...
                <th>Distance</th><th>Observations</th><th>Saisi par</th><th>Date saisie</th><th class="tr2-sticky-actions">Actions</th>
              </tr>
            </thead>
