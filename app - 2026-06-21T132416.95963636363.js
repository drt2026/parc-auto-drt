...
class ParcAutoApp {
  ...
  sendAdminOtpViaWhatsApp(code, user) {
    ...
    if (!phone) {
      this.showToast('❌ Envoi OTP impossible (email et WhatsApp non configurés).', 'error');
      this._pendingAdminUser = null;
      return;
    }
    const msg = `🔐 Parc Auto DRT Sfax — Code de connexion admin: ${code} (valable 5 min)`;
    const waUrl = `https://wa.me/${phone}?text=${encodeURIComponent(msg)}`;
    window.open(waUrl, '_blank');
    this.showToast('📲 Email indisponible — code OTP envoyé via WhatsApp', 'warning');
    this.openOtpModal();
  }
    ...
    container.innerHTML = `
      <div class="vehicle-detail-card">
        ...
        <div class="vehicle-detail-body">
          ...
          ${(vehicle.constructeur || vehicle.typeCommercial || vehicle.numSerie || vehicle.dateCirculation || vehicle.genre || vehicle.energie) ? `
          <div class="detail-section">
            <div class="detail-section-title">📇 Caractéristiques techniques (carte grise)</div>
            <div class="detail-grid">
              ${vehicle.constructeur ? `
              <div class="detail-item">
                <div class="detail-item-label">Constructeur</div>
                <div class="detail-item-value">${vehicle.constructeur}</div>
              </div>` : ''}
              ${vehicle.typeCommercial ? `
              <div class="detail-item">
                <div class="detail-item-label">Type commercial</div>
                <div class="detail-item-value">${vehicle.typeCommercial}</div>
              </div>` : ''}
              ${vehicle.numSerie ? `
              <div class="detail-item">
                <div class="detail-item-label">N° de série (châssis)</div>
                <div class="detail-item-value" style="font-size:13px;letter-spacing:0.5px;">${vehicle.numSerie}</div>
              </div>` : ''}
              ${vehicle.dateCirculation ? `
              <div class="detail-item">
                <div class="detail-item-label">1ère mise en circulation</div>
                <div class="detail-item-value">${new Date(vehicle.dateCirculation).toLocaleDateString('fr-FR')}</div>
              </div>` : ''}
              ${vehicle.genre ? `
              <div class="detail-item">
                <div class="detail-item-label">Genre</div>
                <div class="detail-item-value">${vehicle.genre}</div>
              </div>` : ''}
              ${vehicle.energie ? `
              <div class="detail-item">
                <div class="detail-item-label">⚡ Énergie</div>
                <div class="detail-item-value">${vehicle.energie}</div>
              </div>` : ''}
            </div>
          </div>
          ` : ''}
