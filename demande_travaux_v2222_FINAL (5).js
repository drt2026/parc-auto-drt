...
(function () {
  ...
  // ══════════════════════════════════════════════════════════
  // BLOC ADDITIF — MOTEUR SONORE (Web Audio API)
  // Sons purs générés en temps réel, aucun fichier externe.
  // ══════════════════════════════════════════════════════════
  var TR2_AUDIO_CTX = null;

  function getAudioCtx() {
    if (!TR2_AUDIO_CTX || TR2_AUDIO_CTX.state === 'closed') {
      try { TR2_AUDIO_CTX = new (window.AudioContext || window.webkitAudioContext)(); } catch(e) { return null; }
    }
    // Reprendre si suspendu (politique autoplay navigateur)
    if (TR2_AUDIO_CTX.state === 'suspended') { try { TR2_AUDIO_CTX.resume(); } catch(e){} }
    return TR2_AUDIO_CTX;
  }

  // Jouer une note : fréquence (Hz), durée (s), volume (0-1), type onde, délai (s)
  function tr2PlayTone(freq, dur, vol, type, delay) {
    var ctx = getAudioCtx();
    if (!ctx) return;
    ...
    gain.gain.setValueAtTime(vol || 0.35, ctx.currentTime + (delay||0));
  ...
  // 🔔 Nouvelle demande soumise (double bip montant — pour validateur + chef garage)
  function tr2SoundNewDemande() {
    tr2PlayTone(880, 0.12, 0.4,  'sine', 0);
    tr2PlayTone(880, 0.12, 0.4,  'sine', 0.16);
    tr2PlayTone(1100, 0.18, 0.35,'sine', 0.35);
  }

  // ✅ Demande validée (accord montant joyeux)
  function tr2SoundValidee() {
    tr2PlayTone(523, 0.10, 0.3,  'sine', 0);     // Do
    tr2PlayTone(659, 0.10, 0.3,  'sine', 0.11);  // Mi
    tr2PlayTone(784, 0.20, 0.35, 'sine', 0.22);  // Sol
    tr2PlayTone(1047,0.25, 0.3,  'sine', 0.35);  // Do aigu
  }

  // ❌ Demande rejetée (accord descendant grave)
  function tr2SoundRejetee() {
    tr2PlayTone(440, 0.15, 0.35, 'sawtooth', 0);
    tr2PlayTone(349, 0.15, 0.35, 'sawtooth', 0.18);
    tr2PlayTone(261, 0.25, 0.3,  'sawtooth', 0.36);
  }

  // 🔄 Actualisation / données reçues (bip discret unique)
  function tr2SoundRefresh() {
    tr2PlayTone(660, 0.08, 0.2, 'sine', 0);
    tr2PlayTone(880, 0.12, 0.18,'sine', 0.10);
  }

  // 🟠 Alerte nouvelle demande côté validateur/chef garage (polling détecte nouveau)
  function tr2SoundAlert() {
    tr2PlayTone(1047, 0.10, 0.4, 'square', 0);
    tr2PlayTone(1047, 0.10, 0.4, 'square', 0.15);
    tr2PlayTone(1047, 0.10, 0.4, 'square', 0.30);
    tr2PlayTone(1319, 0.20, 0.35,'sine',   0.48);
  }
  // FIN BLOC ADDITIF — MOTEUR SONORE
  ...
  css.textContent = `
    ...
    .tr2v-hist-back-bar { display:flex; align-items:center; gap:12px; margin-bottom:16px; }
    ...
    #tr2cg-open-btn { position:fixed; bottom:160px; right:18px; z-index:8998;
      background:linear-gradient(135deg,#064e3b,#065f46); color:#fff;
      border:none; border-radius:50px; padding:12px 18px;
      font-size:13px; font-weight:700; cursor:pointer; box-shadow:0 6px 20px rgba(0,0,0,.3);
      display:flex; align-items:center; gap:8px; transition:transform .2s,opacity .2s; }
    #tr2cg-open-btn:hover { transform:translateY(-2px); opacity:.92; }
    #tr2cg-open-btn .badge { background:#E67E22; color:#fff; border-radius:10px;
      font-size:11px; padding:1px 7px; font-weight:800; }
  ...
  function initUserForm() {
      ...
      <div id="tr2-modal" role="dialog" aria-modal="true" aria-label="Demande de Travaux">
        ...
        <div class="tr2-bd">
          ...
          <div id="tr2-sc-form" style="display:none">
            ...
            <!-- BLOC ADDITIF — Signature + Cachet demandeur -->
            <div class="tr2-f" id="tr2-sig-bloc">
              <label>Signature et cachet <em>*</em></label>
              <div style="border:1.5px solid #e2e8f0;border-radius:9px;background:#f8fafc;padding:8px;position:relative;">
                <canvas id="tr2-sig-canvas" width="480" height="120"
                  style="display:block;width:100%;border-radius:6px;background:#fff;cursor:crosshair;touch-action:none;border:1px dashed #cbd5e1;"></canvas>
                <div style="display:flex;gap:8px;margin-top:7px;align-items:center;">
                  <button type="button" id="tr2-sig-clear" style="background:#FEE2E2;color:#991b1b;border:none;border-radius:7px;padding:5px 13px;font-size:12px;font-weight:700;cursor:pointer;">🗑️ Effacer</button>
                  <span id="tr2-sig-hint" style="font-size:11px;color:#94a3b8;">Dessinez votre signature dans le cadre ci-dessus</span>
                </div>
              </div>
              <div id="tr2-sig-err" style="display:none;color:#C0392B;font-size:12px;margin-top:4px;">⚠️ La signature est obligatoire.</div>
            </div>
            <!-- FIN BLOC ADDITIF — Signature + Cachet -->
    ...
    // BLOC ADDITIF — Initialisation canvas signature
    (function initSignatureCanvas() {
      var canvas = document.getElementById('tr2-sig-canvas');
      if (!canvas) return;
      ...
      function draw(e)      { if (!drawing) return; e.preventDefault(); var p = getPos(e); ctx.lineTo(p.x, p.y); ctx.strokeStyle = '#1A1A2E'; ctx.lineWidth = 1.8; ctx.lineCap = 'round'; ctx.stroke(); hasDrawn = true; document.getElementById('tr2-sig-hint').textContent = '✅ Signature saisie'; }
      ...
      document.getElementById('tr2-sig-clear').addEventListener('click', function() {
        ctx.clearRect(0, 0, canvas.width, canvas.height);
        hasDrawn = false;
        document.getElementById('tr2-sig-hint').textContent = 'Dessinez votre signature dans le cadre ci-dessus';
        document.getElementById('tr2-sig-err').style.display = 'none';
      });

      // Exposer pour lecture au submit
      window._tr2SigCanvas = canvas;
      window._tr2SigHasDrawn = function() { return hasDrawn; };
    })();
    // FIN BLOC ADDITIF — canvas signature
    ...
    function notifValidaterAction(id, newStatut, comment, nom) {
      ...
      saveData();
      _notifSelected = null;
      // BLOC ADDITIF — son validation/rejet (notif demandeur)
      if (newStatut === 'VALIDÉE') { tr2SoundValidee(); } else { tr2SoundRejetee(); }
      // FIN BLOC ADDITIF
    ...
    // BLOC ADDITIF — Polling demandeur : détecter changement de statut + son
    var _demPollPrev = {}; // { id: statut }
    setInterval(function() {
      var sess = getSess();
      if (!sess || !sess.nom) return;
      // Recharger depuis localStorage d'abord
      try {
        var raw = localStorage.getItem('parcAutoData_v3');
        if (raw) {
          var parsed = JSON.parse(raw);
          var pa = window.parcAuto;
          if (pa && pa.data && parsed && parsed.demandesTravaux) pa.data.demandesTravaux = parsed.demandesTravaux;
        }
      } catch(e) {}
      var data = getData();
      ...
      mesDemandes.forEach(function(d) {
        ...
        if (prev && prev !== d.statut) {
          // Statut changé !
          changed = true;
          if (d.statut === 'VALIDÉE') { tr2SoundValidee(); }
          else if (d.statut === 'REJETÉE') { tr2SoundRejetee(); }
        }
        _demPollPrev[d.id] = d.statut;
      });
      // Rafraîchir l'écran notif si ouvert et statut a changé
      if (changed) {
        var notifScreen = document.getElementById('tr2-sc-notif');
        if (notifScreen && notifScreen.style.display !== 'none') {
          renderNotifScreen(sess.nom);
        }
      }
    }, 15000);
    ...
    document.getElementById('tr2-notif-refresh-btn').addEventListener('click', function() {
      ...
      function afterRefresh(fromGist) {
        ...
        // BLOC ADDITIF — son actualisation
        tr2SoundRefresh();
        // FIN BLOC ADDITIF
    ...
    document.getElementById('tr2-submit-btn').addEventListener('click', function() {
      ...
      // BLOC ADDITIF — vérification signature
      var sigOk = window._tr2SigHasDrawn && window._tr2SigHasDrawn();
      if (!sigOk) {
        document.getElementById('tr2-sig-err').style.display = 'block';
        ok = false;
      } else {
        document.getElementById('tr2-sig-err').style.display = 'none';
      }
      ...
      function trySubmit() {
        ...
        var demande = {
          ...
          commentaire:        '',
          signatureDataUrl:   (window._tr2SigCanvas ? window._tr2SigCanvas.toDataURL('image/png') : '')
        };
        data.demandesTravaux.push(demande);
        saveData();
        // BLOC ADDITIF — son nouvelle demande
        tr2SoundNewDemande();
        // FIN BLOC ADDITIF
        // BLOC ADDITIF — reset canvas signature après soumission
        if (window._tr2SigCanvas) {
          var ctx2 = window._tr2SigCanvas.getContext('2d');
          ctx2.clearRect(0, 0, window._tr2SigCanvas.width, window._tr2SigCanvas.height);
          var hintEl = document.getElementById('tr2-sig-hint');
          if (hintEl) hintEl.textContent = 'Dessinez votre signature dans le cadre ci-dessus';
        }
        // FIN BLOC ADDITIF
    ...
    var iframeContent = '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>' +
      ...
      '<div class="pg">' +
        ...
        '<div style="display:flex;gap:16px;margin-bottom:14px;">' +
          '<div style="flex:1;border:1px solid #e2e8f0;border-radius:8px;padding:12px 14px;min-height:60px;">' +
            '<div style="font-size:9px;text-transform:uppercase;color:#94a3b8;font-weight:700;margin-bottom:6px;">Signature et cachet du demandeur</div>' +
            (d.signatureDataUrl ? '<img src="' + d.signatureDataUrl + '" style="max-width:100%;max-height:80px;display:block;margin-top:4px;" alt="Signature"/>' : '') +
          '</div>' +
          '<div style="flex:1;border:1px solid ' + statutColor + ';border-radius:8px;padding:12px 14px;min-height:60px;background:' + statutBg + '22;">' +
            ...
            (d.validePar ? '<div style="font-size:12px;font-weight:600;color:#1e293b;">' + d.validePar + '</div>' : '') +
          '</div>' +
        '</div>' +
  ...
  function initValidateurInterface() {
    ...
    function validaterAction(id, newStatut, comment) {
      ...
      // FIN BLOC ADDITIF
      saveData();
      // BLOC ADDITIF — son validation/rejet (validateur)
      if (newStatut === 'VALIDÉE') { tr2SoundValidee(); } else { tr2SoundRejetee(); }
      // FIN BLOC ADDITIF
      if (window.parcAuto && typeof window.parcAuto.showToast === 'function') {
        window.parcAuto.showToast(
    ...
    // BLOC ADDITIF — Polling RÉEL validateur (fetch Gist toutes les 15s)
    var _vPollLastCount = -1;

    function tr2FetchAndInject(onDone) {
      // 1. Chercher URL Worker dans parcAuto
      var pa = window.parcAuto;
      var workerUrl = null;
      if (pa) {
        workerUrl = pa.WORKER_URL || pa.workerUrl || pa._workerUrl || pa.apiUrl || pa.API_URL || null;
      ...
      if (workerUrl) {
        ...
        fetch(workerUrl + sep + '_nc=' + Date.now(), { cache: 'no-store' })
          ...
          .then(function(freshData) {
            if (pa && pa.data && freshData && freshData.demandesTravaux) {
              ...
              try {
                var stored = JSON.parse(localStorage.getItem('parcAutoData_v3') || '{}');
                stored.demandesTravaux = freshData.demandesTravaux;
                localStorage.setItem('parcAutoData_v3', JSON.stringify(stored));
              } catch(e) {}
            }
            if (onDone) onDone();
        ...
        try {
          var raw2 = localStorage.getItem('parcAutoData_v3');
          if (raw2) {
            var parsed2 = JSON.parse(raw2);
            var pa2 = window.parcAuto;
            if (pa2 && pa2.data && parsed2 && parsed2.demandesTravaux) pa2.data.demandesTravaux = parsed2.demandesTravaux;
          }
        } catch(e) {}
        if (onDone) onDone();
      }
    ...
    setInterval(function() {
      tr2FetchAndInject(function() {
        ...
        renderValidStats();
        // Si l'interface validateur est ouverte, rafraîchir aussi la liste
  ...
  function initChefGarageInterface() {
    ...
    cgBtn.innerHTML = '🔧 Chef Garage <span class="badge" id="tr2cg-badge" style="display:none">0</span>';
    document.body.appendChild(cgBtn);
    ...
    function renderCGList(filter) {
      ...
      list.innerHTML = demandes.map(function(d) {
        ...
        var obsHtml = d.commentaire
          ...
          : '';
        var sigHtml = d.signatureDataUrl
          ? '<div style="margin-top:8px;border:1px solid #e2e8f0;border-radius:8px;padding:8px 12px;background:#fafafa;"><div class="tr2cg-lbl">Signature demandeur</div><img src="' + d.signatureDataUrl + '" style="max-width:180px;max-height:55px;display:block;margin-top:4px;" alt="Signature"/></div>'
          : '';
        ...
        return '<div class="tr2cg-card' + (isNew && !d.cgTraite ? ' new-card' : '') + '" id="cgcard-' + d.id + '">' +
          ...
          '<div class="tr2cg-card-body">' +
            ...
            '<div style="margin-top:10px;text-align:right;">' +
              '<button class="tr2cg-done-btn' + (d.cgTraite ? ' done' : '') + '" id="tr2cg-done-' + d.id + '" data-id="' + d.id + '">' + (d.cgTraite ? '✅ Marqué comme traité' : '🔧 Marquer comme traité') + '</button>' +
            '</div>' +
          '</div>' +
        '</div>';
      }).join('');
    ...
    // ── Polling Chef Garage RÉEL (fetch Gist toutes les 15s) ─
    // BLOC ADDITIF — Polling Chef Garage + alerte sonore
    ...
    setInterval(function() {
      tr2FetchAndInject(function() {
        ...
        renderCGStats();
        // Si l'interface chef garage est ouverte, rafraîchir la liste
        var cgOverlay = document.getElementById('tr2cg-overlay');
        if (cgOverlay && cgOverlay.classList.contains('open')) {
          var cgSess = getCGSess();
          if (cgSess) renderCGList(_cgFilter);
        }
      });
    }, 15000);
    setTimeout(renderCGStats, 3000);
    // FIN BLOC ADDITIF — Polling Chef Garage
  }
  // FIN BLOC ADDITIF — INTERFACE CHEF GARAGE
