...
(function () {
  ...
  var TR3_PRIX_LITRE_DEFAUT = {
    ...
    'ESSENCE': 2.525,
    'SANS PLOMB': 2.525,
    // BLOC CORRECTIF — alias reconnus dans le modèle véhicule mais absents du dictionnaire
    // d'origine (ex: champ ENERGIE = "Diesel" au lieu de "GASOIL"), qui empêchaient le
    // calcul du prix du litre, et donc de la quantité consommée et du taux, pour ces véhicules.
    'DIESEL': 1.985,
    'GAZOLE': 1.985,
    'SP': 2.525,
    'SANS-PLOMB': 2.525
  ...
  function buildCarburantSheet(workbook, sheetName, list) {
    ...
    list.forEach(function(e, idx) {
      ...
      // BLOC CORRECTIF — calcul des valeurs réelles en JS, écrites en cache à côté des
      // formules Excel (formula + result), pour qu'elles s'affichent immédiatement même
      // dans les lecteurs qui n'exécutent pas le moteur de calcul d'Excel (aperçu, import,
      // LibreOffice sans recalcul, etc.). Les formules restent identiques : elles se
      // recalculeront normalement si l'utilisateur modifie le fichier dans Excel.
      var idxDebutNum = (typeof e.indexDebut === 'number') ? e.indexDebut : null;
      var idxFinNum   = (typeof e.indexFin === 'number') ? e.indexFin : null;
      var distanceVal = (idxFinNum != null && idxDebutNum != null) ? (idxFinNum - idxDebutNum) : '-';

      var ravitNum  = (typeof e.ravitaillement === 'number') ? e.ravitaillement : null;
      var prixNum   = (typeof prixLitre === 'number') ? prixLitre : null;
      var qteConsoVal = (ravitNum != null && prixNum) ? (ravitNum / prixNum) : 0;

      var distanceNum = (typeof distanceVal === 'number') ? distanceVal : null;
      var tauxVal = (qteConsoVal != null && distanceNum) ? (qteConsoVal / distanceNum) : '-';
      ...
      var row = ws.addRow([
        ...
        prixLitre,
        { formula: 'IF(AND(ISNUMBER(L' + r + '),ISNUMBER(O' + r + '),O' + r + '<>0),L' + r + '/O' + r + ',0)', result: qteConsoVal },
  ...
  function initSuiviCarburantModal() {
    ...
    function renderTable(nom, mois) {
      ...
      var rowsHtml = vehicles.map(function(mat) {
        ...
        // BLOC ADDITIF — Taux de consommation affiché directement à l'écran (pas seulement
        // dans l'export Excel), calculé à partir du prix du litre du véhicule (getVehicleFull/
        // getPrixLitre, même logique que l'export) et du ravitaillement déjà saisi.
        var vFull = getVehicleFull(mat) || {};
        var prixLitreEcran = getPrixLitre(vFull.energie, vFull.prixLitre) || null;
        var tauxTxt = '—';
        if (typeof distTxt === 'number' && distTxt > 0 && ravitVal !== '' && prixLitreEcran) {
          var qteConsoEcran = parseFloat(ravitVal) / prixLitreEcran;
          tauxTxt = (qteConsoEcran / distTxt * 100).toFixed(2) + '%';
        }
        // FIN BLOC ADDITIF
        return '<tr data-mat="' + mat + '"' + (alreadySaisi ? ' class="tr3-row-locked"' : '') + '>' +
          ...
          '<td class="tr3-taux" style="text-align:center" data-prix="' + (prixLitreEcran || '') + '">' + tauxTxt + '</td>' +
      ...
      wrap.innerHTML = '<div style="overflow-x:auto"><table style="min-width:1180px"><thead><tr>' +
        ...
        '<th>Index Fin</th><th>Ravitaillement (DT)</th><th>Solde Carte (DT)</th><th>Distance</th><th>Taux Conso. (%)</th><th>Observations</th><th>Actions</th>' +
      ...
      wrap.querySelectorAll('tr[data-mat]').forEach(function(tr) {
        ...
        var ravitInp = tr.querySelector('.tr3-ravit');
        var distEl = tr.querySelector('.tr3-dist');
        var tauxEl = tr.querySelector('.tr3-taux');
        var prixLitreLive = parseFloat(tauxEl && tauxEl.getAttribute('data-prix')) || null;
        function recalc() {
          ...
          if (!isNaN(d) && !isNaN(f)) {
            distNow = f - d;
            distEl.textContent = distNow;
            distEl.style.color = (f < d) ? '#dc2626' : '#15803d';
          } else {
            distEl.textContent = '—';
            distEl.style.color = '';
          }
          // BLOC ADDITIF — recalcul live du Taux de Consommation à chaque saisie
          if (tauxEl) {
            var ravitNow = parseFloat(ravitInp && ravitInp.value);
            if (distNow != null && distNow > 0 && !isNaN(ravitNow) && prixLitreLive) {
              tauxEl.textContent = (ravitNow / prixLitreLive / distNow * 100).toFixed(2) + '%';
            } else {
              tauxEl.textContent = '—';
            }
          }
        ...
        if (ravitInp) ravitInp.addEventListener('input', recalc);
      });
  ...
  function initSuiviCarburantAdminTab() {
    ...
    function injectTabContent() {
      ...
      tabEl.innerHTML = `
        ...
        <div class="tr2-table-wrap" style="overflow-x:auto">
        ...
        ensureXLSXReadLib(function() {
          try {
            ...
            wb.SheetNames.forEach(function(sheetName) {
              ...
              var iRavit  = findCol(headerRow, /^ravitaillement/i);
