...
class ParcAutoApp {
  ...
  initUserLogin() {
    ...
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      const agentMatInput = (document.getElementById('user-agent-matricule')?.value || '').trim().toUpperCase();
      const vehicleMatInput = document.getElementById('user-matricule').value.trim().toUpperCase();
      ...
      // Trouver le véhicule correspondant aux DEUX matricules
      const vehicle = this.data.vehicles.find(v => {
        const dbVehMat = v.matricule.toUpperCase().replace(/[\s-]/g, '').trim();
        const inputVehMat = vehicleMatInput.replace(/[\s-]/g, '').trim();
        const vehMatch = dbVehMat === inputVehMat;
        if (!vehMatch) return false;

        // Vérifier matricule agent si le véhicule en a un
        if (v.matriculeAgent) {
          const dbAgent = v.matriculeAgent.toUpperCase().replace(/[\s-]/g, '').trim();
          const inputAgent = agentMatInput.replace(/[\s-]/g, '').trim();
          return dbAgent === inputAgent;
        }
        // Si pas de matricule agent configuré → accès libre (rétrocompatibilité)
        return true;
      });

      if (vehicle) {
        this.data.currentUser = { role: 'user', name: vehicle.chauffeur || 'Utilisateur', agentMatricule: agentMatInput };
  ...
  openVehicleModal(vehicleId = null) {
    ...
    if (vehicleId) {
      ...
      if (v) {
        ...
        if (document.getElementById('vehicle-agent-matricule')) document.getElementById('vehicle-agent-matricule').value = v.matriculeAgent || '';
  ...
  async saveVehicle() {
    ...
    const vehicle = {
      ...
      matriculeAgent: (document.getElementById('vehicle-agent-matricule')?.value || '').trim().toUpperCase() || null,
      km: kmVal,
