...
// ============================================
const OTP_CONFIG = {
  sessionToken: null,   // token de session côté serveur (pas le code !)
  attempts: 0,
  maxAttempts: 3,
...
// generateOTP() supprimé — l'OTP est généré côté serveur (Worker)
// Le navigateur ne connaît jamais le code, sécurité renforcée
...
async function requestOTPEmail(vehicleData) {
  ...
  sessionStorage.setItem('_otp_vehicle', JSON.stringify(vehicleData));
  ...
  try {
    ...
    if (!resp.ok || !data.ok) {
      ...
      return { ok: false, msg: data.error || 'Erreur envoi OTP' };
    }

    // 🔑 Stocker le sessionToken (PAS le code !) — le code reste côté serveur
    OTP_CONFIG.sessionToken = data.sessionToken;
    sessionStorage.setItem('_otp_sessionToken', data.sessionToken);

    console.log('✅ OTP demandé, sessionToken:', data.sessionToken);
    return { ok: true };
  } catch (e) {
...
async function verifyOTP(inputCode) {
  ...
  if (!OTP_CONFIG.sessionToken) {
    OTP_CONFIG.sessionToken = sessionStorage.getItem('_otp_sessionToken');
    const saved = sessionStorage.getItem('_otp_vehicle');
    if (saved) OTP_CONFIG.vehicleData = JSON.parse(saved);
  }

  if (!OTP_CONFIG.sessionToken) {
    return { ok: false, msg: 'Session invalide. Reconnectez-vous.' };
  }
  ...
  try {
    const resp = await fetch(`${WORKER_URL}/verify-otp`, {
      ...
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        sessionToken: OTP_CONFIG.sessionToken,
        code: inputCode.trim()
      })
    });
    ...
    if (!resp.ok || !data.ok) {
      ...
      // Mettre à jour les tentatives locales si le serveur les retourne
      if (data.attempts !== undefined) {
        OTP_CONFIG.attempts = data.attempts;
        sessionStorage.setItem('_otp_attempts', String(data.attempts));
      }
      return { ok: false, msg: data.error || 'Erreur vérification' };
    }

    // ✅ Code valide — nettoyer
    OTP_CONFIG.sessionToken = null;
    sessionStorage.removeItem('_otp_sessionToken');
    sessionStorage.removeItem('_otp_vehicle');
    sessionStorage.removeItem('_otp_attempts');

    return { ok: true };
  } catch (e) {
    console.error('OTP verify network error:', e);
    return { ok: false, msg: 'Erreur réseau' };
  }
}
...
async function validateOTPAndLogin() {
  ...
  if (code.length < 6) { console.log('⛔ Code trop court'); validateOTPAndLogin._running = false; return; }
  ...
  if (result.ok) {
    ...
    // Récupérer le nombre de tentatives depuis le serveur ou local
    const usedIdx = OTP_CONFIG.attempts;
...
async function resendOTPWhatsApp() {
  ...
  // Nouveau code (le serveur génère un nouveau OTP)
