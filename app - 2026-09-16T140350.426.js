...
// 🔒 Clé d'application envoyée au Worker (header X-App-Key). DOIT être identique
// au secret APP_KEY configuré côté Cloudflare Worker.
const APP_KEY = 'cd0fb78d46e86d8d8cf493cea6378e1495985edc3038a0c58e7926e69d8c3b16';
// BLOC ADDITIF — exposer sur window pour que demande_travaux_v2.js puisse y accéder
...
class ParcAutoApp {
  ...
  async writeToCloud() {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        console.log(`Écriture cloud via Worker (tentative ${attempt + 1}/3)...`);
        const controller = new AbortController();
        const timeout = setTimeout(() => controller.abort(), 30000); // 30s

        // ⚠️ Ne jamais envoyer currentUser/userVehicle au cloud
        // activityLogs = INCLUS pour que l'admin voie l'activité des chauffeurs
        const { currentUser: _cu, userVehicle: _uv, ...cloudPayload } = this.data;
        const response = await fetch(WORKER_URL, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'X-Action': 'update', 'X-App-Key': APP_KEY },
        ...
        if (!response.ok) {
          console.warn(`Erreur écriture Worker (tentative ${attempt + 1}):`, response.status);
        ...
        const reason = e.name === 'AbortError' ? 'Timeout 12s' : e;
        console.warn(`Erreur écriture Worker (tentative ${attempt + 1}):`, reason);
        if (attempt < 2) await new Promise(r => setTimeout(r, 2000 * (attempt + 1)));
      }
    }
    console.warn('❌ Sync cloud échouée après 3 tentatives');
    return false;
  }
  ...
  async loadData() {
    ...
    // Si session admin active → lire le cloud pour avoir les dernières modifs des chauffeurs
    if (localData && localData.currentUser && localData.currentUser.role === 'admin') {
      if (this.isCloudConfigured()) {
        try {
          ...
          if (cloudData && cloudData.vehicles) {
            ...
            if (cloudTs >= localTs) {
              console.log('Admin: cloud plus récent ou égal au local — chargement cloud');
              this.isCloudSync = true;
              // Session = toujours depuis le LOCAL (jamais depuis le cloud)
              cloudData.currentUser = localData.currentUser;
              cloudData.userVehicle = localData.userVehicle || null;
              // 🔧 FIX: Fusionner les logs cloud + locaux par id unique
              const _localLogs = localData.activityLogs || [];
              const _cloudLogs = cloudData.activityLogs || [];
              const _mergedLogs = [..._localLogs];
              const _existingIds = new Set(_mergedLogs.map(l => l.id));
              for (const cl of _cloudLogs) {
                if (!_existingIds.has(cl.id)) _mergedLogs.push(cl);
              }
              _mergedLogs.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
              cloudData.activityLogs = _mergedLogs.slice(0, 500);
              cloudData.settings = Object.assign(
                { alerteVidange: 1000, alerteChaine: 1000, alerteVisite: 7, entreprise: 'Tunisie Telecom' },
                cloudData.settings || {}
              );
              // 🛡️ Dédupliquer les véhicules du cloud
              if (cloudData.vehicles) {
                const seen = new Map();
                cloudData.vehicles = cloudData.vehicles.filter(v => {
                  if (seen.has(v.matricule)) return false;
                  seen.set(v.matricule, true);
                  return true;
                });
              }
              return cloudData;
          ...
          console.warn('Admin: erreur lecture cloud au chargement, fallback local', e);
        }
      }
    ...
    if (this.isCloudConfigured()) {
      ...
      if (cloudData && cloudData.vehicles) {
        ...
        if (localTs > cloudTs) {
          ...
          this.isCloudSync = true;
          // BLOC ADDITIF — fusionner demandesTravaux avant de pousser le local au démarrage
          // (évite d'écraser une validation faite ailleurs entre la fermeture et la réouverture)
          const byId = new Map();
          (cloudData.demandesTravaux || []).forEach(d => byId.set(d.id, d));
          (localData.demandesTravaux || []).forEach(d => {
            const existing = byId.get(d.id);
            const tExisting = existing ? (existing.dateTraitement ? new Date(existing.dateTraitement).getTime()
                              : (existing.dateDemande ? new Date(existing.dateDemande).getTime() : 0)) : -1;
            const tLocal = d.dateTraitement ? new Date(d.dateTraitement).getTime()
                          : (d.dateDemande ? new Date(d.dateDemande).getTime() : 0);
            if (!existing || tLocal > tExisting) byId.set(d.id, d);
          });
          localData.demandesTravaux = Array.from(byId.values());
          // FIN BLOC ADDITIF
        ...
        this.isCloudSync = true;
        // Session = toujours depuis le LOCAL
        if (localData) {
          cloudData.currentUser = localData.currentUser || null;
          cloudData.userVehicle = localData.userVehicle || null;
        }
        cloudData.settings = Object.assign(
          { alerteVidange: 1000, alerteChaine: 1000, alerteVisite: 7, entreprise: 'Tunisie Telecom' },
  ...
  async saveData(modifiedVehicleId) {
    this.data._lastSaved = Date.now();
    // Horodater le véhicule spécifique modifié (pour merge partiel dans autoSync)
    if (modifiedVehicleId) {
      const v = this.data.vehicles?.find(v => v.id === modifiedVehicleId);
      if (v) v._lastModified = Date.now();
    }
    const _userVehicle = this.data.userVehicle;
    const _currentUser = this.data.currentUser;

    // 1. Sauvegarder IMMÉDIATEMENT en local
    try {
    ...
    // 2. Sync cloud fire & forget avec verrou anti-écrasement
    if (this.isCloudConfigured()) {
      this._isSaving = true;
      ...
      this._saveQueue = (this._saveQueue || Promise.resolve())
        ...
        .then(async () => {
          try {
            ...
            this.data.userVehicle = this.data.userVehicle || _userVehicle;
            this.data.currentUser = this.data.currentUser || _currentUser;
            if (success) {
              this.showSyncStatus('synced');
              this._pendingSync = false;
            } else {
              console.warn('Sync cloud échouée — retry dans 15s');
              this.showSyncStatus('error');
              this._pendingSync = true;
              // Réessayer automatiquement dans 15 secondes
              setTimeout(async () => {
                if (this._pendingSync && !this._isSaving) {
                  console.log('🔄 Retry sync cloud...');
                  this.showSyncStatus('syncing');
                  const retry = await this.writeToCloud().catch(() => false);
                  this.showSyncStatus(retry ? 'synced' : 'error');
                  if (retry) this._pendingSync = false;
                }
              }, 15000);
            }
          } catch (e) {
  ...
  async init() {
    ...
    if (this.data.currentUser) {
      this.startAutoSync(); // admin ET chauffeur
    }
  }
  ...
  startAutoSync() {
    ...
    this._autoSyncInterval = setInterval(() => this._cloudSyncCheck(), 30000);

    // 🚨 Vérifier les chauffeurs inactifs avec alertes (toutes les 2 heures)
    setInterval(() => {
      this.checkInactiveDriversWithAlerts();
    }, 7200000);

    // Vérification immédiate au démarrage
    setTimeout(() => this.checkInactiveDriversWithAlerts(), 30000);

    // BLOC ADDITIF — resynchro immédiate quand l'appli revient au premier plan.
    // Sur Android, le navigateur suspend les setInterval en arrière-plan / écran
    // verrouillé : sans ce correctif, un compte (souvent l'admin, ouvert brièvement
    // puis mis en veille) peut rester bloqué sur d'anciennes données jusqu'à ce que
    // le timer de 30s se réexécute, ce qui peut prendre longtemps voire ne jamais
    // arriver si l'onglet reste suspendu.
    if (!this._foregroundResyncBound) {
      this._foregroundResyncBound = true;
      document.addEventListener('visibilitychange', () => {
        if (document.visibilityState === 'visible') this._cloudSyncCheck();
      });
      window.addEventListener('pageshow', () => this._cloudSyncCheck());
    }
    // FIN BLOC ADDITIF
  }

  // BLOC ADDITIF — logique du check de sync extraite dans une méthode nommée
  // (identique à l'ancien callback du setInterval, juste réutilisable) pour pouvoir
  // être appelée aussi hors du timer, notamment au retour au premier plan sur Android
  async _cloudSyncCheck() {
      // 🔒 Ne pas écraser si une sauvegarde est en cours
      if (this._isSaving) {
        console.log('AutoSync: sauvegarde en cours, ignorée');
        return;
      }

      const cloudData = await this.readFromCloud();

      // 🔒 Re-vérifier après la requête réseau (peut avoir changé pendant le GET)
      if (this._isSaving) return;

      if (cloudData && cloudData.vehicles) {
        const cloudTs = cloudData._lastSaved || 0;
        const localTs = this.data._lastSaved || 0;

        // BLOC ADDITIF — fusionner demandesTravaux par demande individuelle, quel que soit
        // le sens de la synchro (évite qu'un appareil resté ouvert n'écrase la validation
        // faite ailleurs entre-temps : on garde toujours la version la plus à jour par id)
        const mergeDemandesTravaux = (localList, cloudList) => {
          const byId = new Map();
          (cloudList || []).forEach(d => byId.set(d.id, d));
          (localList || []).forEach(d => {
            const existing = byId.get(d.id);
            if (!existing) { byId.set(d.id, d); return; }
            // Comparer par dateTraitement (validation/rejet) puis dateDemande en repli
            const tExisting = existing.dateTraitement ? new Date(existing.dateTraitement).getTime()
                              : (existing.dateDemande ? new Date(existing.dateDemande).getTime() : 0);
            const tLocal = d.dateTraitement ? new Date(d.dateTraitement).getTime()
                          : (d.dateDemande ? new Date(d.dateDemande).getTime() : 0);
            if (tLocal > tExisting) byId.set(d.id, d);
          });
          return Array.from(byId.values());
        };
        const mergedDemandesTravaux = mergeDemandesTravaux(this.data.demandesTravaux, cloudData.demandesTravaux);
        // FIN BLOC ADDITIF

        if (localTs > cloudTs) {
          console.log('AutoSync: local plus récent que cloud — repush');
          // BLOC ADDITIF — préserver les demandes de travaux fusionnées avant le repush
          this.data.demandesTravaux = mergedDemandesTravaux;
          // FIN BLOC ADDITIF
          this.writeToCloud().catch(() => {});
          return;
        }
        ...
        if (cloudTs > localTs) {
          // Cloud plus récent → merge par véhicule (évite d'écraser les modifs locales simultanées)
          const localVehicleMap = {};
          (this.data.vehicles || []).forEach(v => { localVehicleMap[v.id] = v; });

          cloudData.vehicles = (cloudData.vehicles || []).map(cv => {
            const lv = localVehicleMap[cv.id];
            // Garder la version locale si elle a un timestamp individuel plus récent
            if (lv && lv._lastModified && cv._lastModified && lv._lastModified > cv._lastModified) {
              return lv;
            }
            return cv;
          });

          // 🛡️ Dédupliquer les véhicules après merge
          const seenIds = new Map();
          cloudData.vehicles = (cloudData.vehicles || []).filter(v => {
            if (seenIds.has(v.matricule)) return false;
            seenIds.set(v.matricule, true);
            return true;
          });

          // ⚠️ Toujours préserver la session locale
          const localUserVehicle = this.data.userVehicle;
          const localCurrentUser = this.data.currentUser;
          const localActivityLogs = this.data.activityLogs || [];
          const cloudActivityLogs = cloudData.activityLogs || [];

          this.data = cloudData;
          this.data.userVehicle = localUserVehicle;
          this.data.currentUser = localCurrentUser;
          // BLOC ADDITIF — appliquer la fusion des demandes de travaux (déjà calculée plus haut)
          this.data.demandesTravaux = mergedDemandesTravaux;
          // FIN BLOC ADDITIF

          // 🔧 FIX: Fusionner les logs cloud + locaux par id unique (évite les doublons)
          const mergedLogs = [...localActivityLogs];
          const existingIds = new Set(mergedLogs.map(l => l.id));
          for (const cl of cloudActivityLogs) {
            if (!existingIds.has(cl.id)) {
              mergedLogs.push(cl);
            }
          }
          // Trier par timestamp décroissant, garder 500 max
          mergedLogs.sort((a, b) => (b.timestamp || 0) - (a.timestamp || 0));
          this.data.activityLogs = mergedLogs.slice(0, 500);
          try {
            localStorage.setItem('parcAutoData_v3', JSON.stringify(this.data));
          } catch(e) {}
          // Re-render selon le rôle
...
const user = ADMIN_USERS.find(u => u.email === email && u.password === password);
      ...
      if (user) {
        ...
        this._pendingAdminUser = user;
        await this.sendAdminOtp();
      } else {
  ...
  // ============================================
  // OTP ADMIN (2e étape de connexion)
  // ============================================
  generateAdminOtp() {
  ...
  async sendAdminOtp() {
    const user = this._pendingAdminUser;
    if (!user) return;

    // OTP local (utilisé seulement pour le fallback WhatsApp)
    const code = this.generateAdminOtp();
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        this.sendAdminOtpViaWhatsApp(code, user);
        return;
      }

      const result = await resp.json();
      // Le Worker génère et envoie son propre code ; on garde le sessionToken
      // pour la vérification côté serveur (/verify-otp)
      this._adminOtp.sessionToken = result.sessionToken;
      this._adminOtp.serverVerified = true;
  ...
  // 🔧 Fallback: si l'email OTP échoue, envoyer le code via WhatsApp
  // (le code reste obligatoire — pas de connexion directe)
  sendAdminOtpViaWhatsApp(code, user) {
    // WhatsApp abandonné — afficher message d'erreur clair
    this.showToast('❌ Erreur envoi OTP par email. Vérifiez la configuration Resend.', 'error');
    this._pendingAdminUser = null;
  }
  ...
  async loginAdminDirect() {
    ...
    localData.currentUser = this.data.currentUser;
    localData.userVehicle = null;
    localStorage.setItem('parcAutoData_v3', JSON.stringify(localData));

    this.logActivity('ADMIN', 'login', {
      chauffeur: user.name,
      note: 'Connexion administrateur (fallback — OTP indisponible)'
    });
    await this.saveData();
    ...
    inputs.forEach((inp, idx) => {
      inp.oninput = () => {
        ...
        if (inp.value && idx === inputs.length - 1) {
          ...
          if (all.length === 6) this.verifyAdminOtp();
        }
      };
      inp.onkeydown = (e) => {
  ...
  startOtpTimer() {
    ...
    const update = () => {
      ...
      timerEl.textContent = `Code valable encore ${min}:${sec}`;
      if (remaining <= 0) {
        clearInterval(this._otpTimerInterval);
        resendBtn.disabled = false;
        timerEl.textContent = 'Le code a expiré — renvoyez un nouveau code';
      }
      remaining--;
    };
    update();
    this._otpTimerInterval = setInterval(update, 1000);
  }
  ...
  async verifyAdminOtp() {
    ...
    if (entered.length !== 6) {
      errorEl.textContent = 'Veuillez saisir les 6 chiffres';
      return;
    }

    if (!this._adminOtp) {
      errorEl.textContent = 'Session expirée, veuillez réessayer';
      return;
    }

    if (Date.now() > this._adminOtp.expiresAt) {
      errorEl.textContent = 'Code expiré — cliquez sur "Renvoyer le code"';
      return;
    }

    this._adminOtp.attempts++;
    if (this._adminOtp.attempts > this._adminOtp.maxAttempts) {
      errorEl.textContent = '🔒 Trop de tentatives. Veuillez relancer la connexion.';
      this.closeOtpModal();
      return;
    }

    let valid = false;
    if (this._adminOtp.serverVerified && this._adminOtp.sessionToken) {
      try {
        const resp = await fetch(WORKER_URL + '/verify-otp', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'X-App-Key': APP_KEY },
          body: JSON.stringify({ sessionToken: this._adminOtp.sessionToken, code: entered })
        });
        const result = await resp.json().catch(() => ({}));
        valid = resp.ok && result.ok;
        if (!valid && result.error) errorEl.textContent = `❌ ${result.error}`;
      } catch (e) {
        errorEl.textContent = '❌ Erreur de vérification (réseau). Réessayez.';
        return;
      }
    } else {
      // Fallback WhatsApp : vérification locale du code généré côté client
      valid = (entered === this._adminOtp.code);
      if (!valid) errorEl.textContent = `❌ Code incorrect (tentative ${this._adminOtp.attempts}/${this._adminOtp.maxAttempts})`;
    }

    if (!valid) {
      if (!errorEl.textContent) errorEl.textContent = `❌ Code incorrect (tentative ${this._adminOtp.attempts}/${this._adminOtp.maxAttempts})`;
      inputs.forEach(inp => inp.value = '');
      inputs[0].focus();
      return;
    }

    // ✅ OTP correct — connexion admin
    const user = this._pendingAdminUser;
    this.data.currentUser = { email: user.email, role: user.role, name: user.name };
    this.data.userVehicle = null;
    ...
    this.logActivity('ADMIN', 'login', {
      chauffeur: user.name,
      note: 'Connexion administrateur (OTP vérifié)'
    });
    await this.saveData();
  ...
  initUserLogin() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      // Recharger depuis le cloud si configuré (forcer pour avoir les dernières modifs admin)
      if (this.isCloudConfigured()) {
        try {
          const cloudData = await this.readFromCloud();
          if (cloudData && cloudData.vehicles) {
            ...
            // Mettre à jour localStorage avec les données fraîches du cloud
            try {
              const localRaw = JSON.parse(localStorage.getItem('parcAutoData_v3') || '{}');
              localRaw.vehicles = cloudData.vehicles;
              localRaw.repairs  = cloudData.repairs  || [];
              localRaw.settings = cloudData.settings || localRaw.settings || {};
              localRaw._lastSaved = cloudData._lastSaved || localRaw._lastSaved;
              localStorage.setItem('parcAutoData_v3', JSON.stringify(localRaw));
            } catch(e) {}
          }
        } catch(e) {
          console.warn('Login utilisateur: erreur lecture cloud, fallback local', e);
        }
      }
      ...
      // 📝 Logger la connexion chauffeur
      this.logActivity(vehicle.matricule, 'login', {
        chauffeur: vehicle.chauffeur || 'Utilisateur',
        matriculeAgent: vehicle.matriculeAgent || null,
        note: 'Connexion chauffeur'
      });
  ...
  initForms() {
    ...
    // 💡 BLOC ADDITIF — pré-suggestion des prochaines échéances (Vidange / Kit
    // Chaîne) dès que le type d'intervention et le kilométrage sont saisis.
    // Ne fait que PRÉ-REMPLIR les champs "Mise à jour automatique des
    // échéances" (déjà éditables manuellement) — l'utilisateur garde la main
    // et peut ajuster ou vider la valeur avant d'enregistrer. Ne pré-remplit
    // jamais un champ que l'utilisateur a déjà modifié lui-même.
    // Intervalles par défaut (ajustables ici si besoin) :
    const REPAIR_AUTO_INTERVAL_KM = { 'Vidange': 10000, 'Kit Chaîne': 60000 };
    const repairTypeEl = document.getElementById('repair-type');
    const repairKmEl = document.getElementById('repair-km');
    const nextVidangeEl = document.getElementById('repair-next-vidange');
    const nextChaineEl = document.getElementById('repair-next-chaine');
    const suggestNextEcheance = () => {
      const type = repairTypeEl?.value;
      const km = parseInt(repairKmEl?.value) || 0;
      if (!km) return;
      if (type === 'Vidange' && nextVidangeEl && !nextVidangeEl.dataset.userEdited) {
        nextVidangeEl.value = km + REPAIR_AUTO_INTERVAL_KM['Vidange'];
      }
      if (type === 'Kit Chaîne' && nextChaineEl && !nextChaineEl.dataset.userEdited) {
        nextChaineEl.value = km + REPAIR_AUTO_INTERVAL_KM['Kit Chaîne'];
      }
    };
    if (repairTypeEl) repairTypeEl.addEventListener('change', suggestNextEcheance);
    if (repairKmEl) repairKmEl.addEventListener('input', suggestNextEcheance);
    if (nextVidangeEl) nextVidangeEl.addEventListener('input', () => { nextVidangeEl.dataset.userEdited = '1'; });
    if (nextChaineEl) nextChaineEl.addEventListener('input', () => { nextChaineEl.dataset.userEdited = '1'; });
    if (repairForm) {
      repairForm.addEventListener('reset', () => {
        if (nextVidangeEl) delete nextVidangeEl.dataset.userEdited;
        if (nextChaineEl) delete nextChaineEl.dataset.userEdited;
      });
    }
    // FIN BLOC ADDITIF
  ...
  // ALERTES
  // ============================================
  // Génère un identifiant unique pour une alerte (vehicule + type + valeur seuil)
  _alertKey(vehicleId, type) {
    return `${vehicleId}__${type}`;
  }

  getAlerts(includeDismissed = false) {
    const alerts = [];
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const settings = this.data.settings || { alerteVidange: 1000, alerteChaine: 1000, alerteVisite: 7 };
    const dismissed = this.data.dismissedAlerts || {};
    ...
    this.data.vehicles.forEach(v => {
      if (v.prochaineVidange && v.km >= v.prochaineVidange - settings.alerteVidange) {
        ...
        const key = this._alertKey(v.id, 'Vidange');
        // L'alerte réapparaît automatiquement si le véhicule a été modifié depuis le masquage
        const isDismissed = dismissed[key] && dismissed[key].kmAtDismiss === v.km && dismissed[key].seuil === v.prochaineVidange;
      ...
      if (v.prochaineChaine && v.km >= v.prochaineChaine - settings.alerteChaine) {
        ...
        const key = this._alertKey(v.id, 'Kit Chaîne');
        const isDismissed = dismissed[key] && dismissed[key].kmAtDismiss === v.km && dismissed[key].seuil === v.prochaineChaine;
        if (includeDismissed || !isDismissed) {
          alerts.push({
            key,
            vehicle: v,
      ...
      if (v.prochaineVisite) {
        ...
        if (daysDiff <= settings.alerteVisite) {
          const key = this._alertKey(v.id, 'Visite Technique');
          const isDismissed = dismissed[key] && dismissed[key].dateAtDismiss === v.prochaineVisite;
          if (includeDismissed || !isDismissed) {
            alerts.push({
              key,
              vehicle: v,
  ...
  async dismissAlert(key) {
    ...
    // Retrouver le véhicule et type pour stocker les valeurs actuelles (permet le réveil auto)
    const [vehicleId, type] = key.split('__');
    const v = this.data.vehicles.find(x => x.id === vehicleId);
    if (!v) return;
    if (type === 'Vidange')          this.data.dismissedAlerts[key] = { kmAtDismiss: v.km, seuil: v.prochaineVidange };
    else if (type === 'Kit Chaîne')  this.data.dismissedAlerts[key] = { kmAtDismiss: v.km, seuil: v.prochaineChaine };
    else if (type === 'Visite Technique') this.data.dismissedAlerts[key] = { dateAtDismiss: v.prochaineVisite };
    await this.saveData();
    this.renderAlertsTable();
    this.updateAlertCounts();
    this.renderDashboardAlerts();
    this.showToast('✅ Alerte masquée — réapparaîtra si le véhicule change', 'success');
  }

  async restoreAllAlerts() {
    this.data.dismissedAlerts = {};
    await this.saveData();
    this.renderAlertsTable();
    this.updateAlertCounts();
    this.renderDashboardAlerts();
    this.showToast('🔄 Toutes les alertes restaurées', 'success');
  }
  ...
  renderAlertsTable() {
    ...
    // Compter les alertes masquées (pour info)
    const dismissed = this.data.dismissedAlerts || {};
    const dismissedCount = Object.keys(dismissed).length;

    if (alerts.length === 0) {
      const msg = dismissedCount > 0
        ? `<div style="font-size:40px;margin-bottom:8px;">✅</div><strong>Aucune alerte active</strong><br><span style="font-size:13px;">${dismissedCount} alerte(s) masquée(s) — <a href="#" style="color:var(--primary);" onclick="event.preventDefault();parcAuto.restoreAllAlerts()">tout restaurer</a></span>`
        : `<div style="font-size:40px;margin-bottom:8px;">✅</div><strong>Aucune alerte</strong><br><span style="font-size:13px;">Tous les véhicules sont à jour</span>`;
      tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;padding:40px;color:var(--secondary);">${msg}</td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = alerts.map(a => `
      <tr>
        ...
        <td>
          <button
            class="btn btn-sm"
            style="background:#fee2e2;color:#dc2626;border:1px solid #fca5a5;padding:4px 10px;font-size:12px;"
            onclick="parcAuto.dismissAlert('${a.key}')"
            title="Masquer cette alerte — réapparaîtra automatiquement si le km ou la date change">
            🗑️ Masquer
          </button>
        </td>
      </tr>
  ...
  openVehicleModal(vehicleId = null) {
    ...
    if (vehicleId) {
      ...
      if (v) {
        ...
        // BLOC ADDITIF — Ancienne vidange (index + date)
        const vvAncienIndex = document.getElementById('vehicle-vidange-ancien-index');
        if (vvAncienIndex) vvAncienIndex.value = v.ancienneVidangeIndex || '';
        const vvAncienDate = document.getElementById('vehicle-vidange-ancien-date');
        if (vvAncienDate) vvAncienDate.value = v.ancienneVidangeDate || '';
        // FIN BLOC ADDITIF
        ...
        document.getElementById('vehicle-batterie-index').value = v.indiceBatterie || '';
        document.getElementById('vehicle-pneus-date').value = v.dateChangementPneus || '';
        // 📇 Caractéristiques carte grise (purement additif)
        const cgConstructeur = document.getElementById('vehicle-constructeur');
        if (cgConstructeur) cgConstructeur.value = v.constructeur || '';
        const cgType = document.getElementById('vehicle-type-commercial');
        if (cgType) cgType.value = v.typeCommercial || '';
        const cgNumSerie = document.getElementById('vehicle-num-serie');
        if (cgNumSerie) cgNumSerie.value = v.numSerie || '';
        const cgDateCirc = document.getElementById('vehicle-date-circulation');
        if (cgDateCirc) cgDateCirc.value = v.dateCirculation || '';
        const cgGenre = document.getElementById('vehicle-genre');
        if (cgGenre) cgGenre.value = v.genre || '';
        const cgEnergie = document.getElementById('vehicle-energie');
        if (cgEnergie) cgEnergie.value = v.energie || '';
      }
    } else {
  ...
  async saveVehicle() {
    ...
    const vehicle = {
      ...
      // BLOC ADDITIF — Ancienne vidange (index + date), purement informatif
      ancienneVidangeIndex: parseInt(document.getElementById('vehicle-vidange-ancien-index')?.value) || null,
      ancienneVidangeDate: document.getElementById('vehicle-vidange-ancien-date')?.value || null,
      // FIN BLOC ADDITIF
      ...
      indiceBatterie: document.getElementById('vehicle-batterie-index').value.trim() || null,
      dateChangementPneus: document.getElementById('vehicle-pneus-date').value || null,
      // 📇 Caractéristiques carte grise (purement additif — ne casse rien si champs absents du formulaire)
      constructeur: document.getElementById('vehicle-constructeur')?.value?.trim() || null,
      typeCommercial: document.getElementById('vehicle-type-commercial')?.value?.trim() || null,
      numSerie: document.getElementById('vehicle-num-serie')?.value?.trim() || null,
      dateCirculation: document.getElementById('vehicle-date-circulation')?.value || null,
      genre: document.getElementById('vehicle-genre')?.value?.trim() || null,
      energie: document.getElementById('vehicle-energie')?.value?.trim() || null,
      statut: 'actif'
    };

    if (id) {
      // Chercher d'abord par id, puis par matricule en fallback
      let index = this.data.vehicles.findIndex(v => v.id === id);
      if (index === -1) index = this.data.vehicles.findIndex(v => v.matricule === vehicle.matricule);
      if (index !== -1) {
        this.data.vehicles[index] = vehicle;
      } else {
        // Vraiment nouveau — vérifier qu'il n'existe pas déjà par matricule
        const dupIdx = this.data.vehicles.findIndex(v => v.matricule === vehicle.matricule);
        if (dupIdx === -1) this.data.vehicles.push(vehicle);
        else this.data.vehicles[dupIdx] = vehicle; // mettre à jour le doublon existant
      }
    } else {
      // Nouveau véhicule — vérifier doublon par matricule
      const dupIdx = this.data.vehicles.findIndex(v => v.matricule === vehicle.matricule);
      if (dupIdx === -1) {
        this.data.vehicles.push(vehicle);
      } else {
        // Matricule existe déjà — mettre à jour au lieu de créer un doublon
        vehicle.id = this.data.vehicles[dupIdx].id; // garder l'id existant
        this.data.vehicles[dupIdx] = vehicle;
      }
    }

    // 🛡️ Dédupliquer par matricule (sécurité anti-doublon)
    const seen = new Map();
  ...
  renderRepairsTable() {
    ...
    tbody.innerHTML = repairs.map(r => `
      <tr>
        ...
        <td>
          <button class="action-btn edit" onclick="parcAuto.editRepair('${r.id}')">✏️</button>
  ...
  async saveRepair() {
    ...
    // BLOC ADDITIF — mode édition : si repair-id est renseigné, on modifie
    // l'intervention existante au lieu d'en créer une nouvelle
    const editId = document.getElementById('repair-id')?.value || '';
    ...
    if (vehicle) {
      // 🛡️ BLOC ADDITIF — ne mettre à jour le KM/échéances du véhicule que si cette
      // intervention est bien la plus récente (en KM) pour ce véhicule. Sans ce garde-fou,
      // modifier une ancienne intervention (ou en scanner une en désordre) ferait "reculer"
      // le kilométrage affiché dans Parc Véhicules.
      const otherRepairsKmMax = Math.max(
        0,
        ...this.data.repairs.filter(rp => rp.matricule === matricule && rp.id !== repair.id).map(rp => rp.km || 0)
      );
      if (repair.km >= otherRepairsKmMax) {
      ...
      // 🔋 BLOC ADDITIF — synchronisation Batterie / Pneus vers la fiche véhicule
      // (onglet "Mon véhicule"). Quand l'intervention enregistrée est de type
      // "Batterie" ou "Pneus", on répercute automatiquement la date sur
      // vehicle.dateChangementBatterie / vehicle.dateChangementPneus — au lieu
      // de devoir aller les modifier manuellement dans la fiche véhicule.
      // Même garde-fou anti-régression que pour le KM : on ne met à jour que si
      // cette intervention est bien la plus récente (en date) de son type pour
      // ce véhicule, pour ne pas "reculer" la date en cas de modification d'une
      // ancienne intervention ou d'un import dans le désordre.
      if (repair.date && repair.type === 'Batterie') {
        const otherDatesMax = this.data.repairs
          .filter(rp => rp.matricule === matricule && rp.id !== repair.id && rp.type === 'Batterie' && rp.date)
          .reduce((max, rp) => (rp.date > max ? rp.date : max), '');
        if (repair.date >= otherDatesMax) {
          vehicle.dateChangementBatterie = repair.date;
        }
      }
      if (repair.date && repair.type === 'Pneus') {
        const otherDatesMax = this.data.repairs
          .filter(rp => rp.matricule === matricule && rp.id !== repair.id && rp.type === 'Pneus' && rp.date)
          .reduce((max, rp) => (rp.date > max ? rp.date : max), '');
        if (repair.date >= otherDatesMax) {
          vehicle.dateChangementPneus = repair.date;
        }
      }
      // FIN BLOC ADDITIF
    }
    ...
    if (editId) {
      const idx = this.data.repairs.findIndex(rp => rp.id === editId);
      if (idx !== -1) this.data.repairs[idx] = repair;
      else this.data.repairs.push(repair);
    } else {
    ...
    const titleEl = document.getElementById('repair-form-title');
    if (titleEl) titleEl.textContent = '🔧 Nouvelle intervention';
    this.showToast(editId ? 'Intervention modifiée avec succès' : 'Intervention enregistrée et véhicule mis à jour', 'success');
    this.showTab('repairs');
  }

  // BLOC ADDITIF — édition d'une intervention existante
  editRepair(id) {
    const r = this.data.repairs.find(rp => rp.id === id);
    if (!r) return;
    document.getElementById('repair-id').value = r.id;
    document.getElementById('repair-matricule').value = r.matricule;
    document.getElementById('repair-type').value = r.type;
    document.getElementById('repair-date').value = r.date;
    document.getElementById('repair-km').value = r.km;
    document.getElementById('repair-designation').value = r.designation;
    document.getElementById('repair-montant').value = r.montant || '';
    document.getElementById('repair-chauffeur').value = r.chauffeur || '';
    document.getElementById('repair-next-vidange').value = r.nextVidange || '';
    document.getElementById('repair-next-chaine').value = r.nextChaine || '';
    document.getElementById('repair-next-visite').value = r.nextVisite || '';
    const titleEl = document.getElementById('repair-form-title');
    if (titleEl) titleEl.textContent = '✏️ Modifier l\'intervention';
    this.showTab('add-repair');
  }
  // FIN BLOC ADDITIF
  ...
  renderUserVehicleDetail() {
    ...
    // 🔧 FIX: Recharger depuis localStorage si le véhicule a disparu (écrasement cloud)
    if (!this.data.userVehicle || !this.data.vehicles) {
      try {
        const saved = localStorage.getItem('parcAutoData_v3');
        if (saved) {
          const localData = JSON.parse(saved);
          if (localData.userVehicle) {
            this.data.userVehicle = localData.userVehicle;
          }
          if (localData.vehicles) {
            this.data.vehicles = localData.vehicles;
          }
        }
      } catch(e) {}
    }
    ...
    // 📝 Logger la consultation (une seule fois par session, pas à chaque re-render)
    if (matricule && !this._consultationLogged) {
      this._consultationLogged = true;
      this.logActivity(matricule, 'consultation', {
        chauffeur: this.data.currentUser?.name,
        note: 'Consultation fiche véhicule'
      });
    }
    if (!matricule) {
        ...
        <div class="vehicle-detail-body">
          ...
          <div class="detail-section">
            ...
            <div class="detail-grid">
              <div class="detail-item">
                ...
                <div class="detail-item-value" style="display:flex;align-items:center;gap:10px;">
                  ...
                  <button onclick="openUserKmModal('${vehicle.matricule}', ${vehicle.km})"
                    ontouchend="event.preventDefault();openUserKmModal('${vehicle.matricule}', ${vehicle.km});"
                    style="background:linear-gradient(135deg,#10b981,#059669);color:white;border:none;
                    border-radius:7px;padding:8px 16px;font-size:12px;font-weight:700;cursor:pointer;
                    box-shadow:0 2px 8px rgba(16,185,129,0.3);min-height:36px;-webkit-tap-highlight-color:transparent;">
                    ✏️ Modifier
                  </button>
                </div>
              </div>
              <!-- BLOC ADDITIF — Ancienne vidange (index + date) : purement informatif -->
              <div class="detail-item">
                <div class="detail-item-label">Ancienne vidange</div>
                <div class="detail-item-value">${vehicle.ancienneVidangeIndex ? vehicle.ancienneVidangeIndex.toLocaleString() + ' km' : 'Non renseigné'}${vehicle.ancienneVidangeDate ? ' — ' + vehicle.ancienneVidangeDate : ''}</div>
              </div>
              <!-- FIN BLOC ADDITIF -->
              <div class="detail-item">
              ...
              <div class="detail-item">
                ...
                <div class="detail-item-value ${visiteDaysLeft !== null && visiteDaysLeft <= 7 ? 'alert' : visiteDaysLeft !== null && visiteDaysLeft <= 30 ? 'warning' : 'ok'}" style="display:flex;align-items:center;gap:10px;">
                  <span id="user-visite-display">${vehicle.prochaineVisite || 'Non définie'}</span>
                  <button onclick="openUserVisiteModal('${vehicle.matricule}', '${vehicle.prochaineVisite || ''}')"
                    ontouchend="event.preventDefault();openUserVisiteModal('${vehicle.matricule}', '${vehicle.prochaineVisite || ''}');"
                    style="background:linear-gradient(135deg,#10b981,#059669);color:white;border:none;
                    border-radius:7px;padding:8px 16px;font-size:12px;font-weight:700;cursor:pointer;
                    box-shadow:0 2px 8px rgba(16,185,129,0.3);min-height:36px;-webkit-tap-highlight-color:transparent;">
                    ✏️ Modifier
              ...
              <div class="detail-item">
                ...
                <div class="detail-item-value">${vehicle.indiceBatterie || 'Non renseigné'}</div>
              </div>
              <div class="detail-item">
...
<div style="text-align:center;padding-top:16px;border-top:1px solid var(--border);margin-top:16px;display:flex;gap:12px;justify-content:center;flex-wrap:wrap;">
            ...
            <button onclick="exportVehicleHistoryCSV('${matricule}')"
              style="background:linear-gradient(135deg,#3b82f6,#2563eb);color:#fff;border:none;border-radius:10px;padding:11px 24px;font-size:13px;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:8px;">
...
${(()=>{ const d=JSON.parse(localStorage.getItem('parcAutoData_v3')||'{}'); return d.currentUser&&d.currentUser.role==='admin' ? `<button onclick="sendWhatsAppAlert('${matricule}')" style="background:linear-gradient(135deg,#25d366,#128c7e);color:#fff;border:none;border-radius:10px;padding:11px 24px;font-size:13px;font-weight:600;cursor:pointer;display:flex;align-items:center;gap:8px;">📲 Envoyer alerte WhatsApp</button>` : ''; })()}
  ...
  async syncNow() {
    ...
    if (this.isCloudConfigured()) {
      try {
        ...
        const cloudData = await Promise.race([this.readFromCloud(), timeoutPromise]);

        if (cloudData && cloudData.vehicles) {
          const cloudTs = cloudData._lastSaved || 0;
          const localTs = this.data._lastSaved || 0;
          if (localTs > cloudTs) {
            // BLOC ADDITIF — fusionner demandesTravaux avant le repush manuel
            const byId = new Map();
            (cloudData.demandesTravaux || []).forEach(d => byId.set(d.id, d));
            (this.data.demandesTravaux || []).forEach(d => {
              const existing = byId.get(d.id);
              const tExisting = existing ? (existing.dateTraitement ? new Date(existing.dateTraitement).getTime()
                                : (existing.dateDemande ? new Date(existing.dateDemande).getTime() : 0)) : -1;
              const tLocal = d.dateTraitement ? new Date(d.dateTraitement).getTime()
            ...
            this.data.demandesTravaux = Array.from(byId.values());
            // FIN BLOC ADDITIF
            await this.writeToCloud().catch(() => {});
            this.showSyncStatus('synced');
            this.showToast('✅ Données locales envoyées vers le cloud', 'success');
            return;
          }
          ...
          if (cloudTs > localTs) {
            ...
            this.data.userVehicle = localUserVehicle;
            this.data.currentUser = localCurrentUser;
            try { localStorage.setItem('parcAutoData_v3', JSON.stringify(this.data)); } catch(e) {}
            this.renderAll();
          ...
          this.showSyncStatus('synced');
          this.showToast('☁️ Synchronisation terminée', 'success');
          return;
        }
      } catch(e) {
        console.warn('Sync cloud échouée:', e.message);
        this.showSyncStatus('error');
        this.showToast('❌ Sync cloud échouée — données conservées localement', 'error');
        return;
      }
    }
    ...
    this.renderAll();
    this.showSyncStatus('synced');
    this.showToast('✅ Données sauvegardées localement', 'success');
  }
  ...
  // ============================================
  // EXPORT EXCEL (remplace l'export JSON dans l'UI)
  // ============================================
  exportExcel() {
    if (!window.XLSX) {
      this.showToast('❌ Librairie Excel non chargée. Rafraîchissez la page.', 'error');
    ...
    // === Feuille Véhicules (mêmes colonnes que l'import Excel) ===
    const vehHeaders = ['Matricule', 'Matricule Agent', 'Modèle', 'Chauffeur', 'WhatsApp Chauffeur', 'KM', 'Prochaine Vidange', 'Prochaine Chaîne', 'Prochaine Visite', 'Date Changement Batterie', 'Indice Batterie', 'Date Changement Pneus', 'Statut'];
    const vehHint = ['(obligatoire)', '', '', '', '', '', '', '', '', '', '', '', ''];
    const vehRows = (this.data.vehicles || []).map(v => [
      v.matricule || '',
      v.matriculeAgent || '',
      v.modele || '',
      v.chauffeur || '',
      ...
      v.prochaineChaine || '',
      v.prochaineVisite || '',
      v.dateChangementBatterie || '',
      v.indiceBatterie || '',
      v.dateChangementPneus || '',
      v.statut || 'actif'
    ]);
    const wsVeh = XLSX.utils.aoa_to_sheet([vehHeaders, vehHint, ...vehRows]);
    XLSX.utils.book_append_sheet(wb, wsVeh, 'Véhicules');

    // === Feuille Réparations (mêmes colonnes que l'import Excel) ===
    const repHeaders = ['Matricule', 'Type', 'Désignation', 'Date', 'KM', 'Montant'];
    const repHint = ['(ex: 17-123456)', '', '', '', '', ''];
    const repRows = (this.data.repairs || []).map(r => [
      r.matricule || '',
      r.type || '',
      r.designation || '',
      r.date || '',
    ...
    const wsRep = XLSX.utils.aoa_to_sheet([repHeaders, repHint, ...repRows]);
    XLSX.utils.book_append_sheet(wb, wsRep, 'Réparations');

    XLSX.writeFile(wb, `parc-auto-drt-sfax-export-${new Date().toISOString().split('T')[0]}.xlsx`);
    this.showToast('✅ Export Excel effectué avec succès', 'success');
  }

  // ============================================
  // VÉRIFICATION DES DOUBLONS VÉHICULES
  // ============================================
  checkDoublons() {
    const vehicles = this.data.vehicles || [];
    const groups = [];

    // 1) Matricules strictement identiques
    const byMatricule = {};
    vehicles.forEach(v => {
      const m = (v.matricule || '').trim();
      if (!m) return;
      (byMatricule[m] = byMatricule[m] || []).push(v);
    });
    Object.values(byMatricule).forEach(g => {
      if (g.length > 1) groups.push({ reason: 'Matricule identique', vehicles: g });
    });

    // 2) Doublons probables : même KM + même WhatsApp chauffeur (saisie en double, matricule différent)
    const byKmWhatsapp = {};
    vehicles.forEach(v => {
      if (!v.whatsappChauffeur || !v.km) return;
      const key = v.km + '|' + v.whatsappChauffeur;
      (byKmWhatsapp[key] = byKmWhatsapp[key] || []).push(v);
    });
    Object.values(byKmWhatsapp).forEach(g => {
      if (g.length > 1) {
        const mats = new Set(g.map(v => v.matricule));
        if (mats.size > 1) groups.push({ reason: 'Même KM + même WhatsApp chauffeur (matricules différents)', vehicles: g });
      }
    });

    this.showDoublonsReport(groups);
  }

  showDoublonsReport(groups) {
    const existing = document.getElementById('doublons-report-overlay');
    if (existing) existing.remove();

    const overlay = document.createElement('div');
    overlay.id = 'doublons-report-overlay';
    ...
    let bodyHtml;
    if (groups.length === 0) {
      bodyHtml = '<p style="font-size:14px;color:#16a34a;font-weight:600;">✅ Aucun doublon détecté parmi les véhicules.</p>';
    } else {
      bodyHtml = groups.map(g => `
        <div style="margin-bottom:16px;padding:12px;border:1px solid #fca5a5;background:#fef2f2;border-radius:10px;">
          <div style="font-size:12px;font-weight:700;color:#b91c1c;margin-bottom:8px;">⚠️ ${g.reason}</div>
          ${g.vehicles.map(v => `
            <div style="font-size:13px;color:#334155;padding:4px 0;border-top:1px solid #fecaca;">
              <strong>${v.matricule || '—'}</strong> — ${v.modele || '—'} — ${v.chauffeur || '—'} — ${v.km || 0} km
            </div>
          `).join('')}
        </div>
      `).join('');
    ...
    overlay.innerHTML = `
      <div style="background:#fff;border-radius:16px;padding:24px;max-width:520px;width:100%;max-height:80vh;overflow-y:auto;box-shadow:0 24px 64px rgba(0,0,0,0.25);">
        <div style="font-size:17px;font-weight:700;color:#1e293b;margin-bottom:14px;">🔍 Vérification des doublons</div>
        ${bodyHtml}
        <button id="doublons-report-close" style="margin-top:8px;width:100%;padding:12px;border:none;border-radius:10px;background:#1E3A5F;color:#fff;font-size:14px;font-weight:700;cursor:pointer;">Fermer</button>
      </div>`;
    document.body.appendChild(overlay);
    document.getElementById('doublons-report-close').addEventListener('click', () => overlay.remove());
  }
  ...
  // ============================================
  // SYSTÈME DE LOGS D'ACTIVITÉ CHAUFFEURS
  // ============================================

  /**
   * Enregistre une action utilisateur dans les logs
   * @param {string} matricule - Matricule du véhicule
   * @param {string} action - Type d'action: 'consultation', 'modification_km', 'modification_visite', 'login'
   * @param {Object} details - Détails supplémentaires
   */
  logActivity(matricule, action, details = {}) {
    ...
    const logEntry = {
      ...
      matricule: matricule,
      action: action,
      chauffeur: details.chauffeur || (this.data.vehicles.find(v => v.matricule === matricule)?.chauffeur) || 'Inconnu',
      matriculeAgent: details.matriculeAgent || (this.data.vehicles.find(v => v.matricule === matricule)?.matriculeAgent) || null,
      details: details,
      deviceInfo: {
    ...
    this.data.activityLogs.unshift(logEntry);
    ...
    this.saveData();
    console.log('📝 Log activité:', action, '-', matricule);
  }

  /**
   * Vérifie si un chauffeur a consulté son véhicule récemment
   * @param {string} matricule 
   * @param {number} hoursAgo - Nombre d'heures à vérifier (défaut: 24)
   */
  hasDriverConsultedRecently(matricule, hoursAgo = 24) {
    if (!this.data.activityLogs) return false;
    const cutoff = Date.now() - (hoursAgo * 60 * 60 * 1000);
    return this.data.activityLogs.some(log => 
      log.matricule === matricule && 
  ...
  getActivityLogs(filters = {}) {
    ...
    if (filters.matricule) {
      logs = logs.filter(l => l.matricule === filters.matricule);
    }
    if (filters.action) {
      logs = logs.filter(l => l.action === filters.action);
    }
    if (filters.chauffeur) {
      logs = logs.filter(l => l.chauffeur?.toLowerCase().includes(filters.chauffeur.toLowerCase()));
    }
    if (filters.dateFrom) {
  ...
  /**
   * Vérifie les chauffeurs qui n'ont pas consulté leur véhicule
   * malgré des alertes actives, et envoie notification au chef
   */
  async checkInactiveDriversWithAlerts() {
    const alerts = this.getAlerts();
    ...
    for (const alert of urgentAlerts) {
      const matricule = alert.vehicle.matricule;
      const hasConsulted = this.hasDriverConsultedRecently(matricule, 48); // 48h

      if (!hasConsulted) {
        // Le chauffeur n'a pas consulté son véhicule depuis 48h malgré alerte urgente
        const vehicle = alert.vehicle;
        const today = new Date();

        // Vérifier si on a déjà notifié aujourd'hui (éviter spam)
        const todayStr = today.toISOString().split('T')[0];
        const dedupeKey = `chef_notif_${matricule}_${todayStr}`;
        if (localStorage.getItem(dedupeKey)) continue;

        // Envoyer WhatsApp au chef
        const msg = `🚨 *ALERTE CHAUFFEUR INACTIF — DRT SFAX*

📌 Véhicule : *${vehicle.matricule}*
🚙 Modèle : ${vehicle.modele}
👤 Chauffeur : ${vehicle.chauffeur || 'N/A'}
📱 WhatsApp : ${vehicle.whatsappChauffeur || 'Non enregistré'}

⚠️ *ALERTE URGENTE :* ${alert.type}
${alert.detail}

❌ *Le chauffeur n'a pas consulté son véhicule depuis 48h.*
...
_Parc Auto DRT Sfax — Tunisie Telecom_`;
        ...
        const waUrl = `https://wa.me/21698230530?text=${encodeURIComponent(msg)}`;

        // Notification cliquable
        if (Notification.permission === 'granted') {
          try {
            const notif = new Notification(`🚨 Chauffeur inactif — ${vehicle.matricule}`, {
              body: `${vehicle.chauffeur} n'a pas consulté son véhicule malgré alerte ${alert.type}`,
              icon: 'icon-192.png',
              tag: `inactive-${matricule}`,
        ...
        localStorage.setItem(dedupeKey, '1');
        notificationsSent.push({ matricule, chauffeur: vehicle.chauffeur, alert: alert.type });
      }
    }

    if (notificationsSent.length > 0) {
      console.log('Notifications chef envoyées:', notificationsSent);
    }
    return notificationsSent;
  }

  /**
   * Rendu du tableau des logs d'activité (interface admin)
   */
  renderActivityLogsTable() {
    ...
    const filters = {
      matricule: document.getElementById('log-filter-matricule')?.value || '',
      action: document.getElementById('log-filter-action')?.value || '',
      chauffeur: document.getElementById('log-filter-chauffeur')?.value || '',
      dateFrom: document.getElementById('log-filter-date-from')?.value || '',
      dateTo: document.getElementById('log-filter-date-to')?.value || ''
    };

    let logs = this.getActivityLogs(filters);

    // Pagination simple (50 par page)
    const pageSize = 50;
    const currentPage = parseInt(document.getElementById('logs-current-page')?.dataset?.page || '1');
    const totalPages = Math.ceil(logs.length / pageSize) || 1;
    const start = (currentPage - 1) * pageSize;
    const pageLogs = logs.slice(start, start + pageSize);

    if (pageLogs.length === 0) {
      tbody.innerHTML = `<tr><td colspan="6" style="text-align:center;padding:40px;color:var(--secondary);"><div style="font-size:40px;margin-bottom:8px;">📋</div><strong>Aucun log</strong><br><span style="font-size:13px;">Aucune activité enregistrée pour ces critères</span></td></tr>`;
      return;
    }

    const actionLabels = {
      'consultation': { label: '👁️ Consultation', color: '#3b82f6' },
      'modification_km': { label: '✏️ Modif. KM', color: '#10b981' },
      'modification_visite': { label: '📋 Modif. Visite', color: '#f59e0b' },
      'login': { label: '🔐 Connexion', color: '#8b5cf6' }
    };
    ...
    tbody.innerHTML = pageLogs.map(log => {
      ...
      if (log.details) {
        if (log.details.oldValue !== undefined && log.details.newValue !== undefined) {
          detailsHtml = `<span style="color:#64748b;font-size:12px;">${log.details.oldValue} → <strong style="color:#10b981;">${log.details.newValue}</strong></span>`;
        } else if (log.details.note) {
          detailsHtml = `<span style="color:#64748b;font-size:12px;">${log.details.note}</span>`;
        }
      }

      return `
        <tr>
          <td><strong>${dateStr}</strong><br><span style="font-size:11px;color:#94a3b8;">${timeStr}</span></td>
          <td><span style="display:inline-block;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;background:${actionInfo.color}15;color:${actionInfo.color};">${actionInfo.label}</span></td>
          <td><strong>${log.matricule}</strong></td>
          <td>${log.chauffeur || '-'}</td>
          <td>${log.matriculeAgent || '-'}</td>
          <td>${detailsHtml || '-'}</td>
        </tr>
      `;
    }).join('');
    ...
    if (pageInfo) pageInfo.textContent = `Page ${currentPage} / ${totalPages} (${logs.length} logs)`;
  }

  /**
   * Rendu du résumé d'activité pour le dashboard admin
   */
  renderActivitySummary() {
    ...
    const logs = this.data.activityLogs || [];

    // Stats du jour
    const todayLogs = logs.filter(l => l.timestamp >= todayTs);
    const todayConsultations = todayLogs.filter(l => l.action === 'consultation').length;
    const todayModifications = todayLogs.filter(l => l.action === 'modification_km' || l.action === 'modification_visite').length;

    // Stats de la semaine
    const weekLogs = logs.filter(l => l.timestamp >= weekAgo);
    const weekConsultations = weekLogs.filter(l => l.action === 'consultation').length;

    // Véhicules avec alerte non consultés aujourd'hui
    const alerts = this.getAlerts().filter(a => a.priority === 'urgent');
    let nonConsultedCount = 0;
    const nonConsultedList = [];

    for (const alert of alerts) {
      if (!this.hasDriverConsultedRecently(alert.vehicle.matricule, 24)) {
        nonConsultedCount++;
        nonConsultedList.push(alert.vehicle);
      }
    }

    container.innerHTML = `
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px;margin-bottom:20px;">
        <div style="background:linear-gradient(135deg,#3b82f6,#2563eb);color:#fff;border-radius:12px;padding:16px;">
          <div style="font-size:28px;font-weight:700;">${todayConsultations}</div>
          <div style="font-size:12px;opacity:.9;">Consultations aujourd'hui</div>
        </div>
        <div style="background:linear-gradient(135deg,#10b981,#059669);color:#fff;border-radius:12px;padding:16px;">
          <div style="font-size:28px;font-weight:700;">${todayModifications}</div>
          <div style="font-size:12px;opacity:.9;">Modifications aujourd'hui</div>
        </div>
        <div style="background:linear-gradient(135deg,#f59e0b,#d97706);color:#fff;border-radius:12px;padding:16px;">
          <div style="font-size:28px;font-weight:700;">${weekConsultations}</div>
          <div style="font-size:12px;opacity:.9;">Consultations cette semaine</div>
        </div>
        <div style="background:linear-gradient(135deg,#ef4444,#dc2626);color:#fff;border-radius:12px;padding:16px;${nonConsultedCount === 0 ? 'opacity:.7;' : ''}">
          <div style="font-size:28px;font-weight:700;">${nonConsultedCount}</div>
          <div style="font-size:12px;opacity:.9;">Alertes non consultées (24h)</div>
        </div>
      </div>

      ${nonConsultedCount > 0 ? `
      <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:12px;padding:16px;margin-bottom:20px;">
        <div style="font-weight:700;color:#991b1b;margin-bottom:10px;font-size:14px;">🚨 Chauffeurs n'ayant pas consulté leur véhicule (alerte urgente)</div>
        <div style="display:flex;flex-wrap:wrap;gap:8px;">
          ${nonConsultedList.map(v => `
...
// ============================================
// DICTÉE VOCALE INTELLIGENTE — Adaptative par chauffeur
// ============================================
let voiceRecognition = null;
let isListening = false;

// Dictionnaire de prononciations connues (persisté dans localStorage)
function getVoiceCorrections() {
  try {
    return JSON.parse(localStorage.getItem('voiceCorrections') || '{}');
  } catch(e) { return {}; }
}

function saveVoiceCorrection(rawTranscript, correctedMatricule) {
  const corrections = getVoiceCorrections();
  const key = rawTranscript.toLowerCase().trim().replace(/\s+/g, ' ');
  corrections[key] = correctedMatricule;
  // Garder max 50 corrections
  const keys = Object.keys(corrections);
  if (keys.length > 50) {
    delete corrections[keys[0]];
  }
  localStorage.setItem('voiceCorrections', JSON.stringify(corrections));
  console.log('📝 Correction vocale enregistrée:', key, '→', correctedMatricule);
}

function findVoiceCorrection(transcript) {
  const corrections = getVoiceCorrections();
  const key = transcript.toLowerCase().trim().replace(/\s+/g, ' ');
  // Recherche exacte d'abord
  if (corrections[key]) return corrections[key];
  // Recherche floue: similarité des mots
  const words = key.split(' ');
  for (const [savedKey, savedMat] of Object.entries(corrections)) {
    const savedWords = savedKey.split(' ');
    let matchCount = 0;
    for (const w of words) {
      if (savedWords.includes(w)) matchCount++;
    }
    if (matchCount >= Math.max(2, words.length * 0.6)) {
      return savedMat;
    }
  }
  return null;
...
// Convertisseur numérique multi-modes
function numbersToDigits(text, mode = 'mixed') {
  let result = text.toUpperCase();

  // Mode 1: Nombres composés (trente-quatre → 34)
  const compoundNumbers = {
    'VINGTETUN': '21', 'VINGTETDEUX': '22', 'VINGTETTROIS': '23',
    'VINGTETQUATRE': '24', 'VINGTETCINQ': '25', 'VINGTETSIX': '26',
    'VINGTETSEPT': '27', 'VINGTETHUIT': '28', 'VINGTETNEUF': '29',
    'TRENTEETUN': '31', 'TRENTEETDEUX': '32', 'TRENTEETTROIS': '33',
    'TRENTEETQUATRE': '34', 'TRENTEETCINQ': '35', 'TRENTEETSIX': '36',
    'TRENTEETSEPT': '37', 'TRENTEETHUIT': '38', 'TRENTEETNEUF': '39',
    'QUARANTEETUN': '41', 'QUARANTEETDEUX': '42', 'QUARANTEETTROIS': '43',
    'QUARANTEETQUATRE': '44', 'QUARANTEETCINQ': '45', 'QUARANTEETSIX': '46',
    'QUARANTEETSEPT': '47', 'QUARANTEETHUIT': '48', 'QUARANTEETNEUF': '49',
    'CINQUANTEETUN': '51', 'CINQUANTEETDEUX': '52', 'CINQUANTEETTROIS': '53',
    'CINQUANTEETQUATRE': '54', 'CINQUANTEETCINQ': '55', 'CINQUANTEETSIX': '56',
    'CINQUANTEETSEPT': '57', 'CINQUANTEETHUIT': '58', 'CINQUANTEETNEUF': '59',
    'SOIXANTEETONZE': '71', 'SOIXANTEETDOUZE': '72', 'SOIXANTEETTREIZE': '73',
    'SOIXANTEQUATORZE': '74', 'SOIXANTEQUINZE': '75', 'SOIXANTESEIZE': '76',
    'SOIXANTEDIXSEPT': '77', 'SOIXANTEDIXHUIT': '78', 'SOIXANTEDIXNEUF': '79',
    'QUATREVINGTUN': '81', 'QUATREVINGTDEUX': '82', 'QUATREVINGTTROIS': '83',
    'QUATREVINGTQUATRE': '84', 'QUATREVINGTCINQ': '85', 'QUATREVINGTSIX': '86',
    'QUATREVINGTSEPT': '87', 'QUATREVINGTHUIT': '88', 'QUATREVINGTNEUF': '89',
    'QUATREVINGTDIX': '90', 'QUATREVINGTONZE': '91', 'QUATREVINGTDOUZE': '92',
    'QUATREVINGTTREIZE': '93', 'QUATREVINGTQUATORZE': '94', 'QUATREVINGTQUINZE': '95',
    'QUATREVINGTSEIZE': '96', 'QUATREVINGTDIXSEPT': '97', 'QUATREVINGTDIXHUIT': '98',
    'QUATREVINGTDIXNEUF': '99'
  };

  for (const [word, num] of Object.entries(compoundNumbers)) {
    result = result.replace(new RegExp(word, 'gi'), num);
  }

  // Mode 2: Dizaines isolées
  const tens = {
    'VINGT': '20', 'TRENTE': '30', 'QUARANTE': '40', 'CINQUANTE': '50',
    'SOIXANTE': '60', 'SOIXANTEDIX': '70', 'QUATREVINGT': '80', 'QUATREVINGTDIX': '90',
    'CENT': '100'
  };
  for (const [word, num] of Object.entries(tens)) {
    result = result.replace(new RegExp('\\b' + word + '\\b', 'gi'), num);
  }

  // Mode 3: Unités isolées
  const units = {
  ...
  for (const [word, num] of Object.entries(units)) {
    result = result.replace(new RegExp('\\b' + word + '\\b', 'gi'), num);
  }

  // Mode 4: Lettres O → 0 (zéro)
  result = result.replace(/\bO\b/g, '0');

  // Mode 5: Anglais (pour chauffeurs qui parlent anglais)
  const english = {
  ...
  for (const [word, num] of Object.entries(english)) {
    result = result.replace(new RegExp('\\b' + word + '\\b', 'gi'), num);
  }

  // Mode 6: Arabe (translittération commune)
  const arabic = {
    'WAHED': '1', 'WAHID': '1', 'ITHNEN': '2', 'ETHNEN': '2', 'THALATHA': '3',
    'TALATA': '3', 'ARBA': '4', 'ARBAA': '4', 'KHAMSA': '5', 'SETTA': '6',
    'SABAA': '7', 'SEBAA': '7', 'THAMANIA': '8', 'TAMANIA': '8', 'TISA': '9',
    'TESAA': '9', 'ASARA': '10', 'SIFR': '0'
  };
  for (const [word, num] of Object.entries(arabic)) {
    result = result.replace(new RegExp('\\b' + word + '\\b', 'gi'), num);
  }
...
// Extraction intelligente du matricule depuis le transcript brut
function extractMatriculeFromSpeech(transcript) {
  console.log('🎤 Raw transcript:', transcript);

  // Étape 1: Vérifier les corrections personnalisées
  const correction = findVoiceCorrection(transcript);
  if (correction) {
    console.log('✅ Correction personnalisée trouvée:', correction);
    return { result: correction, method: 'correction', raw: transcript };
  }

  // Étape 2: Nettoyage de base
  let cleaned = transcript.toUpperCase()
    .replace(/[,.;:!?]/g, ' ')      // ponctuation → espace
    .replace(/\s+/g, ' ')           // espaces multiples → simple
    .trim();

  // Étape 3: Conversion numérique
  cleaned = numbersToDigits(cleaned);
  console.log('🎤 Après conversion numérique:', cleaned);

  // Étape 4: Extraction des chiffres et tirets
  // Garder uniquement chiffres, lettres (A-Z), tirets
  let digitsOnly = cleaned.replace(/[^0-9A-Z\-]/g, '');
  console.log('🎤 Chiffres extraits:', digitsOnly);

  // Étape 5: Formatage matricule tunisien
  // Patterns valides: 17-345137, 17345137, 17 345 137, etc.
  let matricule = digitsOnly;

  // Si pas de tiret mais 7-8 chiffres, insérer tiret après les 2 premiers
  if (!matricule.includes('-') && /^\d{7,8}$/.test(matricule)) {
    matricule = matricule.slice(0, 2) + '-' + matricule.slice(2);
  }

  // Si format avec espaces: 17 345 137 → 17-345137
  matricule = matricule.replace(/\s/g, '');

  // Vérifier si c'est un matricule valide (format tunisien)
  const isValid = /^\d{2}-\d{6}$/.test(matricule) || /^\d{2}-\d{5}$/.test(matricule);

  return { 
    result: matricule, 
    method: isValid ? 'auto' : 'partial',
    raw: transcript,
    isValid: isValid
  };
}
...
function startVoiceInput(targetId, buttonId) {
  if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
    alert("❌ La reconnaissance vocale n'est pas supportée sur ce navigateur. Essayez Chrome ou Edge.");
    return;
  }
  ...
  if (voiceRecognition) {
    ...
    isListening = false;
    updateVoiceButton(buttonId, false);
    return;
  }

  voiceRecognition = new SpeechRecognition();
  // 🔧 Détection auto de la langue (français par défaut, mais accepte l'arabe et l'anglais)
  voiceRecognition.lang = 'fr-FR';
  voiceRecognition.continuous = false;
  voiceRecognition.interimResults = true;  // 🔧 Résultats intermédiaires pour feedback
  voiceRecognition.maxAlternatives = 3;    // 🔧 3 alternatives pour meilleure reconnaissance

  let interimTranscript = '';

  voiceRecognition.onstart = () => {
    isListening = true;
    updateVoiceButton(buttonId, true);
    const target = document.getElementById(targetId);
    if (target) {
      target.placeholder = '🎤 Écoute en cours...';
  ...
  voiceRecognition.onresult = (event) => {
    ...
    // Feedback temps réel
    const target = document.getElementById(targetId);
    if (target && interimTranscript) {
      target.value = '... ' + interimTranscript;
      target.style.color = '#94a3b8';
    }

    if (finalTranscript) {
      console.log('🎤 Final transcript:', finalTranscript);
      const extraction = extractMatriculeFromSpeech(finalTranscript);

      if (target) {
        target.value = extraction.result;
        target.style.color = extraction.isValid ? '#1e293b' : '#ef4444';
        target.placeholder = '';
        target.style.borderColor = extraction.isValid ? '#10b981' : '#f59e0b';
        target.focus();
      }

      // Feedback sonore
      if (window.speechSynthesis) {
        const msg = extraction.isValid 
          ? `Matricule ${extraction.result.replace('-', ' tiret ')} confirmé`
          : `J'ai entendu ${extraction.result}. Veuillez vérifier.`;
        const confirm = new SpeechSynthesisUtterance(msg);
        confirm.lang = 'fr-FR';
        confirm.rate = 1.1;
        window.speechSynthesis.speak(confirm);
      }

      // 🔧 Si non valide, proposer la correction
      if (!extraction.isValid) {
        showVoiceCorrectionModal(extraction);
      }
    }
  };
  ...
  voiceRecognition.onerror = (event) => {
    ...
    const target = document.getElementById(targetId);
    if (target) {
      target.placeholder = 'Entrez le matricule';
      target.style.borderColor = '#e2e8f0';
      target.style.color = '#1e293b';
    }
    if (event.error === 'not-allowed') {
      alert("❌ Permission micro refusée. Autorisez l'accès au micro dans les paramètres du navigateur.");
    }
  };
...
// 🔧 Modal de correction pour apprendre la prononciation du chauffeur
function showVoiceCorrectionModal(extraction) {
  // Supprimer modal existant
  document.getElementById('voice-correction-modal')?.remove();

  const overlay = document.createElement('div');
  overlay.id = 'voice-correction-modal';
  overlay.style.cssText = 'position:fixed;inset:0;z-index:99999;background:rgba(15,23,42,0.75);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:20px;';

  overlay.innerHTML = `
    <div style="background:#fff;border-radius:20px;padding:28px 24px;max-width:400px;width:100%;text-align:center;box-shadow:0 24px 64px rgba(0,0,0,0.25);">
      <div style="font-size:44px;margin-bottom:12px;">🎤</div>
      <h2 style="font-size:18px;font-weight:700;color:#1e293b;margin-bottom:6px;">Vérification du matricule</h2>

      <div style="background:#f1f5f9;border-radius:12px;padding:14px;margin:16px 0;text-align:left;">
        <div style="font-size:11px;color:#64748b;text-transform:uppercase;letter-spacing:0.5px;margin-bottom:4px;">Ce que j'ai entendu</div>
        <div style="font-size:14px;color:#1e293b;font-style:italic;">"${extraction.raw}"</div>
      </div>

      <div style="margin-bottom:16px;">
        <label style="display:block;font-size:12px;color:#64748b;margin-bottom:6px;text-align:left;">Matricule correct :</label>
        <input type="text" id="voice-correction-input" value="${extraction.result}" 
          style="width:100%;padding:12px 14px;border:2px solid #e2e8f0;border-radius:12px;font-size:18px;font-weight:700;color:#1e293b;outline:none;text-align:center;"
          placeholder="17-345137">
      </div>

      <button id="voice-save-btn" data-raw="${extraction.raw.replace(/"/g, '&quot;')}" data-result="${extraction.result.replace(/"/g, '&quot;')}"
        style="width:100%;padding:13px;border:none;border-radius:12px;background:linear-gradient(135deg,#10b981,#059669);color:#fff;font-size:15px;font-weight:700;cursor:pointer;box-shadow:0 4px 16px rgba(16,185,129,0.3);margin-bottom:10px;">
        ✅ Enregistrer ma prononciation
      </button>
      <button onclick="document.getElementById('voice-correction-modal')?.remove();"
        style="width:100%;padding:12px;border:1.5px solid #e2e8f0;border-radius:12px;background:#fff;color:#64748b;font-size:14px;font-weight:600;cursor:pointer;">
        ❌ Ignorer
      </button>

      <p style="font-size:11px;color:#94a3b8;margin-top:12px;">
        💡 La prochaine fois que vous direz "${extraction.raw}", le système reconnaîtra automatiquement ce matricule.
      </p>
    </div>`;

  document.body.appendChild(overlay);

  // Attach click handler safely (avoiding inline onclick issues)
  setTimeout(() => {
    const input = document.getElementById('voice-correction-input');
    input?.focus();
    input?.select();
    ...
    if (saveBtn) {
      saveBtn.addEventListener('click', () => {
        const raw = saveBtn.dataset.raw;
        saveVoiceCorrectionFromModalData(raw);
      });
    }
  }, 100);
}

function saveVoiceCorrectionFromModalData(rawTranscript) {
  const input = document.getElementById('voice-correction-input');
  const corrected = input?.value?.trim().toUpperCase();
  if (!corrected) return;

  saveVoiceCorrection(rawTranscript, corrected);

  // Mettre à jour le champ principal
  const mainInput = document.getElementById('user-matricule');
  if (mainInput) {
    mainInput.value = corrected;
    mainInput.style.color = '#1e293b';
  ...
  if (window.parcAuto) {
    parcAuto.showToast('✅ Prononciation enregistrée pour la prochaine fois', 'success');
  }
}

function updateVoiceButton(buttonId, listening) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 🔧 FIX: exposer l'instance sur window — de nombreuses fonctions (enregistrement
  // index/visite, alertes, export…) sont gardées par `if (!window.parcAuto) return;`.
  // Sans cette ligne, `window.parcAuto` reste undefined et ces actions ne font rien.
  window.parcAuto = parcAuto;
});

// Injecter le style CSS pour l'animation micro
...
function exportExcel() {
  if (parcAuto) parcAuto.exportExcel();
}

function checkDoublons() {
  if (parcAuto) parcAuto.checkDoublons();
}
...
_Ce message est généré automatiquement._`;
  ...
  // MÉTHODE 2 (désactivée) : on n'ouvre plus WhatsApp automatiquement sans clic.
  // Avant : window.open(waUrl, '_blank') ici ouvrait un onglet WhatsApp pour
  // chaque véhicule en alerte, à chaque connexion d'un utilisateur — bloqué à la demande.
  console.log(`ℹ️ Alerte WhatsApp préparée pour ${vehicle.matricule} (${recipient}) — non ouverte automatiquement (notifications non autorisées).`);
}
...
// ============================================
// ============================================
// EXPORT CSV HISTORIQUE VÉHICULE (Chauffeur)
// ============================================
function exportVehicleHistoryCSV(matricule) {
  const raw = localStorage.getItem('parcAutoData_v3');
  if (!raw) return;
  const data = JSON.parse(raw);
  ...
  const headers = ['Date', 'Type', 'Désignation', 'Kilométrage', 'Montant (TND)', 'Chauffeur'];
  const rows = repairs.map(r => [
    r.date,
    r.type,
    r.designation,
    r.km || 0,
    r.montant ? r.montant.toFixed(2) : '0.00',
    r.chauffeur || vehicle.chauffeur || ''
  ]);
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = 'historique-' + matricule + '-' + new Date().toISOString().split('T')[0] + '.csv';
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
  ...
  if (window.parcAuto) {
    parcAuto.showToast('✅ Historique exporté en CSV', 'success');
  }
}
...
// ============================================
// MODAL KILOMÉTRAGE (Interface Utilisateur)
// ============================================
function openUserKmModal(matricule, currentKm) {
  stopAllTTS();
  document.getElementById('user-km-modal')?.remove();

  const overlay = document.createElement('div');
  overlay.id = 'user-km-modal';
  overlay.style.cssText = 'position:fixed;inset:0;z-index:99999;background:rgba(15,23,42,0.75);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:20px;';

  const box = document.createElement('div');
  box.style.cssText = 'background:#fff;border-radius:20px;padding:28px 24px;max-width:360px;width:100%;text-align:center;box-shadow:0 24px 64px rgba(0,0,0,0.25);';

  const styleEl = document.createElement('style');
  styleEl.textContent = '@keyframes modalFadeIn{from{opacity:0;transform:scale(.95)}to{opacity:1;transform:scale(1)}}';
  document.head.appendChild(styleEl);

  box.insertAdjacentHTML('beforeend', `
    <div style="font-size:44px;margin-bottom:12px;">🚗</div>
    <h2 style="font-size:18px;font-weight:700;color:#1e293b;margin-bottom:6px;">Modifier le kilométrage</h2>
    <p style="font-size:13px;color:#64748b;margin-bottom:20px;">Véhicule : <strong>${matricule}</strong></p>
    <input type="number" id="user-km-input" value="${currentKm || 0}" min="0" max="999999"
      style="width:100%;padding:12px 14px;border:2px solid #e2e8f0;border-radius:12px;font-size:18px;font-weight:700;color:#1e293b;outline:none;text-align:center;margin-bottom:8px;"
      onfocus="this.style.borderColor='#10b981'" onblur="this.style.borderColor='#e2e8f0'">
    <div id="km-modal-error" style="color:#ef4444;font-size:12px;min-height:18px;margin-bottom:12px;text-align:left;"></div>
  `);

  const btnSave = document.createElement('button');
  btnSave.type = 'button';
  btnSave.textContent = '💾 Enregistrer';
  btnSave.style.cssText = 'width:100%;padding:13px;border:none;border-radius:12px;background:linear-gradient(135deg,#10b981,#059669);color:#fff;font-size:15px;font-weight:700;cursor:pointer;box-shadow:0 4px 16px rgba(16,185,129,0.3);margin-bottom:10px;display:block;min-height:50px;';
  btnSave.addEventListener('click', function(e) { e.preventDefault(); saveUserKm(matricule); });
  btnSave.addEventListener('touchend', function(e) { e.preventDefault(); saveUserKm(matricule); });

  const btnCancel = document.createElement('button');
  btnCancel.type = 'button';
  btnCancel.textContent = '❌ Annuler';
  btnCancel.style.cssText = 'width:100%;padding:12px;border:1.5px solid #e2e8f0;border-radius:12px;background:#fff;color:#64748b;font-size:14px;font-weight:600;cursor:pointer;display:block;min-height:46px;';
  btnCancel.addEventListener('click', function(e) { e.preventDefault(); document.getElementById('user-km-modal')?.remove(); });
  btnCancel.addEventListener('touchend', function(e) { e.preventDefault(); document.getElementById('user-km-modal')?.remove(); });
  ...
  setTimeout(() => {
    const input = document.getElementById('user-km-input');
    input?.focus();
    input?.select();
  }, 100);
}

async function saveUserKm(matricule) {
  // Guard contre double-clic / touchend+click sur iOS
  if (saveUserKm._running) return;
  saveUserKm._running = true;
  setTimeout(() => { saveUserKm._running = false; }, 1000);

  const input = document.getElementById('user-km-input');
  const errorEl = document.getElementById('km-modal-error');
  // Lire la valeur AVANT de fermer le modal
  const newKm = parseInt(input?.value) || 0;

  if (newKm < 0 || newKm > 999999) {
    if (errorEl) errorEl.textContent = 'Kilométrage invalide (0 - 999 999 km).';
    saveUserKm._running = false;
    return;
  }

  // Fermer le modal APRÈS avoir lu la valeur
  document.getElementById('user-km-modal')?.remove();

  if (!window.parcAuto) return;

  // 🔧 Récupérer les données fraîches depuis localStorage si nécessaire
  if (!parcAuto.data.vehicles || parcAuto.data.vehicles.length === 0) {
    try {
      const saved = localStorage.getItem('parcAutoData_v3');
      if (saved) {
        const fresh = JSON.parse(saved);
        const _uv = parcAuto.data.userVehicle;
        const _cu = parcAuto.data.currentUser;
        parcAuto.data = fresh;
        parcAuto.data.userVehicle = _uv || fresh.userVehicle;
        parcAuto.data.currentUser = _cu || fresh.currentUser;
      }
    } catch(e) {}
  }

  const vehicle = parcAuto.data.vehicles?.find(v => v.matricule === matricule);
  if (!vehicle) {
    parcAuto.showToast('❌ Véhicule introuvable, rechargez la page', 'error');
    return;
  }

  const oldKm = vehicle.km;
  if (newKm === oldKm) {
    parcAuto.showToast('ℹ️ Kilométrage inchangé', 'info');
    return;
  }

  vehicle.km = newKm;

  parcAuto.logActivity(matricule, 'modification_km', {
    chauffeur: parcAuto.data.currentUser?.name,
    oldValue: oldKm.toLocaleString() + ' km',
    newValue: newKm.toLocaleString() + ' km',
    note: `KM modifié: ${oldKm.toLocaleString()} → ${newKm.toLocaleString()}`
  });

  // 🔧 FIX: Passer l'id du véhicule pour le _lastModified (merge partiel)
  await parcAuto.saveData(vehicle.id);

  parcAuto.renderUserVehicleDetail();
  parcAuto.showToast('✅ Kilométrage mis à jour', 'success');
}

// ============================================
// MODAL DATE VISITE TECHNIQUE (Interface Utilisateur)
// ============================================
function openUserVisiteModal(matricule, currentDate) {
  stopAllTTS();
  document.getElementById('user-visite-modal')?.remove();

  const overlay = document.createElement('div');
  overlay.id = 'user-visite-modal';
  overlay.style.cssText = 'position:fixed;inset:0;z-index:99999;background:rgba(15,23,42,0.75);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:20px;';
  ...
  box.insertAdjacentHTML('beforeend', `
    <div style="font-size:44px;margin-bottom:12px;">📋</div>
    <h2 style="font-size:18px;font-weight:700;color:#1e293b;margin-bottom:6px;">Modifier la date de visite</h2>
    <p style="font-size:13px;color:#64748b;margin-bottom:20px;">Véhicule : <strong>${matricule}</strong></p>
    <input type="date" id="user-visite-input" value="${currentDate || ''}"
      style="width:100%;padding:12px 14px;border:2px solid #e2e8f0;border-radius:12px;font-size:15px;color:#1e293b;outline:none;margin-bottom:8px;"
      onfocus="this.style.borderColor='#3b82f6'" onblur="this.style.borderColor='#e2e8f0'">
    <div id="visite-modal-error" style="color:#ef4444;font-size:12px;min-height:18px;margin-bottom:12px;text-align:left;"></div>
  `);
  ...
  btnSave.textContent = '💾 Enregistrer';
  btnSave.style.cssText = 'width:100%;padding:13px;border:none;border-radius:12px;background:linear-gradient(135deg,#10b981,#059669);color:#fff;font-size:15px;font-weight:700;cursor:pointer;box-shadow:0 4px 16px rgba(16,185,129,0.3);margin-bottom:10px;display:block;min-height:50px;';
  btnSave.addEventListener('click', function(e) { e.preventDefault(); saveUserVisite(matricule); });
  btnSave.addEventListener('touchend', function(e) { e.preventDefault(); saveUserVisite(matricule); });

  const btnCancel = document.createElement('button');
  btnCancel.type = 'button';
  btnCancel.textContent = '❌ Annuler';
  btnCancel.style.cssText = 'width:100%;padding:12px;border:1.5px solid #e2e8f0;border-radius:12px;background:#fff;color:#64748b;font-size:14px;font-weight:600;cursor:pointer;display:block;min-height:46px;';
  btnCancel.addEventListener('click', function(e) { e.preventDefault(); document.getElementById('user-visite-modal')?.remove(); });
  btnCancel.addEventListener('touchend', function(e) { e.preventDefault(); document.getElementById('user-visite-modal')?.remove(); });
  ...
  document.body.appendChild(overlay);
  setTimeout(() => document.getElementById('user-visite-input')?.focus(), 100);
}

async function saveUserVisite(matricule) {
  // Guard contre double-clic / touchend+click sur iOS
  if (saveUserVisite._running) return;
  saveUserVisite._running = true;
  setTimeout(() => { saveUserVisite._running = false; }, 1000);

  const input = document.getElementById('user-visite-input');
  const errorEl = document.getElementById('visite-modal-error');
  // Lire la valeur AVANT de fermer le modal
  const newDate = input?.value?.trim() || '';

  if (!newDate) {
    if (errorEl) errorEl.textContent = 'Veuillez sélectionner une date.';
    saveUserVisite._running = false;
    return;
  }
  ...
  const oldDate = vehicle.prochaineVisite || 'Non définie';
  vehicle.prochaineVisite = newDate;

  parcAuto.logActivity(matricule, 'modification_visite', {
    chauffeur: parcAuto.data.currentUser?.name,
    oldValue: oldDate,
    newValue: newDate,
    note: `Visite technique modifiée: ${oldDate} → ${newDate}`
  });
  ...
  parcAuto.renderUserVehicleDetail();
  parcAuto.showToast('✅ Date de visite technique mise à jour', 'success');
}

function filterLogs() {
  if (parcAuto) parcAuto.renderActivityLogsTable();
}

function resetLogFilters() {
  ['log-filter-matricule','log-filter-action','log-filter-chauffeur',
   'log-filter-date-from','log-filter-date-to'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
  const pageEl = document.getElementById('logs-current-page');
  if (pageEl) pageEl.dataset.page = '1';
  if (parcAuto) parcAuto.renderActivityLogsTable();
}

function changeLogsPage(direction) {
  const pageEl = document.getElementById('logs-current-page');
  let current = parseInt(pageEl?.dataset?.page || '1');
  current += direction;
  if (current < 1) current = 1;
  if (pageEl) pageEl.dataset.page = current.toString();
  if (parcAuto) parcAuto.renderActivityLogsTable();
}

// ============================================
// EFFACEMENT HISTORIQUE ACTIVITÉ
// ============================================
function confirmClearActivityLogs() {
  if (!window.parcAuto) return;
  const count = (parcAuto.data.activityLogs || []).length;
  if (count === 0) {
    parcAuto.showToast('ℹ️ L\'historique est déjà vide', 'info');
    return;
  }
  // Boîte de confirmation native avec le nombre d'entrées
  const confirmed = window.confirm(
    `⚠️ Effacer l'historique d'activité des chauffeurs ?\n\n` +
    `${count} entrée(s) seront supprimées définitivement.\n\n` +
    `Cette action est irréversible.`
  );
  if (confirmed) clearActivityLogs();
}

async function clearActivityLogs() {
  if (!window.parcAuto) return;
  try {
    parcAuto.data.activityLogs = [];
    await parcAuto.saveData();
    parcAuto.renderActivityLogsTable();
    parcAuto.showToast('🗑️ Historique effacé avec succès', 'success');
    // Mettre à jour le résumé d'activité
    if (typeof parcAuto.renderActivitySummary === 'function') {
      parcAuto.renderActivitySummary();
    }
  } catch(err) {
    console.error('Erreur effacement historique:', err);
    parcAuto.showToast('❌ Erreur lors de l\'effacement', 'error');
  }
}

// Export CSV de l'historique d'activité
function exportActivityLogs() {
  if (!window.parcAuto) return;
  const logs = parcAuto.data.activityLogs || [];
  if (logs.length === 0) {
    parcAuto.showToast('ℹ️ Aucune activité à exporter', 'info');
    return;
  }
  const headers = ['Date & Heure', 'Action', 'Matricule', 'Chauffeur', 'Division', 'Détails'];
  ...
  const a = document.createElement('a');
  a.href = url;
  a.download = `activite-chauffeurs-${new Date().toISOString().split('T')[0]}.csv`;
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
  parcAuto.showToast(`📥 Export réussi — ${logs.length} entrée(s)`, 'success');
}
...
function exportCSV() {
  ...
  const headers = ['Matricule','Modèle','Chauffeur','KM Actuel','Prochaine Vidange','Prochaine Chaîne','Prochaine Visite','Date Batterie','Indice Batterie','Date Pneus','Statut','Constructeur','Type Commercial','N° Série (Châssis)','1ère Mise en Circulation','Genre','Énergie','Ancienne Vidange (km)','Date Ancienne Vidange'];
  const rows = vehicles.map(v => [
    ...
    v.indiceBatterie || '', v.dateChangementPneus || '', v.statut || 'actif',
    v.constructeur || '', v.typeCommercial || '', v.numSerie || '',
    v.dateCirculation || '', v.genre || '', v.energie || '',
    v.ancienneVidangeIndex || '', v.ancienneVidangeDate || ''
  ]);
...
// ============================================
// OTP ADMIN — fonctions globales
// ============================================
function closeOtpModal() {
  if (parcAuto) parcAuto.closeOtpModal();
}

function verifyAdminOtp() {
  if (parcAuto) parcAuto.verifyAdminOtp();
}

function resendAdminOtp() {
  if (parcAuto) parcAuto.resendAdminOtp();
}
