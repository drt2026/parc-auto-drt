...
class ParcAutoApp {
  ...
  initAdminLogin() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      const isOtpVisible = otpStep && otpStep.style.display !== 'none';

      if (isOtpVisible) {
        // Étape 2 : vérifier le code OTP
        const inputOtp = document.getElementById('admin-otp')?.value.trim();
        ...
        if (result.ok) {
          ...
          await this.saveData();
          this.showToast('✅ Connexion sécurisée réussie', 'success');
      ...
      if (user) {
        ...
        this.showToast('📱 Code OTP envoyé (démo: ' + otp + ')', 'info');

        // Afficher l'étape OTP
        if (otpStep) {
          otpStep.style.display = 'block';
          document.getElementById('admin-email').closest('.form-group').style.display = 'none';
          document.getElementById('admin-password').closest('.form-group').style.display = 'none';
          document.getElementById('otp-info-email').textContent = email;
      ...
      if (vehicle) {
        ...
        function formatMatriculeVoix(mat) {
          ...
          if (parts.length === 2) {
            ...
            const nums = parts[1];
            // Groupe par 3 chiffres
            const grouped = nums.match(/.{1,3}/g).join(' ');
        ...
        setTimeout(() => {
          ...
          if (typeof showTips === 'function') {
            showTips(() => {
              ...
              this.renderUserVehicleDetail();
              this.startAutoSync();
              // Message de bienvenue vocal après les conseils
              speakWelcome(() => {});
            });
          } else {
            document.getElementById('user-interface').style.display = 'block';
