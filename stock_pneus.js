...
(function () {
  ...
  function injectTab() {
    ...
    tab.innerHTML = `
      <div class="card">
        ...
        <div class="pn-view" id="pn-view-vehicules">
          ...
          <!-- BLOC ADDITIF — Historique complet d'un véhicule (clic sur son matricule ci-dessus) -->
          <div id="pn-veh-detail" style="display:none;margin-top:22px;background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:14px;padding:18px 20px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">
              <div>
                <h3 id="pn-veh-detail-title" style="margin:0;color:#1e3a5f;">🚗 Historique — </h3>
                <div id="pn-veh-detail-sub" style="font-size:12.5px;color:#64748b;margin-top:2px;"></div>
              </div>
              <button class="btn btn-secondary" id="pn-veh-detail-close" style="padding:6px 14px;">✕ Fermer</button>
            </div>
            <div style="overflow-x:auto;margin-top:12px;">
              <table class="data-table">
                <thead><tr>
                  <th>Date</th><th>Référence</th><th>Position</th><th>Km à la pose</th>
                  <th>Travaux signalés</th><th>Résidence</th><th>Observations</th>
                </tr></thead>
                <tbody id="pn-veh-detail-tbody"></tbody>
              </table>
            </div>
          </div>
          <!-- FIN BLOC ADDITIF -->
          ...
          <div style="overflow-x:auto;">
            <table class="data-table">
              <thead><tr>
                ...
                <th>Position</th><th>Travaux signalés</th><th>Résidence</th><th>Fournisseur</th><th>P.U. (DT)</th><th>Observations</th>
              </tr></thead>
      ...
      <div class="pn-modal-overlay" id="pn-modal-sortie">
        <div class="pn-modal">
          ...
          <!-- BLOC ADDITIF — sélection libre des positions (1 à 5 roues), au lieu du choix
               figé "1 position" ou "4 positions" : couvre les demandes de type "changement
               deux roues" comme sur le formulaire papier "Demande de Travaux" -->
          <div class="pn-field"><label>Positions concernées * <span style="font-weight:400;color:#64748b;">(cochez-en une ou plusieurs)</span></label>
            <div id="pn-sortie-pos-group" style="display:flex;flex-direction:column;gap:6px;padding:10px 12px;border:1.5px solid #e2e8f0;border-radius:9px;"></div>
          </div>
          <!-- FIN BLOC ADDITIF -->
          <!-- BLOC ADDITIF — champs alignés sur le formulaire papier "Demande de Travaux" -->
          <div class="pn-field"><label>Travaux signalés <span style="font-weight:400;color:#64748b;">(ex: Changement deux roues)</span></label>
            <input type="text" id="pn-sortie-motif" placeholder="Ex: Changement deux roues"></div>
          <div class="pn-field"><label>Résidence / Site</label>
            <input type="text" id="pn-sortie-residence" placeholder="Ex: CSC Sfax Nord"></div>
          <!-- FIN BLOC ADDITIF -->
    ...
    // BLOC ADDITIF — cases à cocher pour les positions (remplace l'ancien menu déroulant)
    POSITIONS.forEach(function (p) {
      var wrap = document.createElement('label');
      wrap.style.cssText = 'font-weight:400;font-size:13.5px;display:flex;align-items:center;gap:8px;cursor:pointer;';
      var cb = document.createElement('input');
      cb.type = 'checkbox'; cb.value = p; cb.className = 'pn-sortie-pos-cb'; cb.style.width = 'auto';
      wrap.appendChild(cb);
      wrap.appendChild(document.createTextNode(p));
      document.getElementById('pn-sortie-pos-group').appendChild(wrap);
    });
    // FIN BLOC ADDITIF
  ...
  function wireEvents() {
    ...
    // BLOC ADDITIF — fermer le panneau d'historique par véhicule
    document.getElementById('pn-veh-detail-close').addEventListener('click', function () {
      document.getElementById('pn-veh-detail').style.display = 'none';
    });
    // FIN BLOC ADDITIF
    ...
    document.getElementById('pn-btn-sortie').addEventListener('click', function () {
      ...
      // BLOC ADDITIF — décoche toutes les positions à l'ouverture + reset des nouveaux champs
      document.querySelectorAll('.pn-sortie-pos-cb').forEach(function (cb) { cb.checked = false; });
      document.getElementById('pn-sortie-motif').value = '';
      document.getElementById('pn-sortie-residence').value = '';
      // FIN BLOC ADDITIF
    ...
    document.getElementById('pn-sortie-save').addEventListener('click', function () {
      ...
      // BLOC ADDITIF — positions cochées (1 à 5), motif et résidence
      var positions = Array.prototype.slice.call(document.querySelectorAll('.pn-sortie-pos-cb:checked')).map(function (cb) { return cb.value; });
      var motif = document.getElementById('pn-sortie-motif').value.trim();
      var residence = document.getElementById('pn-sortie-residence').value.trim();
      // FIN BLOC ADDITIF
      ...
      if (!date) { err.textContent = 'La date est obligatoire.'; err.classList.add('show'); return; }
      if (!positions.length) { err.textContent = 'Cochez au moins une position.'; err.classList.add('show'); return; }

      var qteNeeded = positions.length;
      ...
      positions.forEach(function (pos) {
        data.stockPneus.mouvements.push({
          ...
          motif: motif, residence: residence,
      ...
      document.getElementById('pn-modal-sortie').classList.remove('active');
      renderAll();
      toast(positions.length + (positions.length > 1 ? ' pneus montés' : ' pneu monté') + ' sur ' + mat, 'success');
    });
  }
  ...
  function renderVehiculeView(search) {
    ...
    tbody.innerHTML = rows.map(function (m) {
      ...
      return '<tr>' +
        '<td style="font-weight:600"><a href="#" class="pn-veh-link" data-mat="' + esc(m.matricule) + '" style="color:#1d4ed8;text-decoration:underline;">' + esc(m.matricule) + '</a></td>' +
    ...
    // BLOC ADDITIF — clic sur un matricule = ouvre son historique complet ci-dessous
    tbody.querySelectorAll('.pn-veh-link').forEach(function (a) {
      a.addEventListener('click', function (ev) {
        ev.preventDefault();
        showVehiculeDetail(this.getAttribute('data-mat'));
      });
    });
  }

  function showVehiculeDetail(mat) {
    var data = getData(); if (!data) return;
    var panel = document.getElementById('pn-veh-detail');
    var veh = getVehicleByMat(mat);

    document.getElementById('pn-veh-detail-title').textContent = '🚗 Historique — ' + mat;
    document.getElementById('pn-veh-detail-sub').textContent = veh && veh.modele ? veh.modele : '';

    var rows = data.stockPneus.mouvements
      .filter(function (m) { return m.type === 'SORTIE' && m.matricule === mat; })
      .sort(function (a, b) { return (b.date || '').localeCompare(a.date || ''); });

    var tbody = document.getElementById('pn-veh-detail-tbody');
    if (!rows.length) {
      tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:#94a3b8;padding:20px;">Aucun mouvement enregistré pour ce véhicule.</td></tr>';
    } else {
      tbody.innerHTML = rows.map(function (m) {
        var ref = getRef(m.referenceId);
        return '<tr>' +
      ...
      document.getElementById('pn-veh-detail-sub').textContent =
        (veh && veh.modele ? veh.modele + ' — ' : '') + rows.length + ' pneu(s) monté(s) au total' +
        (rows[0] ? ' — dernier le ' + fmtDateFr(rows[0].date) : '');
    }
  ...
  function renderHistorique(search) {
    ...
    if (q) {
      rows = rows.filter(function (m) {
        var ref = getRef(m.referenceId);
        var hay = [ref ? ref.dimension : '', m.matricule, m.fournisseur, m.observations, m.motif, m.residence].join(' ').toLowerCase();
        return hay.indexOf(q) !== -1;
      });
    }
    if (!rows.length) {
      tbody.innerHTML = '<tr><td colspan="11" style="text-align:center;color:#94a3b8;padding:24px;">Aucun mouvement enregistré.</td></tr>';
      return;
    }
    tbody.innerHTML = rows.map(function (m) {
      var ref = getRef(m.referenceId);
  ...
  function exportPneusExcel() {
    ...
    function build() {
      ...
      ws2.columns = [
        ...
        { width: 10 }, { width: 12 }, { width: 12 }, { width: 18 }, { width: 22 }, { width: 16 }, { width: 26 }
      ];
      var h2 = ws2.addRow(['Référence', 'Date', 'Matricule', 'Position', 'Entrée', 'Sortie', 'Reste', 'Quantité', 'P.U. (DT)', 'Total (DT)', 'Fournisseur', 'Travaux signalés', 'Résidence', 'Observations']);
      styleHeaderRow(h2);
      ...
      refsTriees.forEach(function (ref) {
        ...
        mvs.forEach(function (m) {
          ...
          var row = ws2.addRow([
            ...
            m.fournisseur || '', m.motif || '', m.residence || '', m.observations || ''
          ]);
          styleDataRow(row);
        });
      ...
      ws2b.columns = [{ width: 12 }, { width: 10 }, { width: 22 }, { width: 10 }, { width: 14 }, { width: 16 }, { width: 22 }, { width: 16 }, { width: 18 }, { width: 12 }, { width: 26 }];
      var h2b = ws2b.addRow(['Date', 'Type', 'Référence', 'Quantité', 'Matricule', 'Position', 'Travaux signalés', 'Résidence', 'Fournisseur', 'P.U. (DT)', 'Observations']);
      styleHeaderRow(h2b);
      data.stockPneus.mouvements.slice().sort(function (a, b) { return (a.date || '').localeCompare(b.date || ''); }).forEach(function (m) {
        ...
        var row = ws2b.addRow([
          ...
          m.matricule || '', m.position || '', m.motif || '', m.residence || '', m.fournisseur || '', m.prixUnitaire || '', m.observations || ''
        ]);
        styleDataRow(row);
      });
  ...
  function importPneusExcel(file) {
    ...
    function run() {
      ...
      reader.onload = function () {
        try {
          // NOTE : on utilise SheetJS (window.XLSX) et non ExcelJS pour LIRE le fichier
          // d'import. Certains fichiers historiques (notamment ceux contenant des photos
          // insérées dans les cellules) sont enregistrés avec un XML de dessin
          // (xl/drawings/drawingN.xml) qui omet le préfixe de namespace "xdr:" attendu par
          // ExcelJS ("wsDr" au lieu de "xdr:wsDr"). ExcelJS échoue alors silencieusement à
          // parser ce dessin et plante ensuite avec "Cannot read properties of undefined
          // (reading 'anchors')". SheetJS ignore complètement les dessins/images lorsqu'il
          // lit les valeurs de cellules, donc il n'est pas affecté par ce problème.
          var swb = window.XLSX.read(reader.result, { type: 'array', cellDates: true });

          // BLOC ADDITIF — détection du format du fichier :
          //  • FORMAT 1 (historique) : une seule feuille, groupée par référence,
          //    colonne A = "Référence (P)".
          //  • FORMAT 2 (une feuille par référence) : chaque feuille porte le nom
          //    d'une référence (ex: "195-14") et contient Date d'entrée / Date de
          //    sortie / Matricule véhicule.
          var sws = swb.Sheets[swb.SheetNames[0]];
          var aoa = window.XLSX.utils.sheet_to_json(sws, { header: 1, raw: true, defval: null });
          var rowCount = aoa.length;
          function cellAt(rowIdx1, colIdx1) { // indices 1-based comme dans l'ancien code ExcelJS
            var row = aoa[rowIdx1 - 1];
            return row ? row[colIdx1 - 1] : null;
          }
          ...
          var isMultiSheetFormat = false;
          if (!headerRow) {
            // pas le FORMAT 1 sur la 1ère feuille → on vérifie si c'est le FORMAT 2
            var anySheetMatches = swb.SheetNames.some(function (name) {
              var a = window.XLSX.utils.sheet_to_json(swb.Sheets[name], { header: 1, raw: true, defval: null });
              return a.some(function (row) { return row && String(row[0] || '').trim().toLowerCase().indexOf("date d'entrée") === 0; });
            });
            if (anySheetMatches) { isMultiSheetFormat = true; }
            else { toast("Fichier non reconnu : ni en-tête 'Référence (P)' ni 'Date d'entrée' trouvé.", 'error'); return; }
          }
          // FIN BLOC ADDITIF
          ...
          // BLOC ADDITIF — FORMAT 2 : une feuille par référence
          if (isMultiSheetFormat) {
            swb.SheetNames.forEach(function (sheetName) {
              var sAoa = window.XLSX.utils.sheet_to_json(swb.Sheets[sheetName], { header: 1, raw: true, defval: null });
              var hRow = null;
              for (var rr = 1; rr <= sAoa.length; rr++) {
                var hc = sAoa[rr - 1] ? sAoa[rr - 1][0] : null;
                if (String(hc || '').trim().toLowerCase().indexOf("date d'entrée") === 0) { hRow = rr; break; }
              }
              if (!hRow) return; // feuille sans cet en-tête (ex: page d'accueil) : ignorée silencieusement

              var ref = findOrCreateRef(sheetName);
              refsCreated++;
              for (var rr2 = hRow + 1; rr2 <= sAoa.length; rr2++) {
                var row = sAoa[rr2 - 1]; if (!row) continue;
                var dEntree = row[0], dSortie = row[1], matRaw = row[2];
                var dateEntreeISO = parseImportDate(dEntree);
                var dateSortieISO = parseImportDate(dSortie);
                if (!dateEntreeISO && !dateSortieISO) continue;
                if (dEntree != null && !dateEntreeISO) skipped.push('Feuille "' + sheetName + '", ligne ' + rr2 + ' : date d\'entrée non reconnue (« ' + dEntree + ' »)');
                if (dSortie != null && !dateSortieISO) skipped.push('Feuille "' + sheetName + '", ligne ' + rr2 + ' : date de sortie non reconnue (« ' + dSortie + ' »)');

                if (dateEntreeISO) {
                  data.stockPneus.mouvements.push({
                    id: uid(), date: dateEntreeISO, referenceId: ref.id, type: 'ENTREE',
                    quantite: 1, prixUnitaire: null, fournisseur: '',
                    matricule: null, position: null, km: null,
                    observations: 'Import historique (1 feuille par référence)',
                    saisiPar: 'Import Excel', dateSaisie: new Date().toISOString()
                  });
                  added++; entrees++;
                }
                if (dateSortieISO) {
                  var matMatched = matchVehicleMatricule(matRaw);
                  var matriculeFinal = matMatched || (matRaw != null ? String(matRaw).trim() : null);
                  if (!matriculeFinal) {
                    skipped.push('Feuille "' + sheetName + '", ligne ' + rr2 + ' : sortie sans matricule — ignorée');
                  } else {
                    data.stockPneus.mouvements.push({
                      id: uid(), date: dateSortieISO, referenceId: ref.id, type: 'SORTIE',
                      quantite: 1, prixUnitaire: null, fournisseur: null,
                      matricule: matriculeFinal, position: null, km: null,
                      observations: 'Import historique (1 feuille par référence)',
                      saisiPar: 'Import Excel', dateSaisie: new Date().toISOString()
                    });
                    added++; sorties++;
          ...
          // FIN BLOC ADDITIF (l'accolade ci-dessus ouvre le FORMAT 1 existant, inchangé)

          for (var row = headerRow + 1; row <= rowCount; row++) {
            var a = cellAt(row, 1), b = cellAt(row, 2), c = cellAt(row, 3),
                d = cellAt(row, 4), e = cellAt(row, 5), h = cellAt(row, 8);
          ...
          } // FIN BLOC ADDITIF — fermeture du else{} du FORMAT 1
    ...
    s.onload = run;
    s.onerror = function () { toast("Impossible de charger la bibliothèque de lecture Excel (XLSX). Vérifiez la connexion internet.", 'error'); };
    document.head.appendChild(s);
  }
