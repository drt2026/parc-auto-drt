...
class ParcAutoApp {
  ...
  // ============================================
  // 🔧 NOUVEAU: SYNCHRONISATION TEMPS SERVEUR
  // ============================================
  ...
  async getServerTime() {
    ...
    try {
      ...
      // Fallback: utiliser le header Date de la réponse HEAD
      try {
        const response = await fetch(WORKER_URL, {
          method: 'HEAD',
          headers: { 'X-App-Key': APP_KEY }
  ...
  // 🔧 NOUVEAU: Merge intelligent des véhicules par _lastModified
  mergeVehicleData(localVehicles, cloudVehicles) {
    const vehicleMap = new Map();

    // Ajouter tous les véhicules locaux
    (localVehicles || []).forEach(v => {
      vehicleMap.set(v.id, { ...v });
    });

    // Merge avec les véhicules cloud
    (cloudVehicles || []).forEach(cv => {
      const lv = vehicleMap.get(cv.id);
      if (!lv) {
        // Véhicule uniquement dans le cloud
        vehicleMap.set(cv.id, { ...cv });
      } else {
        // Véhicule dans les deux — prendre le plus récent par _lastModified
        const localModified = lv._lastModified || lv._lastSaved || 0;
        const cloudModified = cv._lastModified || cv._lastSaved || 0;

        if (cloudModified > localModified) {
          vehicleMap.set(cv.id, { ...cv }); // Cloud plus récent
        }
        // Sinon garder local (plus récent ou égal)
      }
    });
  ...
  // 🔧 NOUVEAU: Merge des réparations (éviter les doublons)
  mergeRepairs(localRepairs, cloudRepairs) {
    ...
    (cloudRepairs || []).forEach(cr => {
      if (!repairMap.has(cr.id)) {
        repairMap.set(cr.id, cr);
      }
    });
  ...
  async loadData() {
    ...
    // 🔧 FIX: Synchroniser l'horloge avec le serveur pour éviter les décalages
    const serverTime = await this.getServerTime();
    this._timeOffset = serverTime ? (serverTime - Date.now()) : 0;
      ...
      if (this.isCloudConfigured()) {
        try {
          ...
          if (cloudData && cloudData.vehicles) {
            const cloudTs = (cloudData._lastSaved || 0) + this._timeOffset;
            const localTs = (localData._lastSaved || 0);

            // 🔧 FIX: Merge intelligent au lieu de remplacer brutalement
            if (cloudTs > localTs + 5000) { // Tolérance de 5 secondes
              console.log('Admin: cloud plus récent — merge intelligent');
              this.isCloudSync = true;

              // Merge: garder le plus récent pour chaque véhicule
              const merged = this.mergeVehicleData(
                localData.vehicles || [],
                cloudData.vehicles || []
              );
              cloudData.vehicles = merged;

              // Préserver la session admin locale
    ...
    if (this.isCloudConfigured()) {
      ...
      if (cloudData && cloudData.vehicles) {
        ...
        // 🔧 FIX: Merge au lieu de simple comparaison timestamp
        if (localData && localTs > cloudTs + 5000) {
          console.log('Local plus récent que cloud — merge et repush');
          this.isCloudSync = true;

          // Merge: fusionner local + cloud
          const merged = this.mergeVehicleData(
            localData.vehicles || [],
            cloudData.vehicles || []
          );
          localData.vehicles = merged;
          localData.repairs = this.mergeRepairs(
            localData.repairs || [],
            cloudData.repairs || []
          );
        ...
        if (cloudTs > localTs + 5000) {
          console.log('Cloud plus récent — merge avec données locales');
          this.isCloudSync = true;

          // 🔧 FIX: Merge intelligent des véhicules
          const mergedVehicles = this.mergeVehicleData(
            localData ? (localData.vehicles || []) : [],
            cloudData.vehicles || []
          );
          cloudData.vehicles = mergedVehicles;

          // Merge des réparations aussi
          cloudData.repairs = this.mergeRepairs(
            localData ? (localData.repairs || []) : [],
            cloudData.repairs || []
          );

          const localUserVehicle = localData ? localData.userVehicle : null;
          const localCurrentUser = localData ? localData.currentUser : null;
          this.data = cloudData;
        ...
        // 🔧 FIX: Si timestamps proches (moins de 5s), faire un merge quand même
        console.log('Timestamps proches — merge de sécurité');
        const merged = this.mergeVehicleData(
          localData ? (localData.vehicles || []) : [],
          cloudData.vehicles || []
        );
  ...
  async saveData(modifiedVehicleId) {
    ...
    // 2. Sync cloud avec retry et confirmation
    if (this.isCloudConfigured()) {
      this._isSaving = true;
      this.showSyncStatus('syncing');

      // 🔧 FIX: Lire le cloud AVANT d'écrire pour éviter d'écraser
      // des modifications faites sur un autre appareil
      try {
        const cloudData = await this.readFromCloud();
        if (cloudData && cloudData.vehicles) {
          ...
          // Si le cloud a des données plus récentes, merge d'abord
          if (cloudTs > localTs - 10000) { // Tolérance 10s
            console.log('saveData: données cloud plus récentes détectées, merge...');
            const mergedVehicles = this.mergeVehicleData(
              this.data.vehicles || [],
              cloudData.vehicles || []
            );
            this.data.vehicles = mergedVehicles;
            this.data.repairs = this.mergeRepairs(
              this.data.repairs || [],
              cloudData.repairs || []
            );
          }
        ...
        console.warn('saveData: erreur lecture pré-écriture:', e);
      }
  ...
  startAutoSync() {
    ...
    this._autoSyncInterval = setInterval(async () => {
      ...
      if (cloudData && cloudData.vehicles) {
        // 🔧 FIX: Toujours faire un merge, pas seulement si cloud > local
        const mergedVehicles = this.mergeVehicleData(
          this.data.vehicles || [],
          cloudData.vehicles || []
        );
        const mergedRepairs = this.mergeRepairs(
          this.data.repairs || [],
          cloudData.repairs || []
        );

        // Vérifier si des changements ont été détectés
        const hasChanges = JSON.stringify(this.data.vehicles) !== JSON.stringify(mergedVehicles) ||
                         JSON.stringify(this.data.repairs) !== JSON.stringify(mergedRepairs);

        if (hasChanges) {
          console.log('AutoSync: changements détectés, mise à jour...');

          const localUserVehicle = this.data.userVehicle;
          const localCurrentUser = this.data.currentUser;

          this.data.vehicles = mergedVehicles;
          this.data.repairs = mergedRepairs;
          this.data.userVehicle = localUserVehicle;
          this.data.currentUser = localCurrentUser;
          this.data._lastSaved = Date.now();
          ...
          // 🔧 FIX: Montrer une notification discrète de sync
          if (typeof this.showToast === 'function') {
            this.showToast('🔄 Données synchronisées depuis le cloud', 'info');
          }
        }
      }
    }, 30000);
    ...
    if (this.isCloudConfigured()) {
      try {
        const timeoutPromise = new Promise((_, reject) =>
          setTimeout(() => reject(new Error('timeout')), 15000) // 🔧 Augmenté à 15s
        );

        // 🔧 FIX: Lire d'abord le cloud pour merge
        ...
        if (cloudData && cloudData.vehicles) {
          ...
          // 🔧 FIX: Toujours faire un merge avant de décider
          const mergedVehicles = this.mergeVehicleData(
            this.data.vehicles || [],
            cloudData.vehicles || []
          );
          ...
          if (localTs > cloudTs + 5000) {
            // Local plus récent — repush après merge
            this.data.vehicles = mergedVehicles;
            this.data.repairs = mergedRepairs;
          ...
          // Cloud plus récent ou égal — mettre à jour local
          const localUserVehicle = this.data.userVehicle;
          const localCurrentUser = this.data.currentUser;
          ...
          this.showSyncStatus('synced');
          this.showToast('☁️ Synchronisation terminée — données fusionnées', 'success');
          return;
        }
      } catch(e) {
