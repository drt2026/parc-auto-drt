...
class ParcAutoApp {
  ...
  async loadData() {
    ...
    // Si session admin active → retourner localStorage immédiatement sans cloud
  ...
  startAutoSync() {
    ...
    this._autoSyncInterval = setInterval(async () => {
      ...
      if (cloudData && cloudData.vehicles) {
        ...
        if (cloudTs > localTs) {
          // Cloud plus récent → fusionner en préservant session
...
function checkAndNotifyAlerts() {
  try {
    ...
    (data.vehicles || []).forEach(v => {
      ...
      // ENVOI AUTOMATIQUE WHATSAPP — chef uniquement (pas de message aux chauffeurs)
      if (vAlerts.length > 0) {
 ...
 * puis les envoie via notification cliquable.
 * UNIQUEMENT pour le chef de parc (recipient === 'chef').
 * Les chauffeurs ne reçoivent aucun message automatique.
 */
function queueWhatsAppMessage(vehicle, alerts, today, recipient) {
  // Bloquer tout envoi vers les chauffeurs
  if (recipient !== 'chef') return;

  const todayStr = today.toISOString().split('T')[0];
  const dedupeKey = `wa_chef_${vehicle.matricule}_${todayStr}`;
  if (localStorage.getItem(dedupeKey)) return;

  const dateStr = today.toLocaleDateString('fr-FR');
  const alertLines = alerts.map(a => `⚠️ ${a}`).join('\n');

  const phone = '21698230530';
...
_Parc Auto DRT Sfax — Tunisie Telecom_`;
  ...
  if (Notification.permission === 'granted') {
    try {
      const pendingKey = `wa_pending_${vehicle.matricule}_chef_${todayStr}`;
      localStorage.setItem(pendingKey, waUrl);

      const notif = new Notification(`🚨 Alerte WhatsApp — ${vehicle.matricule}`, {
        body: `Cliquez pour envoyer l'alerte WhatsApp au Chef de parc\n${alerts[0] || ''}`,
        icon: 'icon-192.png',
        badge: 'icon-192.png',
        tag: `wa-chef-${vehicle.matricule}`,
        requireInteraction: true,
      ...
      localStorage.setItem(dedupeKey, '1');
      console.log(`✅ Notification WhatsApp chef créée pour ${vehicle.matricule}`);
      return;
    } catch(e) {
...
function autoSendWhatsAppToChauffeur(vehicle, alerts, today) {
  // Désactivé — seul le chef de parc reçoit les alertes WhatsApp automatiques
}
...
async function saveUserKm(matricule) {
  ...
  // 🔧 FIX: Utiliser saveData() qui gère _isSaving, _saveQueue et le cloud correctement
