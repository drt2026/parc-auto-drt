    ...
    container.innerHTML = `
      <div class="vehicle-detail-card">
        ...
        <div class="vehicle-detail-body">
          ...
          <!-- BOUTON MODIFIER INDEX ACTUEL -->
          <div style="margin-top:16px;padding:16px;background:linear-gradient(135deg,rgba(16,185,129,0.05),rgba(59,130,246,0.05));border:1px solid rgba(16,185,129,0.15);border-radius:12px;">
            <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;">
              <div>
                <div style="font-size:13px;font-weight:700;color:#1e293b;margin-bottom:4px;">✏️ Modifier l'index actuel</div>
                <div style="font-size:11px;color:#64748b;">Mettez à jour le kilométrage de votre véhicule</div>
              </div>
              <button onclick="openUserIndexModal('${vehicle.matricule}', ${vehicle.km})"
                style="background:linear-gradient(135deg,#10b981,#059669);color:white;border:none;
                padding:10px 20px;border-radius:10px;font-size:13px;font-weight:700;cursor:pointer;
                box-shadow:0 3px 12px rgba(16,185,129,0.3);display:flex;align-items:center;gap:6px;
                transition:all 0.2s;"
    ...
    try {
      ...
      for (let i = headerIdx + 2; i < rowsVeh.length; i++) {
        ...
        vehicles.push({
          id: 'v_' + Date.now() + '_' + i,
          matricule: mat,
          modele: String(r[1] || '').trim(),
          chauffeur: String(r[2] || '').trim(),
          whatsappChauffeur: String(r[3] || '').replace(/\D/g, '') || null,
          km: parseInt(r[4]) || 0,
          prochaineVidange: parseInt(r[5]) || 0,
          prochaineChaine: parseInt(r[6]) || 0,
          prochaineVisite: fmtDate(r[7]),
          dateChangementBatterie: fmtDate(r[8]),
          indiceBatterie: String(r[9] || '').trim(),
          dateChangementPneus: fmtDate(r[10]),
          statut: String(r[11] || 'actif').trim().toLowerCase() || 'actif'
        });
      }
...
// ============================================
// MODIFICATION INDEX PAR L'UTILISATEUR
// ============================================

let _userEditingMatricule = null;
let _userCurrentKm = 0;

function openUserIndexModal(matricule, currentKm) {
  _userEditingMatricule = matricule;
  _userCurrentKm = currentKm || 0;

  // Créer le modal s'il n'existe pas
  let modal = document.getElementById('user-index-modal-overlay');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'user-index-modal-overlay';
    modal.className = 'index-modal-overlay';
    modal.innerHTML = `
      <div class="index-modal" style="max-width:380px;">
        <h3>📊 Modifier l'index actuel</h3>
        <p class="im-sub" id="uim-vehicle-label">Véhicule —</p>
        <div class="im-current">
          <div>
            <div class="im-current-label">Index actuel</div>
            <div class="im-current-val" id="uim-current-val">—</div>
          </div>
          <span style="font-size:24px;">📊</span>
        </div>
        <p class="im-warning">⚠️ Saisissez le kilométrage affiché sur votre compteur</p>
        <input type="number" id="uim-new-index" placeholder="Ex: 229604" min="0" style="width:100%; padding:12px 14px; border-radius:10px; border:2px solid rgba(16,185,129,0.3); font-size:18px; font-family:'IBM Plex Mono',monospace; font-weight:700; color:#1e293b; outline:none; text-align:center; transition: border-color 0.2s; margin-bottom:16px;">
        <div class="im-btns" style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
          <button class="im-btn-cancel" onclick="closeUserIndexModal()" style="padding:12px; border-radius:10px; border:1px solid rgba(0,0,0,0.1); background:rgba(0,0,0,0.04); color:#64748b; font-weight:600; font-size:13px; cursor:pointer;">Annuler</button>
          <button class="im-btn-save" onclick="saveUserNewIndex()" style="padding:12px; border-radius:10px; border:none; background:linear-gradient(135deg,#10b981,#059669); color:white; font-weight:700; font-size:13px; cursor:pointer; box-shadow:0 4px 12px rgba(16,185,129,0.3);">✅ Enregistrer</button>
        </div>
      </div>
    `;
  ...
  document.getElementById('uim-vehicle-label').textContent = 'Véhicule ' + matricule;
  document.getElementById('uim-current-val').textContent = currentKm ? currentKm.toLocaleString('fr-FR') : '—';
  document.getElementById('uim-new-index').value = currentKm || '';

  modal.classList.add('open');
  setTimeout(() => document.getElementById('uim-new-index').focus(), 100);
}

function closeUserIndexModal() {
  const modal = document.getElementById('user-index-modal-overlay');
  if (modal) modal.classList.remove('open');
  _userEditingMatricule = null;
  _userCurrentKm = 0;
}

function saveUserNewIndex() {
  const newVal = parseInt(document.getElementById('uim-new-index').value);
  if (!newVal || newVal < 0) {
    document.getElementById('uim-new-index').style.borderColor = '#ef4444';
    return;
  }
  if (!_userEditingMatricule) return;

  // Mettre à jour les données du véhicule
  if (parcAuto && parcAuto.data && parcAuto.data.vehicles) {
    const vehicle = parcAuto.data.vehicles.find(v => v.matricule === _userEditingMatricule);
    if (vehicle) {
      ...
      parcAuto.saveData().then(() => {
        ...
        // Ajouter à l'historique si la fonction existe
        if (typeof addToFuelHistory === 'function') {
          addToFuelHistory(_userEditingMatricule, newVal, 'user');
        }

        // Toast de succès
        const t = document.createElement('div');
        t.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:#10b981;color:white;padding:12px 20px;border-radius:10px;font-size:13px;font-weight:700;z-index:9999;box-shadow:0 4px 16px rgba(16,185,129,0.4);';
        t.textContent = '✅ Index mis à jour : ' + oldKm.toLocaleString('fr-FR') + ' → ' + newVal.toLocaleString('fr-FR') + ' km';
