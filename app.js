...
class ParcAutoApp {
  ...
  async writeToCloud() {
    ...
    try {
      ...
      const response = await fetch(WORKER_URL, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-Action': 'update' },
...
async function saveUserKm(matricule) {
  ...
  // Sauvegarder en local d'abord (immédiat)
  try {
    parcAuto.data._lastSaved = Date.now();
    localStorage.setItem('parcAutoData_v3', JSON.stringify(parcAuto.data));
  } catch(e) {}

  // Sync cloud fire & forget
  if (parcAuto.isCloudConfigured()) {
    parcAuto.writeToCloud().catch(() => {});
    parcAuto.showSyncStatus('syncing');
    setTimeout(() => parcAuto.showSyncStatus('synced'), 2000);
  }
