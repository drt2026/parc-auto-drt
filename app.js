          ...
          if (typeof showTips === 'function') {
            showTips(() => {
              ...
              if (userAlerts.length > 0) {
                setTimeout(() => this.speakAlerts(userAlerts), 500);
              }
            });
          } else {
            document.getElementById('user-interface').style.display = 'block';
    ...
    container.innerHTML = `
      <div class="vehicle-detail-card">
        ...
        <div class="vehicle-detail-body">
          ...
          <div class="detail-section">
            <div class="detail-section-title">🪪 Carte Grise</div>
            <div class="carte-grise-container" id="carte-grise-${matricule}">
              <div class="carte-grise-side">
                <div class="carte-grise-label">RECTO — ${matricule}</div>
                <div class="carte-grise-photo" id="cg-recto-${matricule}">
                  <div class="carte-grise-placeholder">
                    <span style="font-size:32px">📷</span>
                    <span style="font-size:12px;color:#94a3b8;margin-top:6px;">Recto non disponible</span>
                  </div>
                </div>
              </div>
              <div class="carte-grise-side">
                <div class="carte-grise-label">VERSO — ${matricule.replace(/-/g, '')}</div>
                <div class="carte-grise-photo" id="cg-verso-${matricule}">
                  <div class="carte-grise-placeholder">
                    <span style="font-size:32px">📷</span>
                    <span style="font-size:12px;color:#94a3b8;margin-top:6px;">Verso non disponible</span>
                  </div>
                </div>
              </div>
            </div>
...
<div style="text-align:center;padding-top:16px;border-top:1px solid var(--border);margin-top:16px;display:flex;gap:12px;justify-content:center;flex-wrap:wrap;">
    ...
    // Charger les images de carte grise depuis Drive
    setTimeout(() => this.loadCarteGriseImages(matricule), 500);
  }
  ...
  async loadCarteGriseImages(matricule) {
    if (!window.driveManager || !window.driveManager.isSignedIn) return;

    try {
      const safeMat = matricule.replace(/[^a-zA-Z0-9-]/g, '_');
      const files = await window.driveManager.listFiles('CARTES_GRISES', matricule);

      const rectoFile = files.find(f => f.name.includes('_RECTO_'));
      const versoFile = files.find(f => f.name.includes('_VERSO_'));

      if (rectoFile) {
        const rectoEl = document.getElementById(`cg-recto-${matricule}`);
        if (rectoEl) {
          rectoEl.innerHTML = `<img src="${window.driveManager.getDisplayUrl(rectoFile.id)}" 
            alt="Carte grise recto ${matricule}" 
            style="width:100%;height:180px;object-fit:cover;border-radius:8px;cursor:pointer;"
            onclick="window.open('${rectoFile.webViewLink}', '_blank')">`;
        }
      }
      ...
      if (versoFile) {
        ...
        if (versoEl) {
          versoEl.innerHTML = `<img src="${window.driveManager.getDisplayUrl(versoFile.id)}" 
            alt="Carte grise verso ${matricule}" 
            style="width:100%;height:180px;object-fit:cover;border-radius:8px;cursor:pointer;"
            onclick="window.open('${versoFile.webViewLink}', '_blank')">`;
        }
      }
    } catch(e) {
      console.warn('Erreur chargement carte grise:', e);
    }
  }
