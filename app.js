...
class ParcAutoApp {
  ...
  initUserLogin() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      // Fallback sur plusieurs IDs possibles
      const agentEl = document.getElementById('user-matricule-agent') 
        || document.getElementById('agent-matricule')
        || document.getElementById('matricule-agent')
        || document.querySelector('input[name="agent-matricule"]')
        || document.querySelector('input[placeholder*="Agent"]');
      const vehicleEl = document.getElementById('user-matricule-vehicle')
        || document.getElementById('vehicle-matricule')
        || document.getElementById('matricule-vehicle')
        || document.querySelector('input[name="vehicle-matricule"]')
        || document.querySelector('input[placeholder*="Véhicule"]');

      const agentInput = agentEl?.value?.trim().toUpperCase();
      const vehicleInput = vehicleEl?.value?.trim().toUpperCase();

      console.log('DEBUG: agentEl found?', !!agentEl, 'value=', agentEl?.value);
      console.log('DEBUG: vehicleEl found?', !!vehicleEl, 'value=', vehicleEl?.value);
