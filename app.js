  ...
  async writeToCloud() {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        // 🔧 FIX: Utiliser POST au lieu de PATCH pour éviter les problèmes CORS preflight
        // ⚠️ Ne jamais envoyer currentUser/userVehicle/activityLogs au cloud
        // activityLogs = volumineux + locaux uniquement
        const { currentUser: _cu, userVehicle: _uv, activityLogs: _logs, ...cloudPayload } = this.data;
        const response = await fetch(WORKER_URL, {
          method: 'POST',
        ...
        try {
          ...
          if (cloudData && cloudData.vehicles) {
            ...
            if (cloudTs >= localTs) {
              ...
              // Ne pas écraser activityLogs locaux si plus récents
              if (localData.activityLogs && (!cloudData.activityLogs || localData.activityLogs.length > cloudData.activityLogs.length)) {
                cloudData.activityLogs = localData.activityLogs;
              }
    ...
    this._autoSyncInterval = setInterval(async () => {
      ...
      if (cloudData && cloudData.vehicles) {
        ...
        if (cloudTs > localTs) {
          ...
          // Garder les logs locaux s'ils sont plus nombreux
          if (localActivityLogs && (!this.data.activityLogs || localActivityLogs.length > (this.data.activityLogs || []).length)) {
            this.data.activityLogs = localActivityLogs;
          }
          try {
            localStorage.setItem('parcAutoData_v3', JSON.stringify(this.data));
          } catch(e) {}
