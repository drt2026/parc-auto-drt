...
async function requestOTPEmail(vehicleData) {
  ...
  sessionStorage.setItem('_otp_attempts', '0');

  // 🔓 AFFICHAGE DU CODE OTP SUR LA PAGE (pour débogage/développement)
  // En production, supprimez ce bloc ou commentez-le
  const otpDisplay = document.createElement('div');
  otpDisplay.id = 'otp-debug-display';
  otpDisplay.style.cssText = 'position:fixed;top:20px;right:20px;z-index:99999;background:#1e293b;color:#fff;padding:16px 20px;border-radius:12px;font-family:monospace;font-size:18px;box-shadow:0 8px 32px rgba(0,0,0,0.3);border:2px solid #3b82f6;';
  otpDisplay.innerHTML = `<div style="font-size:11px;color:#94a3b8;margin-bottom:4px;">🔐 CODE OTP (DEV MODE)</div><div style="font-size:24px;font-weight:700;letter-spacing:4px;">${otp}</div><div style="font-size:10px;color:#94a3b8;margin-top:4px;">Valable 10 minutes</div>`;
  document.body.appendChild(otpDisplay);
  console.log('🔐 CODE OTP AFFICHÉ:', otp);
  ...
  try {
    ...
    // Même si l'email échoue, le code est affiché sur la page
    return { ok: true }; // Retourner ok car le code est visible
  }
}

function verifyOTP(inputCode) {
  console.log('🔍 verifyOTP appelé avec:', inputCode);
  ...
  if (!OTP_CONFIG.code) {
    ...
    const attempts = sessionStorage.getItem('_otp_attempts');
    console.log('📦 Restauration sessionStorage:', { saved, expiry, attempts });
    if (saved && expiry) { 
      OTP_CONFIG.code = saved; 
      OTP_CONFIG.expiry = parseInt(expiry); 
      OTP_CONFIG.attempts = parseInt(attempts || '0');
    }
  }
  console.log('🔐 OTP_CONFIG actuel:', { code: OTP_CONFIG.code, expiry: OTP_CONFIG.expiry, attempts: OTP_CONFIG.attempts });
  ...
  OTP_CONFIG.attempts++;
  sessionStorage.setItem('_otp_attempts', OTP_CONFIG.attempts.toString());
  if (OTP_CONFIG.attempts > OTP_CONFIG.maxAttempts) return { ok: false, msg: 'Trop de tentatives. Reconnectez-vous.' };
  console.log('🔍 Comparaison:', inputCode.trim(), '===', OTP_CONFIG.code, '?', inputCode.trim() === OTP_CONFIG.code);
...
async function validateOTPAndLogin() {
  console.log('🚀 validateOTPAndLogin appelé');
  if (validateOTPAndLogin._running) { console.log('⛔ Déjà en cours'); return; }
  ...
  if (result.ok) {
    console.log('✅ Résultat OTP ok, préparation redirection...');
    validateOTPAndLogin._running = false; // 🔧 Réinitialiser le flag
    document.getElementById('otp-overlay')?.remove();
    ...
    if (!OTP_CONFIG.pendingAdmin) {
      ...
      console.log('📦 Admin sessionStorage:', saved);
    ...
    if (OTP_CONFIG.pendingAdmin) {
      ...
      if (window.parcAuto) {
        ...
        if (verify.currentUser && verify.currentUser.role === 'admin') {
          ...
          // Forcer la fermeture de l'overlay avant redirection
          document.getElementById('otp-overlay')?.remove();
          // Redirection avec petit délai pour laisser le DOM se mettre à jour
          setTimeout(() => {
...
async function resendOTPWhatsApp() {
  ...
  if (result.ok) {
    // 🔧 FIX: Le nouveau code est maintenant dans OTP_CONFIG, le sauvegarder dans sessionStorage
    if (OTP_CONFIG.code) {
      sessionStorage.setItem('_otp_code', OTP_CONFIG.code);
      sessionStorage.setItem('_otp_expiry', OTP_CONFIG.expiry.toString());
      sessionStorage.setItem('_otp_attempts', '0');
      console.log('🔄 Nouveau code OTP stocké:', OTP_CONFIG.code);
    }
...
function cancelOTP() {
  ...
  sessionStorage.removeItem('_otp_code');
  sessionStorage.removeItem('_otp_expiry');
  sessionStorage.removeItem('_otp_attempts');
  sessionStorage.removeItem('_otp_admin');
  validateOTPAndLogin._running = false;
}
