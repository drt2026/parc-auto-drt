...
class ParcAutoApp {
  ...
  async initAdminLogin() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (user) {
        // MODE DÉVELOPPEMENT - Connexion directe sans OTP
        // Pour activer l'OTP en production, remplacez ce bloc par le code commenté ci-dessous

        this.data.currentUser = { email: user.email, role: user.role, name: user.name };
        this.data.userVehicle = null;
        localStorage.setItem('parcAutoData_v3', JSON.stringify(this.data));
        this.showToast('🔓 Connexion admin (mode dev)', 'success');
        window.location.replace('admin.html');
        return;

        /* CODE OTP PRODUCTION (décommentez pour utiliser)
...
function validateOTPAndLogin() {
  ...
  if (result.ok) {
    ...
    // IMPORTANT: Réinitialiser le flag pour permettre une nouvelle tentative
