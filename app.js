...
class ParcAutoApp {
  ...
  async saveData() {
    ...
    // 2. Sync cloud — sérialisé pour éviter les écritures concurrentes qui s'écrasent
    if (this.isCloudConfigured()) {
      this.showSyncStatus('syncing');
      // 🔧 FIX: reset queue on previous error so future saves aren't blocked
  ...
  startAutoSync() {
    ...
    this._autoSyncInterval = setInterval(async () => {
      ...
      if (cloudData && cloudData.vehicles) {
        ...
        if (localTs > cloudTs) {
          // Local plus récent → repousser vers le cloud
...
async function saveUserKm(matricule) {
  ...
  // 🔧 FIX: Fermer le modal IMMÉDIATEMENT pour éviter double-clic
  document.getElementById('user-km-modal')?.remove();

  if (window.parcAuto && parcAuto.data && parcAuto.data.vehicles) {
    ...
    if (vehicle) {
      ...
      // 🔧 FIX: Sauvegarder, puis recharger depuis localStorage pour éviter écrasement cloud
      await parcAuto.saveData();

      // Recharger les données fraîches depuis localStorage (pas le cloud)
      try {
        const saved = localStorage.getItem('parcAutoData_v3');
        if (saved) {
          const fresh = JSON.parse(saved);
          // Préserver session
          fresh.userVehicle = parcAuto.data.userVehicle;
          fresh.currentUser = parcAuto.data.currentUser;
          parcAuto.data = fresh;
        }
      } catch(e) {}
...
async function saveUserVisite(matricule) {
  ...
  // 🔧 FIX: Fermer le modal IMMÉDIATEMENT
  document.getElementById('user-visite-modal')?.remove();
  ...
  if (window.parcAuto && parcAuto.data && parcAuto.data.vehicles) {
    ...
    if (vehicle) {
      ...
      // 🔧 FIX: Recharger depuis localStorage frais
      try {
        const saved = localStorage.getItem('parcAutoData_v3');
        if (saved) {
          const fresh = JSON.parse(saved);
