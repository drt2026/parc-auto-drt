  ...
  initUserLogin() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (this.isCloudConfigured()) {
        const cloudData = await this.readFromCloud();
        if (cloudData && cloudData.vehicles) {
          // Fusion: garder les véhicules locaux + ajouter ceux du cloud qui n'existent pas localement
          const localMats = new Set((this.data.vehicles || []).map(v => v.matricule));
          const mergedVehicles = [...(this.data.vehicles || [])];
          (cloudData.vehicles || []).forEach(cv => {
            if (!localMats.has(cv.matricule)) {
