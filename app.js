  ...
  initUserLogin() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (vehicle) {
        ...
        function formatMatriculeVoix(mat) {
          const parts = mat.split('-');
          if (parts.length === 2) {
            const prefix = parts[0];
            const nums = parts[1];
            // Convertir chaque partie en texte français phonétique
            const numToText = (n) => {
              ...
              if (num < 70) {
                ...
                if (unit === 1 && ten === 7) return tens[ten] + ' et ' + units[unit];
                return tens[ten] + '-' + units[unit];
              }
              if (num < 80) {
                const unit = num % 10;
                if (unit === 0) return 'soixante-' + teens[10];
              ...
              if (num < 1000) {
                ...
                if (hundred === 1) {
                  ...
                  return 'cent-' + numToText(rest);
                }
                if (rest === 0) return units[hundred] + '-cents';
                return units[hundred] + '-cent-' + numToText(rest);
              }
              return n;
            };
            return numToText(prefix) + ', ' + numToText(nums.slice(0,3)) + ', ' + numToText(nums.slice(3,6));
          }
          return mat;
        }
        const matriculeFormate = formatMatriculeVoix(matricule);

        const speakWelcome = (callback) => {
          if (!window.speechSynthesis) { callback(); return; }
          const msg = `Bienvenue ${prenom}, vous êtes dans l'interface de votre véhicule ${modele} ${matriculeFormate}.`;
          const utter = new SpeechSynthesisUtterance(msg);
          utter.lang  = 'fr-FR';
          utter.rate  = 0.80;
          utter.pitch = 0.85;
          const voices = window.speechSynthesis.getVoices();
          // Essayer de trouver une voix différente (plus grave/masculine)
        ...
        setTimeout(() => {
          ...
          if (typeof showTips === 'function') {
            showTips(() => {
              ...
              this.renderUserVehicleDetail();
              this.startAutoSync();
              // Message de bienvenue vocal après les conseils, puis alertes
              const vehicle = this.data.vehicles.find(v => v.matricule === matricule);
              const userAlerts = vehicle ? this.getAlerts().filter(a => a.vehicle.id === vehicle.id) : [];
              speakWelcome(() => {
                if (userAlerts.length > 0) {
                  setTimeout(() => this.speakAlerts(userAlerts), 800);
                }
              });
            });
          } else {
            ...
            this.renderUserVehicleDetail();
            this.startAutoSync();
            // Message de bienvenue puis alertes
            ...
            speakWelcome(() => {
              if (userAlerts2.length > 0) {
                setTimeout(() => this.speakAlerts(userAlerts2), 800);
              }
            });
          }
        }, 500);
    ...
    container.innerHTML = `
      <div class="vehicle-detail-card">
        ...
        <div class="vehicle-detail-body">
          ...
          <div class="cgw-wrapper" id="cgw-${matricule}">
            <div class="cgw-title">
              📋 Carte Grise — Certificat d'Immatriculation
              <span class="cgw-matricule">${matricule}</span>
            </div>
            <div class="cgw-sides">
              <!-- RECTO -->
              <div>
                <div class="cgw-side-label">Recto — ${matricule.replace('-', ' - ')}</div>
                <label class="cgw-dropzone" for="cg-recto-${matricule}">
                  <div class="cgw-preview-wrap" id="cgw-recto-preview-${matricule}">
                    ${vehicle.carteGrise && vehicle.carteGrise.recto
                      ? `<img src="${vehicle.carteGrise.recto}" style="width:100%;height:120px;object-fit:cover;display:block;" alt="Recto ${matricule}">`
                      : `<div class="cgw-placeholder"><div style="font-size:28px;">🪪</div><div style="font-size:12px;color:#64748b;margin-top:6px;">Photo recto</div><div style="font-size:11px;color:#94a3b8;">${matricule}</div></div>`
                    }
                  </div>
                  <div class="cgw-side-status" id="cgw-recto-status-${matricule}">
                    ${vehicle.carteGrise && vehicle.carteGrise.recto ? '✅ ' + matricule + '.jpg' : '📷 Cliquer pour ajouter'}
                  </div>
                </label>
                <input type="file" id="cg-recto-${matricule}" accept="image/*" style="display:none"
                  onchange="cgUpload(this,'${matricule}','recto')">
              </div>
              <!-- VERSO -->
              <div>
                <div class="cgw-side-label">Verso — ${matricule.split('-')[1] || matricule}</div>
                <label class="cgw-dropzone" for="cg-verso-${matricule}">
                  <div class="cgw-preview-wrap" id="cgw-verso-preview-${matricule}">
                    ${vehicle.carteGrise && vehicle.carteGrise.verso
                      ? `<img src="${vehicle.carteGrise.verso}" style="width:100%;height:120px;object-fit:cover;display:block;" alt="Verso ${matricule}">`
                      : `<div class="cgw-placeholder"><div style="font-size:28px;">🪪</div><div style="font-size:12px;color:#64748b;margin-top:6px;">Photo verso</div><div style="font-size:11px;color:#94a3b8;">${matricule.split('-')[1] || matricule}</div></div>`
                    }
                  </div>
                  <div class="cgw-side-status" id="cgw-verso-status-${matricule}">
                    ${vehicle.carteGrise && vehicle.carteGrise.verso ? '✅ ' + (matricule.split('-')[1] || matricule) + '.jpg' : '📷 Cliquer pour ajouter'}
                  </div>
                </label>
                <input type="file" id="cg-verso-${matricule}" accept="image/*" style="display:none"
                  onchange="cgUpload(this,'${matricule}','verso')">
              </div>
            </div>
          </div>
...
// ============================================================
//  CARTE GRISE — UPLOAD RECTO/VERSO
//  Enregistre l'image en base64 dans les données du véhicule
// ============================================================
function cgUpload(input, matricule, side) {
  ...
  reader.onload = function(e) {
    ...
    const previewDiv = document.getElementById(`cgw-${side}-preview-${matricule}`);
    const statusDiv  = document.getElementById(`cgw-${side}-status-${matricule}`);
    if (previewDiv) {
      previewDiv.innerHTML = `<img src="${base64}" style="width:100%;height:120px;object-fit:cover;display:block;" alt="${side} ${matricule}">`;
    }
    const label = side === 'recto' ? matricule : (matricule.split('-')[1] || matricule);
    if (statusDiv) statusDiv.textContent = `✅ ${label}.jpg`;

    // Sauvegarder dans les données
    if (parcAuto) {
      const v = parcAuto.data.vehicles.find(v => v.matricule === matricule);
      if (v) {
        if (!v.carteGrise) v.carteGrise = { recto: null, verso: null };
        v.carteGrise[side] = base64;
        parcAuto.saveData();
        parcAuto.showToast(`Carte grise ${side} enregistrée — ${matricule}`, 'success');
      }
    }
  };
