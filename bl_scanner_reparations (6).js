    ...
    ensureTesseract(function () {
      ...
      function next() {
        if (i >= dataUrls.length) {
          ...
          showQueueItem(0);
          if (skipped > 0) {
            setStatus('✅ ' + blQueue.length + ' BL détecté(s) — ' + skipped + ' page(s) ignorée(s) (page blanche, récap facture, ou page sans "Bon de Livraison N°" identifiable).', 'info');
          }
          return;
        }
        setStatus('⏳ Analyse OCR — page ' + (i + 1) + ' / ' + dataUrls.length + ' (peut prendre 10–20 s chacune)…', 'info');
        Tesseract.recognize(dataUrls[i], 'fra', { tessedit_pageseg_mode: '4' })
          .then(function (res) {
            var raw = (res && res.data && res.data.text) || '';
            var parsed = parseBL(raw);
            // BLOC ADDITIF — une page n'est retenue que si un vrai numéro de
            // "Bon de Livraison N°" a été identifié. Ceci écarte automatiquement
            // les pages blanches, les récaps de Facture (en-tête "B.L No" sans
            // le mot "Livraison", regroupant plusieurs BL sur une page) et les
            // tableaux récapitulatifs finaux, qui ne sont pas des BL exploitables
            // un par un.
            if (parsed.blNumero) { blQueue.push(parsed); } else { skipped++; }
            i++; next();
          })
          .catch(function (err) {
            console.error(err);
            i++; next(); // on ignore la page en erreur et on continue avec les suivantes
          });
      }
      next();
    });
