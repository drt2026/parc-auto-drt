...
/* BLOC ADDITIF — Feuilles de détail mensuel (une par mois, structure identique au
   fichier "Suivi Carburant" importé : Qté/Index Début et Fin, Ravitaillement, Distance,
   Prix Litre, Quantité consommée, Taux consommation, Statut). Contrairement à la feuille
   "Par véhicule" (cumul annuel), ces feuilles reprennent les valeurs BRUTES telles
   qu'importées mois par mois, pour que le rapport Excel reflète fidèlement le fichier
   source — y compris les cases vides quand une donnée n'a pas été saisie (ex: Index Fin
   manquant pour un mois en cours). Ne modifie aucune feuille existante — vient en plus. */
const MONTH_SHEET_ABBR = ['Janv','Fev','Mars','Avril','Mai','Juin','Juil','Aout','Septembre','Octobre','Novembre','Decembre'];

function xlDataCellRaw(bg) {
  return {
    fill: { fgColor: { rgb: XLCLR[bg || 'whiteBg'] } },
    font: { color: { rgb: XLCLR.greyFg }, sz: 10 },
    alignment: { horizontal: 'right', vertical: 'center' },
    border: xlThinBorder(),
    numFmt: '#,##0.00'
  };
}
function xlDataCellText(bg, bold) {
  return {
    fill: { fgColor: { rgb: XLCLR[bg || 'whiteBg'] } },
    font: { bold: !!bold, color: { rgb: XLCLR.greyFg }, sz: 10 },
    alignment: { horizontal: 'left', vertical: 'center' },
...
function buildFuelMonthlyDetailSheetsXL(wb, allData, year) {
  const yy = String(year).slice(-2);
  for (let m = 1; m <= 12; m++) {
    const key = `${year}-${String(m).padStart(2, '0')}`;
    const entries = Object.values((allData && allData[key]) || {}).sort((a, b) => a.matricule.localeCompare(b.matricule));
    if (!entries.length) continue; // mois non importé → pas de feuille (comme avant)

    const ws = {};
    xlSetColWidths(ws, [12, 20, 11, 11, 11, 11, 13, 11, 10, 13, 12, 16]);
    xlAddMerge(ws, 0, 0, 0, 11);
    xlWriteCell(ws, 0, 0, `SUIVI CARBURANT — ${MONTH_SHEET_ABBR[m - 1]} ${year}`, xlHdr('navyBg', 'navyFg'));

    const headers = ['IMMAT.', 'MARQUE', 'Qté Début (L)', 'Index Début', 'Qté Fin (L)', 'Index Fin',
      'Ravitaillement (DT)', 'Distance (km)', 'Prix Litre (DT)', 'Qté Consommée (L)', 'Taux Conso (%)', 'Statut'];
    headers.forEach((c, i) => xlWriteCell(ws, 2, i, c, xlHdr('orangeBg', 'orangeFg')));

    entries.forEach((e, idx) => {
      const r = 3 + idx;
      const bg = idx % 2 === 0 ? 'whiteBg' : 'greyBg';
      const blank = (v) => (v === null || v === undefined) ? '' : v;
      xlWriteCell(ws, r, 0, e.matricule, xlDataCellText(bg, true));
      xlWriteCell(ws, r, 1, e.modele || '—', xlDataCellText(bg));
      xlWriteCell(ws, r, 2, blank(e.qteDebut), xlDataCellRaw(bg));
      xlWriteCell(ws, r, 3, blank(e.idxDebut), xlDataCellRaw(bg));
      xlWriteCell(ws, r, 4, blank(e.qteFin), xlDataCellRaw(bg));
      xlWriteCell(ws, r, 5, blank(e.idxFin), xlDataCellRaw(bg));
      xlWriteCell(ws, r, 6, +e.montant.toFixed(2), xlDataCellRaw(bg));
      xlWriteCell(ws, r, 7, e.km, xlDataCellRaw(bg));
      xlWriteCell(ws, r, 8, blank(e.prixLitre), xlDataCellRaw(bg));
      xlWriteCell(ws, r, 9, +e.litres.toFixed(2), xlDataCellRaw(bg));
      xlWriteCell(ws, r, 10, +e.pct.toFixed(2), xlDataCellRaw(bg));
      const statutStyle = e.statut && e.statut !== 'OK'
        ? { fill: { fgColor: { rgb: XLCLR.redBg } }, font: { bold: true, color: { rgb: XLCLR.redFg }, sz: 10 }, alignment: { horizontal: 'center', vertical: 'center' }, border: xlThinBorder() }
        : { fill: { fgColor: { rgb: XLCLR.greenBg } }, font: { bold: true, color: { rgb: XLCLR.greenFg }, sz: 10 }, alignment: { horizontal: 'center', vertical: 'center' }, border: xlThinBorder() };
      xlWriteCell(ws, r, 11, e.statut || 'OK', statutStyle);
    });

    const tr = 3 + entries.length;
    ws['!ref'] = `A1:L${tr + 1}`;
    // Nom de feuille limité à 31 caractères par Excel (déjà respecté ici : max "Septembre26" = 11).
    XLSX.utils.book_append_sheet(wb, ws, `${MONTH_SHEET_ABBR[m - 1]}${yy}`);
  }
}
// FIN BLOC ADDITIF
...
function buildFuelReportExcel(allData, year) {
  ...
  buildFuelMonthlyDetailSheetsXL(wb, allData, year); // BLOC ADDITIF — feuilles mensuelles détaillées
  return wb;
}
