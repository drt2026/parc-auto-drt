...
class ParcAutoApp {
  ...
  async sendAdminOtp() {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        this.showToast('❌ Envoi du code OTP impossible. Vérifiez la configuration (RESEND_TOKEN) et réessayez.', 'error');
      ...
      this.showToast('❌ Envoi du code OTP impossible (connexion réseau). Réessayez.', 'error');
      this._pendingAdminUser = null;
    }
  }
  ...
  async verifyAdminOtp() {
    ...
    if (entered !== this._adminOtp.code) {
      errorEl.textContent = `❌ Code incorrect (tentative ${this._adminOtp.attempts}/${this._adminOtp.maxAttempts})`;
      inputs.forEach(inp => inp.value = '');
      inputs[0].focus();
      return;
    }
