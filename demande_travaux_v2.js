  ...
  function initSuiviCarburantModal() {
    ...
    function renderTable(nom, mois) {
      ...
      var rowsHtml = vehicles.map(function(mat) {
        ...
        var debVal = entry ? entry.indexDebut : (lastIdx != null ? lastIdx : '');
        var debReadonly = (lastIdx != null) && (!entry || entry.indexDebut === lastIdx);
        var finVal = entry ? entry.indexFin : '';
        // BLOC ADDITIF — champs Quantité / Ravitaillement / Solde / Observations (TABLEAU_A_COMPLETER)
        var qDebVal = entry ? entry.qteDebut : (lastQte != null ? lastQte : '');
        var qDebReadonly = (lastQte != null) && (!entry || entry.qteDebut === lastQte);
        var qFinVal = (entry && entry.qteFin != null) ? entry.qteFin : '';
        var ravitVal = (entry && entry.ravitaillement != null) ? entry.ravitaillement : '';
        var soldeVal = (entry && entry.soldeCarte != null) ? entry.soldeCarte : '';
        var obsVal = entry ? String(entry.observations || '').replace(/"/g, '&quot;') : '';
        // FIN BLOC ADDITIF
        var distTxt = (entry && entry.indexFin != null && entry.indexDebut != null) ? (entry.indexFin - entry.indexDebut) : '—';
        return '<tr data-mat="' + mat + '">' +
          '<td style="font-weight:600">' + mat + '</td>' +
          '<td>' + marque + '</td>' +
          '<td><input type="number" class="tr3-qdeb" value="' + qDebVal + '"' + (qDebReadonly ? ' readonly' : '') + '></td>' +
          '<td><input type="number" class="tr3-deb" value="' + debVal + '"' + (debReadonly ? ' readonly' : '') + '></td>' +
          '<td><input type="number" class="tr3-qfin" value="' + qFinVal + '"></td>' +
          '<td><input type="number" class="tr3-fin" value="' + finVal + '" placeholder="Ex: 45230"></td>' +
          '<td><input type="number" class="tr3-ravit" value="' + ravitVal + '" placeholder="DT"></td>' +
          '<td><input type="number" class="tr3-solde" value="' + soldeVal + '" placeholder="DT"></td>' +
          '<td class="tr3-dist" style="text-align:center">' + distTxt + '</td>' +
          '<td><input type="text" class="tr3-obs" value="' + obsVal + '" placeholder="Optionnel" style="width:170px"></td>' +
          '</tr>';
      }).join('');
    ...
    document.getElementById('tr3-save-btn').addEventListener('click', function() {
      ...
      if (hasError) {
        errEl.textContent = "L'index de fin doit être supérieur ou égal à l'index de début. Vérifiez vos saisies (en rouge).";
        errEl.classList.add('show');
        return;
      }
