...
(function () {
      ...
      listEl.innerHTML = demandes.map(function(d) {
        ...
        return '<div data-notif-id="' + d.id + '" style="' + selectedStyle + 'border-radius:11px;padding:12px 14px;margin-bottom:10px;background:' + bg + '22;cursor:pointer;transition:box-shadow .2s,border .15s">' +
          '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">' +
            ...
            '<button id="tr2-notif-print-' + d.id + '" style="background:#1E3A5F;color:#fff;border:none;border-radius:7px;padding:4px 10px;font-size:11px;font-weight:700;cursor:pointer;white-space:nowrap;">🖨️</button>' +
          '</div>' +
  ...
  // ── Logo Tunisie Telecom SVG (inline, pas de dépendance externe) ──
  var TT_LOGO_SVG = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 40" width="120" height="40">' +
    '<rect width="120" height="40" rx="4" fill="#1E3A5F"/>' +
    '<text x="8" y="15" font-family="Arial,sans-serif" font-size="7" font-weight="700" fill="#EF6C00">TUNISIE</text>' +
    '<text x="8" y="26" font-family="Arial,sans-serif" font-size="7" font-weight="700" fill="#EF6C00">TELECOM</text>' +
    '<circle cx="100" cy="20" r="13" fill="none" stroke="#EF6C00" stroke-width="2.5"/>' +
    '<circle cx="100" cy="20" r="7" fill="#EF6C00"/>' +
    '<line x1="100" y1="7" x2="100" y2="33" stroke="#EF6C00" stroke-width="1.5"/>' +
    '<line x1="87" y1="20" x2="113" y2="20" stroke="#EF6C00" stroke-width="1.5"/>' +
  '</svg>';
  ...
  function printDemandeShared(d) {
    ...
    var statutBanner = (isOk || isKo)
      ? '<div style="background:' + statutBg + ';border:2.5px solid ' + statutColor + ';border-radius:10px;padding:14px 20px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between;">' +
          '<div>' +
            ...
            (d.validePar ? '<div style="font-size:12px;color:#374151;margin-top:3px;">Par : <strong>' + d.validePar + '</strong>' + (d.dateTraitement ? '  —  Le ' + formatDate(d.dateTraitement) : '') + '</div>' : '') +
    ...
    var iframeContent = '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>' +
      ...
      '.entete{display:flex;align-items:center;justify-content:space-between;padding:16px 20px;background:#1E3A5F;border-radius:10px 10px 0 0;margin-bottom:0;}' +
      ...
      '.entete-num{background:#EF6C00;color:#fff;padding:5px 16px;border-radius:20px;font-size:13px;font-weight:800;white-space:nowrap;}' +
      '.sous-entete{background:#EF6C00;color:#fff;text-align:center;padding:6px;font-size:10px;font-weight:700;letter-spacing:1px;margin-bottom:16px;border-radius:0 0 6px 6px;}' +
      ...
      '.nat-box{background:#FEF3E2;border-left:4px solid #EF6C00;padding:12px 16px;margin-bottom:14px;border-radius:0 8px 8px 0;}' +
      ...
      '.no-print{text-align:center;margin-bottom:16px;}' +
      '.pbtn{background:#1E3A5F;color:#fff;border:none;padding:10px 28px;border-radius:8px;font-size:14px;cursor:pointer;margin-right:8px;font-weight:700;}' +
      ...
      '<div class="pg">' +
        ...
        '</div>' +
        '<div class="sous-entete">PARC AUTO DRTE SFAX</div>' +
        ...
        '<div style="display:flex;gap:16px;margin-bottom:14px;">' +
          '<div style="flex:1;border:1px solid #e2e8f0;border-radius:8px;padding:12px 14px;min-height:60px;">' +
            '<div style="font-size:9px;text-transform:uppercase;color:#94a3b8;font-weight:700;margin-bottom:6px;">Signature du demandeur</div>' +
          '</div>' +
          '<div style="flex:1;border:1px solid ' + statutColor + ';border-radius:8px;padding:12px 14px;min-height:60px;background:' + statutBg + '22;">' +
            '<div style="font-size:9px;text-transform:uppercase;color:' + statutColor + ';font-weight:700;margin-bottom:6px;">Visa Chef de Parc</div>' +
  ...
  function initValidateurInterface() {
    ...
    function renderValidList(filter) {
      ...
      list.innerHTML = demandes.map(function(d) {
        ...
        return '<div class="tr2v-card" id="card-' + d.id + '">' +
          '<div class="tr2v-card-hd">' +
            ...
            '<button id="tr2v-print-' + d.id + '" style="background:#1E3A5F;color:#fff;border:none;border-radius:7px;padding:4px 10px;font-size:11px;font-weight:700;cursor:pointer;white-space:nowrap;">🖨️ Imprimer</button>' +
          '</div>' +
      ...
      demandes.forEach(function(d) {
        // Bouton impression (toutes les cartes)
