...
(function () {
  ...
  function buildCarburantSheet(workbook, sheetName, list) {
    ...
    list.forEach(function(e, idx) {
      ...
      // BLOC CORRECTIF — Distance / Quantité Consommée / Taux Consommation étaient écrites
      // comme formules Excel ({formula: ...}) sans valeur mise en cache. ExcelJS n'exécute
      // jamais les formules : le fichier ne contenait donc AUCUNE valeur tant qu'il n'avait
      // pas été ouvert puis ré-enregistré dans Excel. Résultat : toute réimportation dans
      // l'appli (ou lecture par un autre outil) trouvait ces colonnes vides / à 0.
      // On calcule maintenant la valeur réelle en JS et on l'écrit en cache via
      // {formula, result}, en gardant la formule pour que la cellule reste "vivante" si
      // le fichier est ensuite modifié à la main dans Excel.
      var distanceVal = (typeof e.indexFin === 'number' && typeof e.indexDebut === 'number')
        ? (e.indexFin - e.indexDebut) : '-';
      var qteConsommeeVal = (typeof e.ravitaillement === 'number' && typeof prixLitre === 'number' && prixLitre !== 0)
        ? (e.ravitaillement / prixLitre) : 0;
      var tauxVal = (typeof qteConsommeeVal === 'number' && typeof distanceVal === 'number' && distanceVal !== 0)
        ? (qteConsommeeVal / distanceVal) : '-';
      ...
      var row = ws.addRow([
        ...
        typeof distanceVal === 'number'
          ? { formula: 'IF(AND(ISNUMBER(K' + r + '),ISNUMBER(I' + r + ')),K' + r + '-I' + r + ',"-")', result: distanceVal }
          : distanceVal,
        prixLitre,
        { formula: 'IF(AND(ISNUMBER(L' + r + '),ISNUMBER(O' + r + '),O' + r + '<>0),L' + r + '/O' + r + ',0)', result: qteConsommeeVal },
        typeof tauxVal === 'number'
          ? { formula: 'IF(AND(ISNUMBER(P' + r + '),ISNUMBER(N' + r + '),N' + r + '<>0),P' + r + '/N' + r + ',"-")', result: tauxVal }
          : tauxVal,
  ...
  function initUserForm() {
    ...
    function notifValidaterAction(id, newStatut, comment, nom) {
      ...
      // ➕ SYNC FIX: Force re-fetch après validation pour s'assurer que le Gist est à jour
      setTimeout(function() {
        if (typeof tr2FetchAndInject === 'function') {
          tr2FetchAndInject(function() {
            // Re-render après re-fetch forcé pour afficher le nouveau statut
            if (typeof renderNotifScreen === 'function') {
              renderNotifScreen(nom);
            }
          }); // force refresh depuis le Gist avec callback
        }
      }, 2000);
      _notifSelected = null;
      if (window.parcAuto && typeof window.parcAuto.showToast === 'function') {
    ...
    document.getElementById('tr2-notif-refresh-btn').addEventListener('click', function() {
      ...
      if (workerUrl) {
        ...
        fetch(workerUrl + sep + '_nc=' + Date.now(), fetchOpts)
