...
(function () {
  ...
  // ── Bip d'alerte × 3 (AudioContext) ────────────────────────
  function bipAlert3(type) {
    // type: 'new' (nouvelle demande) | 'valid' (validation/rejet)
    try {
      var ctx = new (window.AudioContext || window.webkitAudioContext)();
      var count = 0;
      var freq  = type === 'valid' ? 880 : 660;
      var dur   = type === 'valid' ? 0.12 : 0.18;
      var gap   = type === 'valid' ? 0.22 : 0.32;
      function bip() {
        if (count >= 3) { setTimeout(function(){ ctx.close(); }, 500); return; }
  ...
  css.textContent = `
    ...
    /* ── Bouton flottant Validateur ── */
    #tr2v-open-btn {
      position:fixed; bottom:96px; right:16px; z-index:8999;
      background:linear-gradient(145deg,#1a237e,#283593);
      color:#fff; border:none; border-radius:18px;
      padding:0; width:64px; height:64px;
      font-size:11px; font-weight:700; cursor:pointer;
      box-shadow:0 6px 20px rgba(26,35,126,.45), 0 2px 6px rgba(0,0,0,.2);
      display:flex; flex-direction:column; align-items:center; justify-content:center; gap:2px;
      transition:transform .2s, box-shadow .2s; }
    #tr2v-open-btn:hover { transform:translateY(-3px) scale(1.05);
      box-shadow:0 10px 28px rgba(26,35,126,.55); }
    #tr2v-open-btn:active { transform:scale(.96); }
    #tr2v-open-btn .btn-icon { font-size:22px; line-height:1; }
    #tr2v-open-btn .btn-lbl { font-size:9px; font-weight:800; letter-spacing:.3px;
      text-transform:uppercase; opacity:.9; }
    #tr2v-open-btn .badge { position:absolute; top:-5px; right:-5px;
      background:#E53935; color:#fff; border-radius:12px;
      font-size:11px; padding:2px 6px; font-weight:800;
    ...
    /* ── Bouton flottant Chef Garage ── */
    #tr2cg-open-btn {
      position:fixed; bottom:176px; right:16px; z-index:8998;
      background:linear-gradient(145deg,#1b5e20,#2e7d32);
      color:#fff; border:none; border-radius:18px;
      padding:0; width:64px; height:64px;
      font-size:11px; font-weight:700; cursor:pointer;
      box-shadow:0 6px 20px rgba(27,94,32,.45), 0 2px 6px rgba(0,0,0,.2);
      display:flex; flex-direction:column; align-items:center; justify-content:center; gap:2px;
      transition:transform .2s, box-shadow .2s; }
    #tr2cg-open-btn:hover { transform:translateY(-3px) scale(1.05);
      box-shadow:0 10px 28px rgba(27,94,32,.55); }
    #tr2cg-open-btn:active { transform:scale(.96); }
    #tr2cg-open-btn .btn-icon { font-size:22px; line-height:1; }
    #tr2cg-open-btn .btn-lbl { font-size:9px; font-weight:800; letter-spacing:.3px;
      text-transform:uppercase; opacity:.9; }
    #tr2cg-open-btn .badge { position:absolute; top:-5px; right:-5px;
      background:#E53935; color:#fff; border-radius:12px;
      font-size:11px; padding:2px 6px; font-weight:800;
      box-shadow:0 2px 6px rgba(0,0,0,.3); border:2px solid #fff; }
  ...
  function initValidateurInterface() {
    ...
    openBtn.innerHTML = '<span class="btn-icon">🛡️</span><span class="btn-lbl">Validateur</span><span class="badge" id="tr2v-float-badge" style="display:none">0</span>';
  ...
  function initChefGarageInterface() {
    ...
    cgBtn.innerHTML = '<span class="btn-icon">🔧</span><span class="btn-lbl">Chef Parc</span><span class="badge" id="tr2cg-badge" style="display:none">0</span>';
    ...
    setInterval(function() { renderCGStats(); }, 10000);
