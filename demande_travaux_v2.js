  ...
  function initUserForm() {
    ...
    document.getElementById('tr2-notif-refresh-btn').addEventListener('click', function() {
      ...
      if (workerUrl) {
        ...
        // BLOC ADDITIF — ajout du header X-App-Key (manquant ici, présent dans tr2FetchAndInject)
        var refreshFetchOpts = { cache: 'no-store' };
        var refreshAppKey = window.APP_KEY || (pa && pa.APP_KEY) || null;
        if (refreshAppKey) refreshFetchOpts.headers = { 'X-App-Key': refreshAppKey };
        // FIN BLOC ADDITIF
        fetch(workerUrl + sep + '_nc=' + Date.now(), refreshFetchOpts)
          .then(function(r) {
            // BLOC ADDITIF — détecter les réponses non-ok (401/403/etc.) au lieu de les parser comme données
