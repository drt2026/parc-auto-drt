...
(function () {
  ...
  function initOrdreMissionModal() {
    ...
    // ══════════════════════════════════════════════════════════
    // Export Excel — reproduit la mise en forme du formulaire officiel
    // "إذن بمأمورية لاستعمال عربة مصلحة" (RTL, Tunisie Telecom)
    // ══════════════════════════════════════════════════════════
    ...
    function buildOrdreMissionSheet(workbook, om) {
      var ws = workbook.addWorksheet('Ordre de Mission', {
        views: [{ rightToLeft: true, showGridLines: false }],
        pageSetup: { orientation: 'portrait', paperSize: 9, fitToPage: true, fitToWidth: 1, fitToHeight: 1 }
      });
      ws.columns = [
        { width: 20 }, { width: 15 }, { width: 15 }, { width: 13 },
        { width: 13 }, { width: 20 }, { width: 13 }, { width: 13 },
        { width: 13 }, { width: 15 }
      ];

      var NAVY = 'FF1E3A5F', PURPLE = 'FF5B21B6', LIGHT = 'FFF5F3FF', BORDER = 'FFCBD5E1';
      function thin() {
        var s = { style: 'thin', color: { argb: BORDER } };
        return { top: s, bottom: s, left: s, right: s };
      }
      function mergeSet(range, text, opts) {
        opts = opts || {};
        ws.mergeCells(range);
        var cell = ws.getCell(range.split(':')[0]);
        cell.value = text;
        cell.font = Object.assign({ name: 'Calibri', size: opts.size || 11, bold: !!opts.bold, color: { argb: opts.color || 'FF1E293B' } }, opts.font || {});
        cell.alignment = Object.assign({ horizontal: 'center', vertical: 'middle', wrapText: true, readingOrder: 2 }, opts.alignment || {});
        if (opts.fill) cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: opts.fill } };
        return cell;
      }
      function labelCell(addr, text) {
        var c = ws.getCell(addr);
        c.value = text;
        c.font = { name: 'Calibri', size: 10.5, bold: true, color: { argb: 'FF374151' } };
        c.alignment = { horizontal: 'right', vertical: 'middle', wrapText: true, readingOrder: 2 };
        c.border = thin();
        c.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF8FAFC' } };
        return c;
      }
      function valueCell(range, text) {
        ws.mergeCells(range);
        var c = ws.getCell(range.split(':')[0]);
        c.value = text || '';
        c.font = { name: 'Calibri', size: 11, color: { argb: 'FF1E293B' } };
        c.alignment = { horizontal: 'right', vertical: 'middle', wrapText: true, readingOrder: 2 };
        c.border = thin();
        return c;
      }
      ...
      // ── En-tête / masthead ──
      mergeSet('A1:J1', 'الجمهورية التونسية  —  وزارة تكنولوجيات الاتصال  —  الشركة الوطنية للاتصالات', { size: 11, bold: true });
      ws.getRow(1).height = 20;
      mergeSet('A2:J2', 'إذن بمأمورية لاستعمال عربة مصلحة', { size: 16, bold: true, fill: LIGHT, color: PURPLE });
      ws.getRow(2).height = 30;
      mergeSet('A3:J3', 'هام جدا: لا تستعمل العربة في غير حاجة المصلحة ويتعين الاستظهار بهذا الإذن عند الطلب', { size: 9, color: 'FFC0392B' });
      ws.getRow(3).height = 16;
      mergeSet('A4:J4', 'رقم الوثيقة: ' + om.numero, { size: 10, bold: true, fill: NAVY, color: 'FFFFFFFF' });
      ws.getRow(4).height = 18;

      // ── Identité ──
      labelCell('A5', 'يرخص السيد');
      valueCell('B5:E5', om.nom);
      labelCell('F5', 'بطاقة تعريف وطنية عدد');
      valueCell('G5:J5', om.cin);

      labelCell('A6', 'الرتبة أو الصنف');
      valueCell('B6:E6', om.rang);
      labelCell('F6', 'الرتبة الوظيفية');
      valueCell('G6:J6', om.fonction);

      labelCell('A7', 'في استعمال عربة مصلحة رقم');
      valueCell('B7:E7', om.matricule + (om.marque ? ' — ' + om.marque : ''));
      labelCell('F7', 'الغاية من استعمال العربة');
      valueCell('G7:J7', om.motif);
      ws.getRow(7).height = 34;

      labelCell('A8', 'مكان الانطلاق');
      valueCell('B8:C8', om.lieuDepart);
      labelCell('D8', 'التاريخ');
      valueCell('E8:E8', fmtDateFr(om.dateDepart));
      labelCell('F8', 'الساعة');
      valueCell('G8:G8', om.heureDepart);

      labelCell('A9', 'المكان المقصود');
      valueCell('B9:C9', om.lieuDestination);
      labelCell('D9', 'التاريخ');
      valueCell('E9:E9', fmtDateFr(om.dateRetour));
      labelCell('F9', 'الساعة');
      valueCell('G9:G9', om.heureRetour);

      // ── Accompagnateurs ──
      mergeSet('A11:J11', 'المرافقون (السيد / ة)', { size: 11, bold: true, fill: LIGHT, color: PURPLE, alignment: { horizontal: 'right' } });
      ws.getRow(11).height = 18;
      for (var i = 0; i < 4; i++) {
        var r = 12 + i;
        var acc = (om.accompagnateurs || [])[i] || {};
        labelCell('A' + r, 'السيد / ة');
        valueCell('B' + r + ':E' + r, acc.nom || '');
        labelCell('F' + r, 'بطاقة تعريف وطنية عدد');
        valueCell('G' + r + ':J' + r, acc.cin || '');
      }

      // ── Charge / autorisation ──
      labelCell('A16', 'الحمولة');
      valueCell('B16:D16', om.charge || '—');
      if (om.horsHeures) {
        mergeSet('E16:J16', 'يرخص له استعمال السيارة خارج أوقات العمل الرسمية والآحاد والعطل.', { size: 9.5, bold: true, alignment: { horizontal: 'right' } });
      } else {
        ws.mergeCells('E16:J16');
      }

      // ── Signature / cachet ──
      labelCell('A18', 'بصفاقس في');
      valueCell('B18:C18', fmtDateFr(om.dateRedaction));

      mergeSet('H18:J18', 'رئيس الإدارة', { size: 11, bold: true, alignment: { horizontal: 'center' } });
      ws.getRow(18).height = 18;
      ws.mergeCells('H19:J23');
      var stampCell = ws.getCell('H19');
      stampCell.value = 'الإمضاء والختم';
      stampCell.font = { name: 'Calibri', size: 10, italic: true, color: { argb: 'FF64748B' } };
      stampCell.alignment = { horizontal: 'center', vertical: 'top', readingOrder: 2 };
      stampCell.border = thin();

      mergeSet('A25:J25', 'Document généré automatiquement — Parc Auto DRT Sfax, Tunisie Telecom', { size: 8, color: 'FF94A3B8', border: false, alignment: { horizontal: 'center' } });
    ...
    function downloadOrdreMission(om) {
      ensureExcelJSLib(function() {
        try {
          var workbook = new window.ExcelJS.Workbook();
          workbook.creator = 'Parc Auto DRT Sfax — Tunisie Telecom';
          buildOrdreMissionSheet(workbook, om);
          workbook.xlsx.writeBuffer().then(function(buffer) {
            var blob = new Blob([buffer], { type: 'application/octet-stream' });
            var url = URL.createObjectURL(blob);
            var a = document.createElement('a');
