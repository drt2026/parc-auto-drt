...
(function () {
  ...
  // ── Bip d'alerte 3 BIP × 3 FOIS (AudioContext) ────────────
  function bipAlert3(type) {
    // 3 séquences de 3 bips, séparées par une pause
    var freq    = type === 'valid' ? 880 : 660;
    var bipDur  = 0.12;   // durée d'un bip (secondes)
    var bipGap  = 0.18;   // silence entre bips dans une séquence
    var seqGap  = 600;    // silence entre séquences (ms)
    var seqCount = 0;
    function playSequence() {
      if (seqCount >= 3) return;
      seqCount++;
      try {
        ...
        for (var i = 0; i < 3; i++) {
          ...
          var start = t + i * (bipDur + bipGap);
          gain.gain.setValueAtTime(0, start);
          gain.gain.linearRampToValueAtTime(0.45, start + 0.01);
          gain.gain.linearRampToValueAtTime(0, start + bipDur);
          osc.start(start);
          osc.stop(start + bipDur + 0.02);
        }
        // Fermer le contexte après la fin de la séquence
        var seqLen = 3 * (bipDur + bipGap) * 1000 + 100;
        setTimeout(function() { try { ctx.close(); } catch(e) {} }, seqLen);
      } catch(e) { /* AudioContext non disponible */ }
      if (seqCount < 3) setTimeout(playSequence, seqGap);
    }
    playSequence();
  }
  ...
  css.textContent = `
    ...
    #tr2v-open-btn .btn-icon { display:flex; align-items:center; justify-content:center; }
    ...
    #tr2cg-open-btn .btn-icon { display:flex; align-items:center; justify-content:center; }
  ...
  function initValidateurInterface() {
    ...
    openBtn.innerHTML = '<span class="btn-icon"><svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M14 2L4 6v8c0 5.5 4.3 10.7 10 12 5.7-1.3 10-6.5 10-12V6L14 2z" fill="rgba(255,255,255,0.22)" stroke="#fff" stroke-width="1.8" stroke-linejoin="round"/><path d="M9.5 14l3 3 6-6" stroke="#fff" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg></span><span class="btn-lbl">Validateur</span><span class="badge" id="tr2v-float-badge" style="display:none">0</span>';
  ...
  function initChefGarageInterface() {
    ...
    cgBtn.innerHTML = '<span class="btn-icon"><svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="10" cy="10" r="4.5" stroke="#fff" stroke-width="1.9"/><line x1="13.2" y1="13.2" x2="24" y2="24" stroke="#fff" stroke-width="2.4" stroke-linecap="round"/><circle cx="21" cy="21" r="1.5" fill="#fff"/></svg></span><span class="btn-lbl">Chef Parc</span><span class="badge" id="tr2cg-badge" style="display:none">0</span>';
