...
(function () {
  ...
  css.textContent = `
    ...
    .tr2-notif-card { border-radius:11px; padding:12px 14px; margin-bottom:10px;
      cursor:pointer; transition:box-shadow .2s,border .15s,transform .1s; }
    .tr2-notif-card:hover { transform:translateY(-1px); }
    .tr2-notif-card.selected { outline:3px solid #C0392B; }
    .tr2-del-btn { width:100%; margin-top:10px; padding:9px; background:#C0392B;
    ...
    .tr2-del-btn:hover { opacity:.85; }
    .tr2-notif-actions { display:flex; gap:8px; padding:0 26px 16px; }
    .tr2-notif-actions .tr2-btn { margin-top:0; }
    .tr2-clear-all-btn { width:100%; padding:11px; border:none; border-radius:11px;
      font-size:14px; font-weight:700; cursor:pointer; transition:opacity .2s,transform .15s;
      background:#FEE2E2; color:#991b1b; display:flex; align-items:center;
      justify-content:center; gap:7px; }
    .tr2-clear-all-btn:hover { opacity:.85; transform:translateY(-1px); }
  ...
  function initUserForm() {
        ...
        <div class="tr2-bd">
          ...
          <div id="tr2-sc-notif" style="display:none">
            ...
            <div style="padding:0 26px 20px;display:flex;gap:8px;flex-direction:column">
              ...
              <button class="tr2-clear-all-btn" id="tr2-notif-clear-btn">🗑️ Effacer tout l'historique</button>
            </div>
          </div>
    ...
    function renderNotifScreen(nom) {
      ...
      listEl.innerHTML = demandes.map(function(d) {
        ...
        } else if (!isValidateur && !isSelected) {
          clickHint = '<div style="font-size:10px;color:#94a3b8;margin-top:5px;text-align:right">👆 Cliquer pour sélectionner</div>';
        }

        // Bouton supprimer la demande (visible uniquement si sélectionnée)
        var deleteBtn = '';
        if (isSelected) {
          deleteBtn = '<button class="tr2-del-btn" id="tr2-notif-del-' + d.id + '" data-delid="' + d.id + '">🗑️ Supprimer cette demande</button>';
        }

        var selectedStyle = isSelected
          ? 'border:2px solid #C0392B;box-shadow:0 4px 18px #C0392B22;'
          : 'border:1.5px solid ' + color + '33;';

        return '<div data-notif-id="' + d.id + '" class="tr2-notif-card' + (isSelected ? ' selected' : '') + '" style="' + selectedStyle + 'background:' + bg + '22;">' +
          ...
          extra + clickHint + validPanel + deleteBtn +
        '</div>';
      }).join('');
      ...
      demandes.forEach(function(d) {
        ...
        card.addEventListener('click', function(e) {
          // Ne pas déclencher si clic sur un bouton ou textarea dans la carte
          if (e.target.tagName === 'BUTTON' || e.target.tagName === 'TEXTAREA') return;
          _notifSelected = (_notifSelected === d.id) ? null : d.id;
          renderNotifScreen(nom);
        });

        // Bouton supprimer une demande individuelle
        (function(did, dnum) {
          var delBtn = document.getElementById('tr2-notif-del-' + did);
          if (delBtn) {
            delBtn.addEventListener('click', function(e) {
              e.stopPropagation();
              if (!confirm('Supprimer la demande ' + (dnum||'') + ' ?\nCette action est irréversible.')) return;
              var data2 = getData();
              if (!data2) return;
              data2.demandesTravaux = (data2.demandesTravaux || []).filter(function(x){ return x.id !== did; });
              saveData();
              _notifSelected = null;
              renderNotifScreen(nom);
              if (window.parcAuto && typeof window.parcAuto.showToast === 'function') {
                window.parcAuto.showToast('🗑️ Demande ' + (dnum||'') + ' supprimée', 'info');
              }
            });
          }
        })(d.id, d.numero);
