...
(function () {
  ...
  // Charge la librairie SheetJS (xlsx) à la demande, une seule fois
  var _tr3XlsxLoading = null;
  function ensureXLSXLib(cb, errCb) {
    if (window.XLSX) { cb(); return; }
    if (_tr3XlsxLoading) { _tr3XlsxLoading.push(cb); return; }
    _tr3XlsxLoading = [cb];
    var s = document.createElement('script');
    s.src = 'https://cdnjs.cloudflare.com/ajax/libs/xlsx/0.18.5/xlsx.full.min.js';
    s.onload = function() {
      var cbs = _tr3XlsxLoading || [];
      _tr3XlsxLoading = null;
  ...
  // Construit les lignes au format "Consommation Carburants" (mêmes colonnes que le fichier de référence)
  function buildCarburantExportAOA(list) {
    var header = ['  IMMAT.', '  MARQUE', 'CV', 'M. E. C', ' ENERGIE', 'PLACES', 'Division',
      'Quantité de carburant restante dans le réservoir Début du mois', 'Index Début du mois',
      'Quantité de carburant restante dans le réservoir Fin du mois', 'Index Fin du mois',
      'Total ravitaillement (en DT)', 'Solde  Restant (en DT) ', 'Distance parcouru', 'Prix du Litre',
      'Quantité consommée en litre', 'Taux consommation', 'Seuil', 'Observations'];
    var rows = list.map(function(e) {
      var v = getVehicleFull(e.matricule) || {};
      var distance = (e.indexFin != null && e.indexDebut != null) ? (e.indexFin - e.indexDebut) : '';
      var prixLitre = getPrixLitre(v.energie, v.prixLitre);
      var ravit = (e.ravitaillement != null) ? e.ravitaillement : '';
      var qteConsommee = (ravit !== '' && prixLitre) ? (ravit / prixLitre) : '';
      var taux = '';
      if (qteConsommee !== '') {
        taux = distance ? (qteConsommee / distance) : 'Erreur';
      }
      var mec = v.mec;
      ...
      return [
        ...
        ravit, e.soldeCarte != null ? e.soldeCarte : '',
        distance, prixLitre || '', qteConsommee, taux, v.seuil || '', e.observations || ''
      ];
    });
    return [header].concat(rows);
  }

  // Génère et télécharge le classeur Excel (un onglet par mois saisi)
    ...
    ensureXLSXLib(function() {
      try {
        ...
        Object.keys(byMois).sort().forEach(function(mois) {
          var aoa = buildCarburantExportAOA(byMois[mois]);
          var ws = XLSX.utils.aoa_to_sheet(aoa);
          ws['!cols'] = aoa[0].map(function() { return { wch: 16 }; });
          XLSX.utils.book_append_sheet(wb, ws, moisToSheetName(mois));
        });
  ...
  function initSuiviCarburantModal() {
    ...
    css.textContent = `
      ...
      #tr3-modal { background:#fff; border-radius:20px; width:min(620px,96vw);
        max-height:93vh; overflow-y:auto;
        box-shadow:0 24px 64px rgba(0,0,0,0.22);
        animation:tr2Up .28s cubic-bezier(.22,1,.36,1); }
      #tr3-table-wrap table { width:100%; border-collapse:collapse; font-size:12.5px; margin:8px 0; }
      #tr3-table-wrap thead tr { background:#1F3864; color:#fff; }
      #tr3-table-wrap th { padding:7px 6px; text-align:left; font-weight:600; }
      #tr3-table-wrap td { padding:6px; border-bottom:1px solid #e2e8f0; }
      #tr3-table-wrap td input[type=number] { width:78px; padding:5px 7px; border:1.5px solid #e2e8f0;
        border-radius:7px; font-size:12.5px; font-family:inherit; }
      #tr3-table-wrap td input[type=text] { padding:5px 7px; border:1.5px solid #e2e8f0;
        border-radius:7px; font-size:12.5px; font-family:inherit; }
    ...
    function renderTable(nom, mois) {
      ...
      var rowsHtml = vehicles.map(function(mat) {
        ...
        return '<tr data-mat="' + mat + '">' +
          ...
          '<td><input type="text" class="tr3-obs" value="' + obsVal + '" placeholder="Optionnel" style="width:110px"></td>' +
          '</tr>';
      }).join('');
      wrap.innerHTML = '<div style="overflow-x:auto"><table style="min-width:920px"><thead><tr>' +
