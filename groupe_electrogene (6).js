...
(function () {
  ...
  function injectGEStyles() {
    ...
    css.textContent = `
      ...
      #ge-modal { background:#fff; border-radius:16px; width:min(1700px,98vw); max-height:95vh; overflow:hidden;
      ...
      .ge-table th, .ge-table td { border:1px solid #e2e8f0; padding:8px 10px; text-align:center; white-space:nowrap; }
      ...
      .ge-table input { width:78px; padding:5px 6px; border:1px solid #cbd5e1; border-radius:5px; font-size:13.5px;
  ...
  // BLOC ADDITIF — accès en lecture seule pour les modules de rapport
  // (Bilan Global Carburant, Rapport 1-clic). N'altère aucune fonction
  // existante : simple exposition de ce qui était déjà interne au module.
  window.getGEData = function () { return geList(); };
  window.__GE_SITES = GE_SITES;
  window.__GE_MOIS = GE_MOIS;
  window.__geCompute = geCompute;
