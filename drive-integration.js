...
class DriveManager {
  ...
  /** Obtenir les fichiers d'un véhicule depuis le cache (max 2 photos) */
  getVehicleFiles(matricule, maxPhotos = 2) {
    const result = {};
    const safeMat = matricule.replace(/[^a-zA-Z0-9-]/g, '_');
    // Extraire aussi le numéro sans préfixe (ex: 17-356835 → 356835)
    const matParts = matricule.split('-');
    const shortMat = matParts.length > 1 ? matParts[matParts.length - 1] : matricule;
    const safeShortMat = shortMat.replace(/[^a-zA-Z0-9]/g, '_');

    for (const [key, files] of Object.entries(this.fileCache)) {
      let filtered = (files || []).filter(f => {
        if (!f.name) return false;
        const safeName = f.name.replace(/[^a-zA-Z0-9-]/g, '_');
        // Rechercher avec le matricule complet OU le numéro court
        return safeName.includes(safeMat) || safeName.includes(safeShortMat);
      });
      // Limiter à 2 photos maximum pour la galerie véhicule
      if (key === 'PHOTOS' && filtered.length > maxPhotos) {
