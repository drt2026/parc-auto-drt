...
(function () {
  ...
  function initUserForm() {
    ...
    // BLOC ADDITIF — Bouton Actualiser (rechargement Gist sans quitter l'app)
    document.getElementById('tr2-notif-refresh-btn').addEventListener('click', function() {
      ...
      var pa = window.parcAuto;
      // Si parcAuto expose loadData (rechargement depuis Gist), on l'utilise
      if (pa && typeof pa.loadData === 'function') {
        pa.loadData().then(function() {
          renderNotifScreen(sess.nom);
          btn.disabled = false;
          btn.textContent = '🔄 Actualiser';
          if (typeof pa.showToast === 'function') pa.showToast('✅ Données actualisées', 'success');
        }).catch(function() {
          // Fallback : afficher quand même les données locales
          renderNotifScreen(sess.nom);
          btn.disabled = false;
          btn.textContent = '🔄 Actualiser';
          if (typeof pa.showToast === 'function') pa.showToast('⚠️ Rechargement partiel (données locales)', 'warning');
        });
      } else if (pa && typeof pa.syncData === 'function') {
        // Certaines versions exposent syncData
        pa.syncData().then(function() {
          renderNotifScreen(sess.nom);
          btn.disabled = false;
          btn.textContent = '🔄 Actualiser';
          if (typeof pa.showToast === 'function') pa.showToast('✅ Données actualisées', 'success');
        ...
        // Fallback ultime : re-render depuis les données en mémoire
        setTimeout(function() {
          renderNotifScreen(sess.nom);
          btn.disabled = false;
          btn.textContent = '🔄 Actualiser';
          if (pa && typeof pa.showToast === 'function') pa.showToast('✅ Affichage actualisé', 'success');
        }, 400);
      }
    });
