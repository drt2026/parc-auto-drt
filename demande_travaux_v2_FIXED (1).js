    ...
    setInterval(function() {
      ...
      tr2FetchAndInject(function() {
        ...
        mesDemandes.forEach(function(d) {
          ...
          if (prev.statut && prev.statut !== d.statut) {
            changed = true;
            bipAlert3(d.statut === 'VALIDÉE' ? 'valid' : 'new');
          }
          // Nouvelle réponse chef garage
          if (prev.cgReponse !== undefined && prev.cgReponse !== d.cgReponse && d.cgReponse) {
            changed = true;
            bipAlert3('new');
          }
        ...
        if (changed) {
          ...
          if (notifScreen && notifScreen.style.display !== 'none') renderNotifScreen(sess.nom);
        }
      });
    }, 15000);
  ...
  function printDemandeShared(d) {
    ...
    var iframeContent = '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>' +
      ...
      '<div class="pg">' +
        ...
        (d.commentaire && !isKo ? '<div style="background:#EFF6FF;border-left:4px solid #3b82f6;border-radius:0 8px 8px 0;padding:10px 14px;margin-bottom:14px;"><div style="font-size:9px;text-transform:uppercase;color:#1d4ed8;font-weight:700;margin-bottom:4px;">&#128172; Observation du validateur</div><div style="font-size:13px;color:#1e293b;">' + d.commentaire + '</div></div>' : '') +
        ...
        '<div style="display:flex;gap:12px;margin-bottom:14px;">' +
          ...
          '<div style="flex:1;border:1.5px solid #e2e8f0;border-radius:8px;padding:10px 14px;min-height:70px;">' +
            ...
            (d.signatureDataUrl ? '<img src="' + d.signatureDataUrl + '" style="max-width:100%;max-height:60px;display:block;margin-top:4px;" alt="Signature"/>' : '<div style="font-size:11px;color:#94a3b8;font-style:italic;margin-top:4px;">Non fournie</div>') +
          '</div>' +
        '</div>' +
