...
(function () {
  ...
  function initValidateurInterface() {
    ...
    setInterval(function() {
      ...
      window._tr2FetchAndInject(function() {
        ...
        // BLOC ADDITIF — le bip se répète à chaque cycle tant qu'il reste des demandes EN ATTENTE
        // (il ne s'arrête plus après une seule séquence ; il s'arrête uniquement quand pendingCount === 0,
        // c'est-à-dire quand toutes les demandes ont été traitées/validées/rejetées)
        if (pendingCount > 0) {
          bipAlert3('new');
  ...
  function initChefGarageInterface() {
    ...
    setInterval(function() {
      ...
      window._tr2FetchAndInject(function() {
        var data = getData();
        // BLOC ADDITIF — le bip se base désormais sur "non traitée" (cgTraite) et non plus
        // sur "non lue" (cgLu), pour que l'alerte continue tant que le travail n'est pas réellement traité,
        // et se répète à chaque cycle de polling au lieu de sonner une seule fois.
        var newCount = (data && data.demandesTravaux||[]).filter(function(d){ return d.statut==='VALIDÉE' && !d.cgTraite; }).length;
        if (newCount > 0) {
          bipAlert3('new');
