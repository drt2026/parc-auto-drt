...
(function () {
  ...
  function initOrdreMissionModal() {
        ...
        <div class="tr2-bd">
          ...
          <div id="tr4-sc-form">
            ...
            <div class="tr2-row">
              <div class="tr2-f">
                ...
                <input type="text" id="tr4-nom" readonly style="background:#f1f5f9;color:#374151;cursor:default;">
              </div>
              <div class="tr2-f">
                <label>N° Carte d'Identité Nationale <em>*</em></label>
                <input type="text" id="tr4-cin" placeholder="Ex: 04123456">
              </div>
            </div>
            <div class="tr2-row">
              <div class="tr2-f">
                <label>Rang ou Catégorie <em>*</em></label>
                <input type="text" id="tr4-rang" placeholder="Ex: Technicien Principal">
              </div>
              <div class="tr2-f">
                <label>Rang fonctionnel / Fonction <em>*</em></label>
            ...
            <div class="tr2-row">
              <div class="tr2-f">
                <label>Date de départ <em>*</em></label>
                <input type="date" id="tr4-date-depart">
              </div>
              <div class="tr2-f">
                <label>Heure de départ <em>*</em></label>
                <input type="time" id="tr4-heure-depart">
              </div>
            </div>
            <div class="tr2-row">
              <div class="tr2-f">
                <label>Date de retour <em>*</em></label>
                <input type="date" id="tr4-date-retour">
              </div>
              <div class="tr2-f">
                <label>Heure de retour <em>*</em></label>
                <input type="time" id="tr4-heure-retour">
              </div>
            </div>
            <div class="tr2-f">
            ...
            <button class="tr2-btn tr2-btn-sec" id="tr4-hist-btn" style="margin-top:6px;background:#F3E8FF;color:#6b21a8;">📁 Mes ordres de mission</button>
          ...
          <div id="tr4-sc-ok" style="display:none">
            <div class="tr2-ok">
              ...
              <p>Téléchargez le document ci-dessous puis faites-le viser par le chef de parc.</p>
          ...
          <div id="tr4-sc-hist" style="display:none">
            ...
            <button class="tr2-btn tr2-btn-primary" id="tr4-hist-new-btn" style="margin-top:10px">➕ Nouvel ordre de mission</button>
    ...
    css.textContent = `
      ...
      #tr4-hist-list .tr4-card { background:#fff; border:1.5px solid #e2e8f0; border-radius:12px;
        margin-bottom:12px; overflow:hidden; }
      #tr4-hist-list .tr4-card-hd { display:flex; align-items:center; justify-content:space-between;
        padding:11px 15px; background:#F5F3FF; border-bottom:1px solid #ede9fe; gap:8px; flex-wrap:wrap; }
      #tr4-hist-list .tr4-card-num { font-weight:800; color:#5b21b6; font-size:13.5px; }
      #tr4-hist-list .tr4-card-date { font-size:11.5px; color:#64748b; }
      #tr4-hist-list .tr4-card-bd { padding:12px 15px; font-size:13px; color:#1e293b; }
      #tr4-hist-list .tr4-card-row { display:flex; justify-content:space-between; gap:10px; padding:3px 0; }
      #tr4-hist-list .tr4-card-row .l { color:#64748b; font-weight:600; }
      #tr4-hist-list .tr4-card-actions { display:flex; gap:8px; padding:0 15px 13px; }
      #tr4-hist-list .tr4-card-actions button { flex:1; padding:8px; border:none; border-radius:8px;
        font-size:12.5px; font-weight:700; cursor:pointer; }
      #tr4-hist-list .tr4-dl-btn { background:#EDE9FE; color:#5b21b6; }
      #tr4-hist-list .tr4-dl-btn:hover { background:#DDD6FE; }
    ...
    function trScreen4(n) {
      document.getElementById('tr4-sc-form').style.display = n === 'form' ? '' : 'none';
      document.getElementById('tr4-sc-ok').style.display   = n === 'ok'   ? '' : 'none';
      document.getElementById('tr4-sc-hist').style.display = n === 'hist' ? '' : 'none';
    }
    ...
    // ── Restreindre le champ matricule aux véhicules autorisés (réutilise le style de tr2-mat) ──
    ...
    document.getElementById('tr4-mat').addEventListener('change', function() {
      ...
      if (marqEl) marqEl.value = this.value ? (getMarqueFromMat(this.value) || '') : '';
    });
    ...
    // ── Pré-remplir CIN / Rang / Fonction depuis le dernier ordre de mission de cet utilisateur ──
    function prefillFromLastOM(nom) {
      var data = getData();
      if (!data) return;
      ...
      if (cinEl && !cinEl.value && last.cin) cinEl.value = last.cin;
      if (rangEl && !rangEl.value && last.rang) rangEl.value = last.rang;
      if (fctEl && !fctEl.value && last.fonction) fctEl.value = last.fonction;
    }

    function resetForm(nom) {
      ['tr4-motif','tr4-depart','tr4-destination','tr4-date-depart','tr4-heure-depart',
       'tr4-date-retour','tr4-heure-retour','tr4-charge'].forEach(function(id) {
    ...
    function renderOMHistory(nom) {
      ...
      var mine = data ? (data.ordresDeMission || []).filter(function(o) { return o.nom === nom; }) : [];
      mine.sort(function(a, b) { return (b.dateCreation || '').localeCompare(a.dateCreation || ''); });
      if (!mine.length) {
      ...
      listEl.innerHTML = mine.map(function(o) {
        var dateAff = o.dateDepart ? o.dateDepart.split('-').reverse().join('/') : '—';
        return '' +
          '<div class="tr4-card">' +
            '<div class="tr4-card-hd"><span class="tr4-card-num">' + o.numero + '</span><span class="tr4-card-date">' + dateAff + '</span></div>' +
            '<div class="tr4-card-bd">' +
              '<div class="tr4-card-row"><span class="l">Véhicule</span><span>' + (o.matricule || '—') + (o.marque ? ' — ' + o.marque : '') + '</span></div>' +
              '<div class="tr4-card-row"><span class="l">Trajet</span><span>' + (o.lieuDepart||'—') + ' → ' + (o.lieuDestination||'—') + '</span></div>' +
    ...
    document.getElementById('tr4-hist-btn').addEventListener('click', function() {
      var sess = getSess();
      if (sess && sess.nom) { renderOMHistory(sess.nom); trScreen4('hist'); }
    });
    document.getElementById('tr4-hist-new-btn').addEventListener('click', function() {
      var sess = getSess();
    ...
    document.getElementById('tr4-submit-btn').addEventListener('click', function() {
      ...
      var ids = ['tr4-cin','tr4-rang','tr4-fonction','tr4-mat','tr4-motif','tr4-depart',
                 'tr4-destination','tr4-date-depart','tr4-heure-depart','tr4-date-retour','tr4-heure-retour'];
      var vals = {};
      var ok = true;
      ids.forEach(function(id) {
        var e = document.getElementById(id);
      ...
      // Sécurité : le véhicule doit être autorisé pour cet utilisateur
      var allowedVehicles = getVehiclesForUser(sess.nom);
      if (allowedVehicles && allowedVehicles.length > 0 && allowedVehicles.indexOf(vals['tr4-mat']) === -1) {
        errEl.textContent = '⛔ Véhicule non autorisé. Vous ne pouvez établir un ordre de mission que pour vos véhicules assignés.';
        errEl.classList.add('show');
        return;
      }

      // Cohérence des dates
      var dtDepart = vals['tr4-date-depart'] + 'T' + vals['tr4-heure-depart'];
      var dtRetour = vals['tr4-date-retour'] + 'T' + vals['tr4-heure-retour'];
      if (dtRetour < dtDepart) {
        errEl.textContent = '❌ La date/heure de retour doit être postérieure à la date/heure de départ.';
        errEl.classList.add('show');
        return;
      }
      ...
      var om = {
        ...
        dateDepart: vals['tr4-date-depart'],
        heureDepart: vals['tr4-heure-depart'],
        dateRetour: vals['tr4-date-retour'],
        heureRetour: vals['tr4-heure-retour'],
        ...
        dateRedaction: new Date().toISOString().slice(0, 10),
        dateCreation: new Date().toISOString()
      };
