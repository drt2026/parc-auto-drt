...
class ParcAutoApp {
  ...
  startAutoSync() {
    ...
    this._autoSyncInterval = setInterval(async () => {
      ...
      if (this._lastLocalSave && (Date.now() - this._lastLocalSave < 60000)) {
        console.log('AutoSync: modification locale récente, lecture cloud ignorée');
        return;
      }
  ...
  renderUserVehicleDetail() {
    ...
    // 🔧 FIX CRITIQUE: FORCER le rechargement depuis localStorage pour avoir
    // les données les plus récentes (évite l'écrasement par sync cloud)
    try {
      const saved = localStorage.getItem('parcAutoData_v3');
      if (saved) {
...
async function saveUserKm(matricule) {
  ...
  try {
    ...
    // Sauvegarder UNIQUEMENT en localStorage
    localStorage.setItem('parcAutoData_v3', JSON.stringify(data));
    parcAuto.data = data;
    ...
    // Sync cloud en arrière-plan (délai 3s)
    setTimeout(() => {
    ...
    console.error('Erreur modification KM:', e);
  }
}
// ============================================
...
async function saveUserVisite(matricule) {
  ...
  try {
    ...
    console.error('Erreur modification visite:', e);
  }
}
