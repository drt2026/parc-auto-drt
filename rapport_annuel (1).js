...
(function () {
  ...
  function buildAnnualDataset(annee) {
    ...
    /* ---- Entretien périodique : Vidange / Chaîne / Accident / Lavage / Joint Culasse / Pneumatique ---- */
    var entretien = { nbVidange: 0, nbChaine: 0, nbAccident: 0, nbLavage: 0, nbJointCulasse: 0, nbPneumatique: 0 };
    var entretienParDivision = {};
    repEntries.forEach(function (e) {
      entretien.nbVidange += Number(e.nbVidange) || 0;
      entretien.nbChaine += Number(e.nbChaine) || 0;
      entretien.nbAccident += Number(e.nbAccident) || 0;
      entretien.nbLavage += Number(e.nbLavage) || 0;
      entretien.nbJointCulasse += Number(e.nbJointCulasse) || 0;
      entretien.nbPneumatique += Number(e.nbPneumatique) || 0;

      var raw = (e.division || 'Non renseigné').trim();
      var k = normKey(raw);
      if (!entretienParDivision[k]) entretienParDivision[k] = { label: cap(raw), nbVidange: 0, nbChaine: 0, nbAccident: 0, nbLavage: 0, nbJointCulasse: 0, nbPneumatique: 0 };
      entretienParDivision[k].nbVidange += Number(e.nbVidange) || 0;
      entretienParDivision[k].nbChaine += Number(e.nbChaine) || 0;
      entretienParDivision[k].nbAccident += Number(e.nbAccident) || 0;
      entretienParDivision[k].nbLavage += Number(e.nbLavage) || 0;
      entretienParDivision[k].nbJointCulasse += Number(e.nbJointCulasse) || 0;
      entretienParDivision[k].nbPneumatique += Number(e.nbPneumatique) || 0;
    });
    var entretienParDivisionList = Object.values(entretienParDivision).sort(function (a, b) {
      var totA = a.nbVidange + a.nbChaine + a.nbAccident + a.nbLavage + a.nbJointCulasse + a.nbPneumatique;
      var totB = b.nbVidange + b.nbChaine + b.nbAccident + b.nbLavage + b.nbJointCulasse + b.nbPneumatique;
      return totB - totA;
    });
    ...
    /* ---- Dépense par véhicule (entretien + carburant + CSC), liste complète ---- */
    var byVehCombined = {};
    repEntries.forEach(function (e) {
      var m = e.matricule;
      if (!byVehCombined[m]) byVehCombined[m] = { matricule: m, chauffeur: e.chauffeur, division: e.division || '—', marque: e.marque || '—', entretien: 0, carburant: 0, csc: 0 };
      byVehCombined[m].entretien += Number(e.montant) || 0;
      if (e.division) byVehCombined[m].division = e.division;
      if (e.marque) byVehCombined[m].marque = e.marque;
    });
    Object.values(byFuelVehicle).forEach(function (e) {
      var m = e.matricule;
      if (!byVehCombined[m]) byVehCombined[m] = { matricule: m, chauffeur: e.chauffeur, division: '—', marque: e.modele || '—', entretien: 0, carburant: 0, csc: 0 };
      byVehCombined[m].carburant += e.montant;
      if (!byVehCombined[m].chauffeur || byVehCombined[m].chauffeur === '—') byVehCombined[m].chauffeur = e.chauffeur;
    });
    cscVehicles.forEach(function (v) {
      var montant = (v.montants && v.montants[annee]) ? Number(v.montants[annee]) : 0;
      if (!montant) return;
      var m = v.matricule;
      if (!byVehCombined[m]) byVehCombined[m] = { matricule: m, chauffeur: '—', division: v.subdivision || '—', marque: v.modele || '—', entretien: 0, carburant: 0, csc: 0 };
      byVehCombined[m].csc += montant;
      if (byVehCombined[m].division === '—' && v.subdivision) byVehCombined[m].division = v.subdivision;
    });
    var depenseParVehiculeList = Object.values(byVehCombined).map(function (v) {
      v.total = v.entretien + v.carburant + v.csc; return v;
    }).sort(function (a, b) { return b.total - a.total; });
    var top10Combined = depenseParVehiculeList.slice(0, 10);
    ...
    return {
      ...
      entretien: entretien, entretienParDivisionList: entretienParDivisionList,
      ...
      depenseParVehiculeList: depenseParVehiculeList,
  ...
  function buildRapportAnnuelPptx(PptxGenJSCtor, AN, meta) {
    ...
    /* ============ SLIDE 2 — VUE D'ENSEMBLE (KPIs) ============ */
    {
      ...
      var kpis = [
        ...
        { label: 'Vidanges / Accidents', val: fmt(AN.entretien.nbVidange) + ' / ' + fmt(AN.entretien.nbAccident), color: PPT.red },
    ...
    /* ============ SLIDE 7 — ENTRETIEN PÉRIODIQUE ============ */
    {
      var s = pres.addSlide();
      s.background = { color: PPT.midnight };
      s.addText('Entretien périodique', { x: 0.6, y: 0.4, w: 10, h: 0.6, fontSize: 28, bold: true, color: PPT.paper, fontFace: 'Cambria' });
      s.addText('Interventions comptabilisées — exercice ' + AN.annee, { x: 0.6, y: 0.95, w: 9, h: 0.4, fontSize: 13, color: '94A3B8', fontFace: 'Calibri' });

      var etKpis = [
        { label: 'Vidanges', val: AN.entretien.nbVidange, icon: '🛢️', color: PPT.navy },
        { label: 'Kits chaîne', val: AN.entretien.nbChaine, icon: '⚙️', color: PPT.orange },
        { label: 'Accidents', val: AN.entretien.nbAccident, icon: '⚠️', color: PPT.red },
        { label: 'Lavages', val: AN.entretien.nbLavage, icon: '🚿', color: PPT.teal },
        { label: 'Joints culasse', val: AN.entretien.nbJointCulasse, icon: '🔩', color: PPT.purple },
        { label: 'Pneumatiques', val: AN.entretien.nbPneumatique, icon: '🛞', color: PPT.green }
      ];
      var cw2 = 3.9, ch2 = 1.75, gx2 = 0.25, x02 = 0.6, y02 = 1.6;
      etKpis.forEach(function (k, i) {
        var col = i % 3, row = Math.floor(i / 3);
        var x = x02 + col * (cw2 + gx2), y = y02 + row * (ch2 + 0.25);
        s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: x, y: y, w: cw2, h: ch2, rectRadius: 0.08, fill: { color: '16213E' }, line: { type: 'none' } });
        s.addText(k.icon, { x: x + 0.25, y: y + 0.2, w: 1, h: 0.8, fontSize: 30, align: 'left' });
        s.addText(String(k.val), { x: x + 0.25, y: y + 0.85, w: cw2 - 0.5, h: 0.6, fontSize: 26, bold: true, color: PPT.paper, fontFace: 'Cambria' });
        s.addText(k.label, { x: x + 0.25, y: y + 1.4, w: cw2 - 0.5, h: 0.35, fontSize: 12, color: '94A3B8', fontFace: 'Calibri' });
      });

      if (AN.entretienParDivisionList.length) {
        var rows = [[
          { text: 'Division', options: { bold: true, color: PPT.paper, fill: { color: PPT.navy }, fontSize: 9 } },
          { text: 'Vidanges', options: { bold: true, color: PPT.paper, fill: { color: PPT.navy }, fontSize: 9, align: 'center' } },
          { text: 'Chaîne', options: { bold: true, color: PPT.paper, fill: { color: PPT.navy }, fontSize: 9, align: 'center' } },
          { text: 'Accidents', options: { bold: true, color: PPT.paper, fill: { color: PPT.navy }, fontSize: 9, align: 'center' } },
          { text: 'Lavages', options: { bold: true, color: PPT.paper, fill: { color: PPT.navy }, fontSize: 9, align: 'center' } },
          { text: 'Joint cul.', options: { bold: true, color: PPT.paper, fill: { color: PPT.navy }, fontSize: 9, align: 'center' } },
          { text: 'Pneus', options: { bold: true, color: PPT.paper, fill: { color: PPT.navy }, fontSize: 9, align: 'center' } }
        ]];
        AN.entretienParDivisionList.slice(0, 8).forEach(function (d, i) {
          var fill = i % 2 === 0 ? '1E293B' : '16213E';
          rows.push([
            { text: d.label, options: { fill: { color: fill }, fontSize: 8.5, color: PPT.paper } },
            { text: String(d.nbVidange), options: { fill: { color: fill }, fontSize: 8.5, color: PPT.mist, align: 'center' } },
            { text: String(d.nbChaine), options: { fill: { color: fill }, fontSize: 8.5, color: PPT.mist, align: 'center' } },
            { text: String(d.nbAccident), options: { fill: { color: fill }, fontSize: 8.5, color: PPT.mist, align: 'center' } },
            { text: String(d.nbLavage), options: { fill: { color: fill }, fontSize: 8.5, color: PPT.mist, align: 'center' } },
            { text: String(d.nbJointCulasse), options: { fill: { color: fill }, fontSize: 8.5, color: PPT.mist, align: 'center' } },
            { text: String(d.nbPneumatique), options: { fill: { color: fill }, fontSize: 8.5, color: PPT.mist, align: 'center' } }
          ]);
        });
        s.addTable(rows, { x: 0.6, y: 4.95, w: 12.1, colW: [3.6, 1.4, 1.4, 1.5, 1.4, 1.4, 1.4], border: { pt: 0.5, color: '334155' }, valign: 'middle', rowH: 0.28 });
      }
    }
    ...
    /* ============ SLIDE 11 — TOP 10 VÉHICULES LES PLUS COÛTEUX ============ */
    {
      var s = pres.addSlide();
      s.background = { color: PPT.bg };
      s.addText('Top 10 véhicules les plus coûteux', { x: 0.6, y: 0.4, w: 11, h: 0.6, fontSize: 28, bold: true, color: PPT.ink, fontFace: 'Cambria' });
      s.addText('Entretien + carburant + CSC cumulés — ' + AN.annee, { x: 0.6, y: 0.95, w: 9, h: 0.4, fontSize: 13, color: PPT.slate, fontFace: 'Calibri' });

      if (AN.top10Combined.length) {
        var rows = [[
          { text: 'Matricule', options: { bold: true, color: PPT.paper, fill: { color: PPT.navy }, fontSize: 9.5 } },
          { text: 'Chauffeur', options: { bold: true, color: PPT.paper, fill: { color: PPT.navy }, fontSize: 9.5 } },
          { text: 'Entretien (DT)', options: { bold: true, color: PPT.paper, fill: { color: PPT.navy }, fontSize: 9.5, align: 'right' } },
          { text: 'Carburant (DT)', options: { bold: true, color: PPT.paper, fill: { color: PPT.navy }, fontSize: 9.5, align: 'right' } },
          { text: 'CSC (DT)', options: { bold: true, color: PPT.paper, fill: { color: PPT.navy }, fontSize: 9.5, align: 'right' } },
          { text: 'Total (DT)', options: { bold: true, color: PPT.paper, fill: { color: PPT.orange }, fontSize: 9.5, align: 'right' } }
        ]];
        AN.top10Combined.forEach(function (v, i) {
          var fill = i % 2 === 0 ? PPT.paper : PPT.bg;
          rows.push([
            { text: v.matricule || '—', options: { fill: { color: fill }, fontSize: 9, color: PPT.ink } },
            { text: v.chauffeur || '—', options: { fill: { color: fill }, fontSize: 9, color: PPT.slate } },
            { text: fmt(v.entretien, 0), options: { fill: { color: fill }, fontSize: 9, color: PPT.slate, align: 'right' } },
            { text: fmt(v.carburant, 0), options: { fill: { color: fill }, fontSize: 9, color: PPT.slate, align: 'right' } },
            { text: fmt(v.csc, 0), options: { fill: { color: fill }, fontSize: 9, color: PPT.slate, align: 'right' } },
            { text: fmt(v.total, 0), options: { fill: { color: fill }, fontSize: 9, bold: true, color: PPT.navy, align: 'right' } }
          ]);
        });
        s.addTable(rows, { x: 0.6, y: 1.6, w: 12.1, colW: [2.2, 3.0, 1.9, 1.9, 1.5, 1.6], border: { pt: 0.5, color: PPT.mist }, valign: 'middle', rowH: 0.42 });
      } else {
        s.addText('Aucune donnée disponible.', { x: 0.6, y: 3, w: 10, h: 0.6, fontSize: 16, color: PPT.slate });
      }
    }

    /* ============ SLIDE 12 — SYNTHÈSE & RECOMMANDATIONS ============ */
    {
      var s = pres.addSlide();
      s.background = { color: PPT.midnight };
  ...
  async function buildRapportAnnuelExcel(ExcelJSCtor, AN) {
    ...
    /* ---- Feuille Entretien Périodique ---- */
    var wsEnt = wb.addWorksheet('Entretien Périodique');
    var nextRowEnt = styleSheetSection(wsEnt, 1, 'TOTAUX ENTRETIEN PÉRIODIQUE — ' + AN.annee,
      ['Type d\u2019intervention', 'Nombre'], [
        ['Vidanges', AN.entretien.nbVidange],
        ['Kits chaîne', AN.entretien.nbChaine],
        ['Accidents', AN.entretien.nbAccident],
        ['Lavages', AN.entretien.nbLavage],
        ['Joints culasse', AN.entretien.nbJointCulasse],
        ['Pneumatiques', AN.entretien.nbPneumatique]
      ], [30, 16]);
    styleSheetSection(wsEnt, nextRowEnt, 'DÉTAIL PAR DIVISION',
      ['Division', 'Vidanges', 'Kits chaîne', 'Accidents', 'Lavages', 'Joints culasse', 'Pneumatiques'],
      AN.entretienParDivisionList.map(function (d) { return [d.label, d.nbVidange, d.nbChaine, d.nbAccident, d.nbLavage, d.nbJointCulasse, d.nbPneumatique]; }),
      [30, 12, 12, 12, 12, 14, 14]);
    ...
    /* ---- Feuille Dépense par Véhicule (liste complète) ---- */
    var wsVeh = wb.addWorksheet('Dépense par Véhicule');
    styleSheetSection(wsVeh, 1, 'DÉPENSE PAR VÉHICULE (ENTRETIEN + CARBURANT + CSC) — ' + AN.annee,
      ['Matricule', 'Chauffeur', 'Division', 'Marque', 'Entretien (DT)', 'Carburant (DT)', 'CSC (DT)', 'Total (DT)', '% du total'],
      AN.depenseParVehiculeList.map(function (v) {
        return [v.matricule || '—', v.chauffeur || '—', v.division || '—', v.marque || '—',
          Math.round(v.entretien), Math.round(v.carburant), Math.round(v.csc), Math.round(v.total),
          AN.dépenseGlobale ? ((v.total / AN.dépenseGlobale) * 100).toFixed(1) + '%' : '0%'];
      }), [16, 26, 22, 18, 16, 16, 14, 16, 12]);
  ...
  window.renderRapportAnnuelKpis = function () {
    ...
    grid.innerHTML =
      ...
      card('Vidanges', fmt(AN.entretien.nbVidange)) +
      card('Accidents', fmt(AN.entretien.nbAccident)) +
      card('Lavages', fmt(AN.entretien.nbLavage)) +
      card('Pneumatiques', fmt(AN.entretien.nbPneumatique));
  };
