  ...
  initAdminLogin() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (user) {
        ...
        sessionStorage.setItem('_otp_admin', JSON.stringify(OTP_CONFIG.pendingAdmin));
...
async function requestOTPEmail(vehicleData) {
  ...
  // Sauvegarder dans sessionStorage pour survivre aux rechargements
...
function verifyOTP(inputCode) {
  // Restaurer depuis sessionStorage si mémoire perdue
  if (!OTP_CONFIG.code) {
    const saved = sessionStorage.getItem('_otp_code');
    const expiry = sessionStorage.getItem('_otp_expiry');
    if (saved && expiry) { OTP_CONFIG.code = saved; OTP_CONFIG.expiry = parseInt(expiry); }
  }
...
async function validateOTPAndLogin() {
  ...
  if (result.ok) {
    ...
    // Cas Admin
    // Restaurer pendingAdmin depuis sessionStorage si perdu
    if (!OTP_CONFIG.pendingAdmin) {
      const saved = sessionStorage.getItem('_otp_admin');
      if (saved) OTP_CONFIG.pendingAdmin = JSON.parse(saved);
    }
    if (OTP_CONFIG.pendingAdmin) {
      ...
      if (window.parcAuto) {
        ...
        // Nettoyer OTP sessionStorage
        sessionStorage.removeItem('_otp_code');
        sessionStorage.removeItem('_otp_expiry');
        sessionStorage.removeItem('_otp_admin');
        // Vérifier que la sauvegarde est bien faite
        const verify = JSON.parse(localStorage.getItem('parcAutoData_v3') || '{}');
        if (verify.currentUser && verify.currentUser.role === 'admin') {
          window.location.replace('admin.html');
        } else {
          parcAuto.showToast('Erreur session. Réessayez.', 'error');
        }
      }
      return;
    }
