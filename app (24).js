...
class ParcAutoApp {
        ...
        setTimeout(() => {
          ...
          if (typeof showTips === 'function') {
            showTips(() => {
              ...
              speakWelcome(() => {});
              // Lire les alertes vocalement s'il y en a
              const vehicle = this.data.vehicles.find(v => v.matricule === matricule);
              if (vehicle) {
                const userAlerts = this.getAlerts().filter(a => a.vehicle.id === vehicle.id);
                if (userAlerts.length > 0) {
                  setTimeout(() => this.speakAlerts(userAlerts), 3500);
                }
              }
            });
          } else {
            ...
            if (vehicle2) {
              const userAlerts2 = this.getAlerts().filter(a => a.vehicle.id === vehicle2.id);
              if (userAlerts2.length > 0) {
                setTimeout(() => this.speakAlerts(userAlerts2), 2500);
              }
            }
          }
        }, 500);
  ...
  speakAlerts(alerts) {
    ...
    const alertTexts = alerts.map((a, i) => {
      return `Alerte ${i + 1}: ${a.type} pour le véhicule ${a.vehicle.matricule}. ${a.detail}.`;
    });
