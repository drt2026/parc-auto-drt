...
(function () {
  ...
  function initSuiviCarburantModal() {
    ...
    css.textContent = `
      ...
      #tr3-table-wrap td input[type=number] { width:88px; padding:5px 7px; border:1.5px solid #e2e8f0;
    ...
    function renderTable(nom, mois) {
      ...
      var rowsHtml = vehicles.map(function(mat) {
        ...
        var last = getLastIndexFin(mat, mois);
        var debVal = entry ? entry.indexDebut : (last != null ? last : '');
        var debReadonly = (last != null) && (!entry || entry.indexDebut === last);
      ...
      wrap.innerHTML = '<table><thead><tr>' +
        '<th>Matricule</th><th>Marque</th><th>Index Début</th><th>Index Fin du mois</th><th>Distance</th>' +
    ...
    document.getElementById('tr3-save-btn').addEventListener('click', function() {
      ...
      rows.forEach(function(tr) {
        ...
        toSave.push({ mat: mat, deb: deb, fin: fin });
      });
  ...
  function initSuiviCarburantAdminTab() {
      ...
      tabEl.innerHTML = `
        ...
        <div class="tr2-table-wrap">
          <table class="tr2-table">
            <thead>
              <tr>
                <th>Mois</th><th>Matricule</th><th>Marque</th><th>Index Début</th>
                <th>Index Fin</th><th>Distance</th><th>Saisi par</th><th>Date saisie</th>
              </tr>
            </thead>
    ...
    function renderCarburantTable(search) {
      ...
      if (!list.length) {
        tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;color:#94a3b8;padding:20px">Aucune saisie pour le moment.</td></tr>';
        return;
      }
    ...
    function exportCarburantExcel() {
      ...
      var bom = '\uFEFF';
      var header = ['Mois','Matricule','Marque','Index Début','Index Fin','Distance','Saisi par','Date saisie'];
      var rows = list.slice().sort(function(a,b){
        ...
        var marque = getMarqueFromMat(e.matricule) || '';
        var dist = (e.indexFin != null && e.indexDebut != null) ? (e.indexFin - e.indexDebut) : '';
        return [e.mois||'', e.matricule||'', marque, e.indexDebut||'', e.indexFin||'', dist, e.saisiPar||'', formatDate(e.dateSaisie)].join(';');
      });
      var csv = bom + header.join(';') + '\n' + rows.join('\n');
      var blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
      var url  = URL.createObjectURL(blob);
      var a    = document.createElement('a');
      a.href     = url;
      a.download = 'suivi_carburant_' + new Date().toISOString().slice(0,10) + '.csv';
      a.click();
      URL.revokeObjectURL(url);
    }
