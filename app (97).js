...
// ============================================
// MODIFICATION DATE VISITE PAR L'UTILISATEUR
// ============================================

function openUserVisiteModal(matricule, currentDate) {
  // Créer le modal s'il n'existe pas
  let modal = document.getElementById('user-visite-modal');
  if (!modal) {
    modal = document.createElement('div');
    modal.id = 'user-visite-modal';
    modal.className = 'modal-overlay';
    modal.innerHTML = `
      <div class="modal" style="max-width:420px;">
        <div class="modal-header">
          <div class="modal-title">📅 Modifier la date de visite technique</div>
          <button class="modal-close" onclick="closeUserVisiteModal()">✕</button>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label class="form-label">Nouvelle date de visite technique</label>
            <input type="date" id="user-visite-input" class="form-control" required>
          </div>
          <p style="font-size:12px;color:#64748b;margin-top:8px;">
            ℹ️ Cette date sera mise à jour pour le véhicule <strong id="user-visite-matricule"></strong>
          </p>
        </div>
        <div class="modal-footer">
          <button class="btn btn-secondary" onclick="closeUserVisiteModal()">Annuler</button>
          <button class="btn btn-primary" onclick="saveUserVisite()">💾 Enregistrer</button>
        </div>
      </div>
    `;
  ...
  document.getElementById('user-visite-matricule').textContent = matricule;
  document.getElementById('user-visite-input').value = currentDate || '';
  document.getElementById('user-visite-input').dataset.matricule = matricule;

  modal.classList.add('active');
}

function closeUserVisiteModal() {
  const modal = document.getElementById('user-visite-modal');
  if (modal) modal.classList.remove('active');
}

async function saveUserVisite() {
  const input = document.getElementById('user-visite-input');
  const matricule = input.dataset.matricule;
  const newDate = input.value;

  if (!newDate) {
    if (window.parcAuto) parcAuto.showToast('⚠️ Veuillez sélectionner une date', 'error');
    return;
  }
  ...
  const raw = localStorage.getItem('parcAutoData_v3');
  if (!raw) {
    if (window.parcAuto) parcAuto.showToast('❌ Données non trouvées', 'error');
    return;
  }
  ...
  if (!vehicle) {
    if (window.parcAuto) parcAuto.showToast('❌ Véhicule non trouvé', 'error');
    return;
  }
  ...
  // Sync cloud si configuré
  if (window.parcAuto && window.parcAuto.isCloudConfigured()) {
    window.parcAuto.data = data;
    await window.parcAuto.writeToCloud();
  }

  // Mettre à jour l'affichage
  if (window.parcAuto) {
    window.parcAuto.data = data;
    window.parcAuto.renderUserVehicleDetail();
  }

  closeUserVisiteModal();

  if (window.parcAuto) {
    window.parcAuto.showToast('✅ Date de visite technique mise à jour', 'success');
  }
}
