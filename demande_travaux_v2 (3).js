...
(function () {
  ...
  function initUserForm() {
    ...
    document.getElementById('tr2-new-btn').addEventListener('click', function() {
      ['tr2-nom','tr2-div','tr2-subdiv','tr2-mat','tr2-marq','tr2-km','tr2-nat'].forEach(function(id) {
  ...
  function initAdminTab() {
    ...
    function openDetail(id, mode) {
      ...
      if (d.statut === 'EN ATTENTE') {
        ...
        actionsEl.innerHTML = `
          <button class="tr2-det-btn tr2-det-print" id="tr2-det-print-btn" style="flex:1">🖨️ Imprimer</button>
          <button class="tr2-det-btn tr2-det-close" id="tr2-det-close-btn" style="flex:1">Fermer</button>`;
        document.getElementById('tr2-det-close-btn').addEventListener('click', function() {
          detailOverlay.classList.remove('open');
        });
      }
    ...
    // BLOC ADDITIF — Impression demande
    function printDemande(d) {
      ...
      var html = `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8">
        <title>Demande ${d.numero||''}</title>
        <style>
          * { margin:0; padding:0; box-sizing:border-box; }
          body { font-family:Arial,sans-serif; color:#1e293b; background:#fff; }
          .page { max-width:700px; margin:0 auto; padding:32px; }
          .header { background:linear-gradient(135deg,#1A1A2E,#16213E); color:#fff; border-radius:12px; padding:24px 28px; margin-bottom:24px; display:flex; align-items:center; justify-content:space-between; }
          .header h1 { font-size:20px; margin-bottom:4px; }
          .header p  { font-size:12px; opacity:.7; }
          .num-badge { background:#E67E22; color:#fff; padding:6px 18px; border-radius:20px; font-size:14px; font-weight:700; }
          .section { border:1px solid #e2e8f0; border-radius:12px; overflow:hidden; margin-bottom:20px; }
          .section-title { background:#f8fafc; padding:10px 16px; font-size:11px; text-transform:uppercase; letter-spacing:.5px; color:#64748b; font-weight:700; border-bottom:1px solid #e2e8f0; }
          .row { display:flex; border-bottom:1px solid #f1f5f9; }
          .row:last-child { border-bottom:none; }
          .cell { flex:1; padding:12px 16px; }
          .cell .lbl { font-size:10px; text-transform:uppercase; color:#94a3b8; letter-spacing:.5px; margin-bottom:3px; }
          .cell .val { font-size:14px; font-weight:600; }
          .nature-box { background:#FEF3E2; border-left:4px solid #E67E22; padding:14px 18px; border-radius:0 8px 8px 0; margin-bottom:20px; }
          .nature-box .lbl { font-size:10px; text-transform:uppercase; color:#92400e; font-weight:700; letter-spacing:.5px; margin-bottom:6px; }
          .nature-box .val { font-size:14px; line-height:1.7; }
          .statut-badge { display:inline-block; background:${statutColor}22; color:${statutColor}; padding:4px 14px; border-radius:20px; font-size:12px; font-weight:700; border:1px solid ${statutColor}44; }
          .comment-box { background:#f8fafc; border:1px solid #e2e8f0; border-radius:8px; padding:12px 16px; margin-bottom:20px; }
          .comment-box .lbl { font-size:10px; text-transform:uppercase; color:#64748b; font-weight:700; margin-bottom:4px; }
          .footer { text-align:center; font-size:10px; color:#94a3b8; border-top:1px solid #e2e8f0; padding-top:16px; margin-top:8px; }
        </style>
      </head><body><div class="page">
        <div class="header">
          <div>
            <h1>🔧 Demande de Travaux</h1>
            <p>Parc Auto DRT Sfax — Tunisie Telecom</p>
          </div>
          <div class="num-badge">${d.numero||''}</div>
        </div>
        <div class="section">
          ...
          <div class="row">
            <div class="cell"><div class="lbl">Statut</div><div class="val"><span class="statut-badge">${statutLabel}</span></div></div>
            <div class="cell"><div class="lbl">Date création</div><div class="val">${formatDate(d.dateCreation)}</div></div>
          </div>
        </div>
        <div class="nature-box">
          <div class="lbl">Nature de l'intervention demandée</div>
          <div class="val">${d.natureIntervention||'-'}</div>
        </div>
        ${d.commentaire ? '<div class="comment-box"><div class="lbl">Commentaire / Décision</div><div style="font-size:14px;margin-top:4px;">' + d.commentaire + '</div></div>' : ''}
        <div class="footer">
          Imprimé le ${new Date().toLocaleDateString('fr-FR')} à ${new Date().toLocaleTimeString('fr-FR', {hour:'2-digit',minute:'2-digit'})}<br>
          Parc Auto DRT Sfax — Tunisie Telecom — Direction Régionale de Sfax
        </div>
      </div></body></html>`;
