...
(function () {
  ...
  function ensureTesseract(cb) {
    console.log('[BL-DEBUG] ensureTesseract() — window.Tesseract déjà présent ?', !!window.Tesseract); // BLOC DEBUG
    if (window.Tesseract) { cb(); return; }
    var s = document.createElement('script');
    s.src = 'https://cdn.jsdelivr.net/npm/tesseract.js@5/dist/tesseract.min.js';
    console.log('[BL-DEBUG] Chargement de', s.src); // BLOC DEBUG
    s.onload = function () { console.log('[BL-DEBUG] Tesseract.js chargé avec succès'); cb(); }; // BLOC DEBUG
    s.onerror = function (e) { console.error('[BL-DEBUG] ÉCHEC chargement Tesseract.js', e); setStatus('❌ Impossible de charger le moteur OCR (Tesseract). Vérifiez la connexion internet.', 'error'); };
    document.head.appendChild(s);
  }

  function ensurePDFJS(cb) {
    if (window.pdfjsLib) { cb(); return; }
    // BLOC CORRECTIF — cdnjs ne distribue plus, à partir de pdf.js 4.x, que des
    // modules ES (.mjs) ; le fichier classique "pdf.min.js" qu'on chargeait ici
    // en <script> n'existe plus à cette URL (404 silencieux), d'où l'échec
    // systématique "Impossible de charger le moteur PDF". On utilise à la
    // place le build "legacy" (classique, compatible <script> sans type=module)
    // de pdfjs-dist@3.11.174 via jsDelivr, largement utilisé et stable pour cet
    // usage, y compris sur navigateurs/WebView Android.
    var s = document.createElement('script');
  ...
  function pdfToImages(dataUrl, cb) {
    ensurePDFJS(function () {
      // BLOC CORRECTIF — passer les octets bruts (Uint8Array) plutôt que le
      // dataURL en tant que "url". Avec { url: dataUrl }, PDF.js tente de
      // RE-TÉLÉCHARGER ce data-URL via le réseau (fetch/XHR interne), ce qui
      // échoue silencieusement ou lève une erreur sur de nombreux navigateurs
      // mobiles (limites de taille d'URL, gestion différente des data: URIs
      // en contexte fetch). En passant { data: Uint8Array }, PDF.js lit les
      // octets directement en mémoire, sans repasser par le réseau — fiable
      // sur tous les navigateurs, y compris Android.
  ...
  function processFiles(fileList) {
    console.log('[BL-DEBUG] processFiles() appelé avec', fileList.length, 'fichier(s)'); // BLOC DEBUG
    ...
    files.forEach(function (file, idx) {
      console.log('[BL-DEBUG] Lecture du fichier', idx, ':', file.name, file.type, file.size, 'octets'); // BLOC DEBUG
      var isPdf = file.type === 'application/pdf' || /\.pdf$/i.test(file.name || '');
      var reader = new FileReader();
      reader.onerror = function () { console.error('[BL-DEBUG] ERREUR FileReader pour', file.name); results[idx] = []; checkDone(); }; // BLOC DEBUG
      if (isPdf) {
        reader.onload = function (e) { console.log('[BL-DEBUG] PDF lu, conversion en images...'); pdfToImages(e.target.result, function (pages) { results[idx] = pages; checkDone(); }); }; // BLOC DEBUG
      } else {
        reader.onload = function (e) { console.log('[BL-DEBUG] Image lue avec succès (dataURL généré)'); results[idx] = [e.target.result]; checkDone(); }; // BLOC DEBUG
      }
  ...
  function ocrQueue(dataUrls) {
    ensureTesseract(function () {
      ...
      function recognizeAndParse(dataUrl, cb) {
        console.log('[BL-DEBUG] Appel Tesseract.recognize()...'); // BLOC DEBUG
        Tesseract.recognize(dataUrl, 'fra', { tessedit_pageseg_mode: '4' })
          .then(function (res) {
            console.log('[BL-DEBUG] Tesseract.recognize() terminé avec succès'); // BLOC DEBUG
            var raw = (res && res.data && res.data.text) || '';
            cb(parseBL(raw));
          })
          .catch(function (err) { console.error('[BL-DEBUG] ERREUR Tesseract.recognize()', err); cb(null); });
      }
      function next() {
  ...
  function injectStylesAndModal() {
      ...
      '<div class="bl-scan-modal">' +
        ...
        '<div class="bl-scan-body">' +
          ...
          '<div id="bl-scan-upload-zone">' +
            ...
            '<input type="file" id="bl-scan-file-input" accept="image/*,application/pdf" multiple style="display:none;">' +
    ...
    el('bl-scan-file-input').addEventListener('change', function (evt) {
      console.log('[BL-DEBUG] change event déclenché'); // BLOC DEBUG
      // BLOC CORRECTIF — copier les fichiers dans un vrai tableau AVANT de
      // vider evt.target.value. Sur certains navigateurs Android, evt.target.files
      // est une référence live : réinitialiser value juste après l'avoir lue
      // vidait aussi ce FileList avant que processFiles() ne s'exécute, ce qui
      // faisait échouer silencieusement l'import (rien ne se passait après le
      // choix du fichier).
      var files = Array.prototype.slice.call(evt.target.files || []);
      console.log('[BL-DEBUG] fichiers sélectionnés :', files.length); // BLOC DEBUG
      evt.target.value = '';
  ...
  // BLOC ADDITIF — Détection Vidange / Kit Chaîne dans la désignation d'une ligne BL
  // et calcul automatique du prochain kilométrage d'échéance, exactement comme pour
  // l'Index Carburant : plus besoin de ressaisir "Prochaine Vidange" / "Prochaine Chaîne"
  // à la main après un import de BL.
  var INTERVAL_VIDANGE_KM = 10000; // Vidange tous les 10 000 km
  var INTERVAL_CHAINE_KM  = 60000; // Kit Chaîne tous les 60 000 km

  function detectNextEcheances(designation, kmActuel) {
    var txt = String(designation || '').toLowerCase();
    var km = parseInt(kmActuel, 10);
    var result = { nextVidange: null, nextChaine: null };
    if (isNaN(km)) return result;

    var isVidange = /vidange|huile\s*moteur|filtre\s*(à|a)?\s*huile/.test(txt);
    var isChaine  = /kit\s*cha[iî]ne|cha[iî]ne\s*(de\s*)?distribution|cha[iî]ne\s*(de\s*)?transmission/.test(txt);

    if (isVidange) result.nextVidange = km + INTERVAL_VIDANGE_KM;
    if (isChaine)  result.nextChaine  = km + INTERVAL_CHAINE_KM;
    return result;
  }
  // FIN BLOC ADDITIF
  ...
  function importLignes() {
    ...
    var i = 0;
    var echeancesDetectees = []; // BLOC ADDITIF — pour le message récapitulatif final
    ...
    function importNext() {
      if (i >= lignesAImporter.length) {
        ...
        var recap = '✅ ' + lignesAImporter.length + ' réparation(s) importée(s) depuis le BL' + (blNum ? ' n°' + blNum : '') + '.';
        // BLOC ADDITIF — mention explicite des échéances auto-calculées (transparence)
        if (echeancesDetectees.length) recap += ' ' + echeancesDetectees.join(' ');
        // FIN BLOC ADDITIF
        setStatus(recap + (hasNext ? ' Passage au BL suivant…' : ''), 'success');
      ...
      // BLOC ADDITIF — auto-remplissage Prochaine Vidange / Prochaine Chaîne si détectées
      // dans la désignation de la ligne scannée (sinon, comportement inchangé : champ vide)
      var echeances = detectNextEcheances(ligne.designation, km);
      var nvEl = el('repair-next-vidange'); if (nvEl) nvEl.value = echeances.nextVidange || '';
      if (echeances.nextVidange) echeancesDetectees.push('🛢️ Vidange détectée → prochaine à ' + echeances.nextVidange.toLocaleString('fr-FR') + ' km.');
      var ncEl = el('repair-next-chaine');  if (ncEl) ncEl.value = echeances.nextChaine  || '';
      if (echeances.nextChaine) echeancesDetectees.push('⛓️ Kit Chaîne détecté → prochain à ' + echeances.nextChaine.toLocaleString('fr-FR') + ' km.');
      // FIN BLOC ADDITIF
