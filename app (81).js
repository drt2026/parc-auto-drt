...
class ParcAutoApp {
  ...
  initUserLogin() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      // --- ÉTAPE 1 : Récupérer les deux matricules ---
      // Chercher tous les inputs text dans le formulaire
      const form = document.getElementById('user-login-form');
      const allInputs = form ? form.querySelectorAll('input[type="text"], input:not([type])') : [];

      let agentEl = null, vehicleEl = null;

      // Méthode 1: Par ID
      agentEl = document.getElementById('user-matricule-agent') 
        || document.getElementById('agent-matricule')
        || document.getElementById('matricule-agent');
      vehicleEl = document.getElementById('user-matricule-vehicle')
        || document.getElementById('vehicle-matricule')
        || document.getElementById('matricule-vehicle');

      // Méthode 2: Par name
      if (!agentEl) agentEl = document.querySelector('input[name="agent-matricule"]') || document.querySelector('input[name="matricule-agent"]');
      if (!vehicleEl) vehicleEl = document.querySelector('input[name="vehicle-matricule"]') || document.querySelector('input[name="matricule-vehicle"]');

      // Méthode 3: Par placeholder
      if (!agentEl) agentEl = document.querySelector('input[placeholder*="Agent"]') || document.querySelector('input[placeholder*="agent"]');
      if (!vehicleEl) vehicleEl = document.querySelector('input[placeholder*="Véhicule"]') || document.querySelector('input[placeholder*="véhicule"]');

      // Méthode 4: Par label text (chercher le label puis l'input associé)
      if (!agentEl) {
        const labels = document.querySelectorAll('label');
        for (const lbl of labels) {
          if (lbl.textContent.toLowerCase().includes('agent') || lbl.textContent.toLowerCase().includes('agent')) {
            const forId = lbl.getAttribute('for');
            if (forId) agentEl = document.getElementById(forId);
            if (!agentEl) agentEl = lbl.querySelector('input') || lbl.nextElementSibling?.querySelector('input');
            if (agentEl) break;
          }
        }
      }
      if (!vehicleEl) {
        const labels = document.querySelectorAll('label');
        for (const lbl of labels) {
          if (lbl.textContent.toLowerCase().includes('véhicule') || lbl.textContent.toLowerCase().includes('vehicule')) {
            const forId = lbl.getAttribute('for');
            if (forId) vehicleEl = document.getElementById(forId);
            if (!vehicleEl) vehicleEl = lbl.querySelector('input') || lbl.nextElementSibling?.querySelector('input');
            if (vehicleEl) break;
          }
        }
      }

      // Méthode 5: Par position dans le formulaire (premier input = agent, deuxième = véhicule)
      if (!agentEl && allInputs.length >= 1) agentEl = allInputs[0];
      if (!vehicleEl && allInputs.length >= 2) vehicleEl = allInputs[1];

      const agentInput = agentEl?.value?.trim().toUpperCase();
      const vehicleInput = vehicleEl?.value?.trim().toUpperCase();

      console.log('DEBUG: agentEl found?', !!agentEl, 'id=', agentEl?.id, 'value=', agentEl?.value);
      console.log('DEBUG: vehicleEl found?', !!vehicleEl, 'id=', vehicleEl?.id, 'value=', vehicleEl?.value);
      console.log('DEBUG: form found?', !!form, 'inputs count=', allInputs.length);

      // Vérifier que les deux champs sont remplis
      if (!agentInput) {
        this.showToast('Veuillez saisir votre matricule agent', 'error');
        return;
      }
      if (!vehicleInput) {
      ...
      // Recharger les données depuis le cloud (fusion avec local)
      if (this.isCloudConfigured()) {
        const cloudData = await this.readFromCloud();
        if (cloudData && cloudData.vehicles) {
          // Fusion intelligente: pour chaque véhicule, merger les champs cloud + locaux
          const localVehicles = (this.data.vehicles || []);
          const cloudVehicles = (cloudData.vehicles || []);
          const mergedVehicles = [];

          // Traiter tous les véhicules locaux
          localVehicles.forEach(lv => {
            const cv = cloudVehicles.find(v => v.matricule === lv.matricule);
            if (cv) {
              // Véhicule existe dans les deux: fusionner (cloud prend priorité sur certains champs, local sur matriculeAgent)
              const merged = { ...lv, ...cv };
              // MAIS si le cloud a perdu matriculeAgent, garder la valeur locale
              if (!cv.matriculeAgent && lv.matriculeAgent) {
                merged.matriculeAgent = lv.matriculeAgent;
              }
              mergedVehicles.push(merged);
            } else {
              // Véhicule local uniquement
              mergedVehicles.push(lv);
            }
          });

          // Ajouter les véhicules cloud qui n'existent pas localement
          cloudVehicles.forEach(cv => {
            if (!localVehicles.find(v => v.matricule === cv.matricule)) {
          ...
          console.log('Fusion cloud: ' + mergedVehicles.length + ' véhicules');
        }
      }

      // Vérifier si des véhicules existent
      if (!this.data.vehicles || this.data.vehicles.length === 0) {
        this.showToast("Aucun véhicule n'est enregistré dans le système.", "error");
        return;
      }

      // --- ÉTAPE 2 : Rechercher le véhicule par matricule ---
      console.log('DEBUG: Recherche véhicule, input=' + vehicleInput);
      console.log('DEBUG: Nombre véhicules=' + (this.data.vehicles || []).length);

      const vehicle = this.data.vehicles.find(v => {
        const dbMat = v.matricule.toUpperCase().replace(/[\s-]/g, '').trim();
        const inputMat = vehicleInput.replace(/[\s-]/g, '').trim();
        console.log('DEBUG: Comparaison véhicule db=' + dbMat + ' vs input=' + inputMat + ' -> ' + (dbMat === inputMat));
        return dbMat === inputMat;
      });

      // Véhicule non trouvé → ERREUR
      if (!vehicle) {
        console.log('DEBUG: Véhicule NON TROUVÉ');
        this.showToast('Matricule véhicule non trouvé. Vérifiez votre saisie.', 'error');
        return;
      }
      console.log('DEBUG: Véhicule trouvé=' + vehicle.matricule + ', agent=' + vehicle.matriculeAgent);

      // --- ÉTAPE 3 : Vérifier que le matricule agent correspond ---
      // Table de correspondance hardcodée (fallback si données cloud perdues)
      const AGENT_VEHICLE_MAP = {
        '17-351511': '75970',
        '17-351512': '76004',
        '17-356835': '72858',
        '17-356842': '71620',
      ...
      let agentFromDb = vehicle.matriculeAgent;

      // Si undefined, chercher dans la table hardcodée
      if (!agentFromDb && AGENT_VEHICLE_MAP[vehicle.matricule]) {
        agentFromDb = AGENT_VEHICLE_MAP[vehicle.matricule];
        console.log('DEBUG: matriculeAgent trouvé dans MAP: ' + agentFromDb);
      }

      // Si toujours undefined, chercher dans DEFAULT_DATA
      if (!agentFromDb) {
        try {
          const defaultData = JSON.parse(JSON.stringify(DEFAULT_DATA));
          const defaultVehicle = defaultData.vehicles.find(v => v.matricule === vehicle.matricule);
          if (defaultVehicle && defaultVehicle.matriculeAgent) {
            agentFromDb = defaultVehicle.matriculeAgent;
            console.log('DEBUG: matriculeAgent trouvé dans DEFAULT_DATA: ' + agentFromDb);
          }
        } catch(e) {}
      }

      const dbAgentMat = String(agentFromDb || '').toUpperCase().replace(/[\s-]/g, '').trim();
      const inputAgentMat = agentInput.replace(/[\s-]/g, '').trim();
      console.log('DEBUG: Comparaison agent db=' + dbAgentMat + ' vs input=' + inputAgentMat + ' -> ' + (dbAgentMat === inputAgentMat));

      if (dbAgentMat !== inputAgentMat) {
        console.log('DEBUG: ÉCHEC AUTH - dbAgentMat=' + dbAgentMat + ' (length=' + dbAgentMat.length + ') vs inputAgentMat=' + inputAgentMat + ' (length=' + inputAgentMat.length + ')');
        this.showToast('Matricule agent incorrect. Attendu: ' + (agentFromDb || 'NON DÉFINI') + ' | Saisi: ' + agentInput, 'error');
        return;
      }

      // --- ÉTAPE 4 : Tout est correct → ouvrir l'interface ---
      this.data.currentUser = { 
        role: 'user', 
        name: vehicle.chauffeur || 'Utilisateur',
        matriculeAgent: vehicle.matriculeAgent 
      };
      this.data.userVehicle = vehicle.matricule;
      await this.saveData();
      this.showToast('Accès autorisé ! Redirection...', 'success');
