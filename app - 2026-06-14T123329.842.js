...
class ParcAutoApp {
  ...
  startAutoSync() {
    ...
    this._autoSyncInterval = setInterval(async () => {
      ...
      // 🔧 FIX CRITIQUE: Ne pas lire le cloud si modification locale récente (< 60s)
      // Cloudflare KV est "eventually consistent" - lecture immédiate après écriture
      // peut retourner l'ancienne valeur du cache
      if (this._lastLocalSave && (Date.now() - this._lastLocalSave < 60000)) {
        console.log('AutoSync: modification locale récente (< 60s), lecture cloud ignorée');
        return;
      }
  ...
  renderUserVehicleDetail() {
    ...
    // 🔧 FIX CRITIQUE: Toujours recharger depuis localStorage pour avoir les données
    // les plus récentes (évite l'écrasement par sync cloud concurrente)
    try {
      const saved = localStorage.getItem('parcAutoData_v3');
      if (saved) {
        const localData = JSON.parse(saved);
        const _uv = this.data.userVehicle;
        const _cu = this.data.currentUser;
        this.data = localData;
        // Préserver la session utilisateur
        this.data.userVehicle = _uv || localData.userVehicle;
        this.data.currentUser = _cu || localData.currentUser;
      }
    } catch(e) {
      console.warn('Erreur rechargement localStorage dans renderUserVehicleDetail:', e);
    }
