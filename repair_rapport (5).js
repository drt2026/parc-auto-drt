...
(function () {
  ...
  const CLR = {
    navyFg  : 'FFFFFFFF', navyBg  : 'FF132C4C',
    orangeBg: 'FFC97A1A', orangeFg: 'FFFFFFFF',
    greyBg  : 'FFF2F5F9', greyFg  : 'FF1C2536',
    whiteBg : 'FFFFFFFF',
    greenBg : 'FFDCF3E8', greenFg : 'FF0F5C3E',
    redBg   : 'FFFBE4E4', redFg   : 'FF8A2A2A',
    borderClr: 'FFE4E9F0', alertBg : 'FFFBF3E3',
  };
  ...
  function barColorByRank(idx, n) {
    ...
    const palette = ['FFC97A1A','FFD99A4E','FFE4B87A','FF132C4C','FF24476F','FF5B6B82','FF8A99AC'];
  ...
  function genRepairRapportAnnuel(annee) {
    ...
    // Chrome premium : en-tête figé, filtre auto, mise en page impression sur toutes les feuilles de données
    const HEADER_ROW_BY_SHEET = { 'Par Division': 2, 'Par Marque': 2, 'Top 10': 2, 'Détail': 2 };
    wb.SheetNames.forEach((name) => {
      const ws = wb.Sheets[name];
      if (!ws || !ws['!ref']) return;
      const isGraph = /^Graph\./.test(name);
      const ySplit = isGraph ? 3 : (HEADER_ROW_BY_SHEET[name] !== undefined ? HEADER_ROW_BY_SHEET[name] + 1 : 0);
      if (ySplit) ws['!views'] = [{ state: 'frozen', ySplit, topLeftCell: `A${ySplit + 1}`, activePane: 'bottomLeft' }];
      if (HEADER_ROW_BY_SHEET[name] !== undefined) {
  ...
  function buildRepairReportPptx(PptxGenJSCtor, allData, annee, meta) {
    ...
    {
      DRT_KIT.titleSlide({
        pres, icon: '🔧', kicker: 'Réparations & maintenance — DRT Sfax', title: 'Rapport Réparations',
        subtitle: `Exercice ${annee} — Parc Automobile DRT Sfax`,
        accentColor: PPT.orange, accentSoftColor: 'FBEBD6',
        chips: [
    ...
    /* ============ SLIDE 2 — BILAN ============ */
    {
      ...
      DRT_KIT.footer(pres, s, { wordmark: 'DRT SFAX · RÉPARATIONS', accentColor: PPT.orange, dark: false });
    }
    ...
    /* ============ SLIDE 5 — TOP 10 VÉHICULES / MARQUES ============ */
    {
      ...
      if (top10.length) {
        ...
        top10.forEach((e, i) => {
          const fill = i < 3 ? 'FBF3E3' : (i % 2 === 0 ? PPT.paper : PPT.bg);
          rows.push([
    ...
    /* ============ SLIDE 6 — POINTS D'ATTENTION ============ */
    {
      ...
      DRT_KIT.footer(pres, s, { wordmark: 'DRT SFAX · RÉPARATIONS', accentColor: PPT.orange, dark: true });
    }
