...
async function saveUserKm(matricule) {
  ...
  // Fermer le modal IMMÉDIATEMENT pour éviter double-clic
  document.getElementById('user-km-modal')?.remove();

  if (!window.parcAuto) return;

  // 🔧 FIX CRITIQUE: Toujours recharger les données fraîches depuis localStorage
  // avant modification pour éviter que des données stale en mémoire écrasent la modif
  try {
    const saved = localStorage.getItem('parcAutoData_v3');
    if (saved) {
    ...
    console.warn('Erreur rechargement localStorage:', e);
  }
  ...
  // Mettre à jour le timestamp pour que le cloud reconnaisse cette version comme plus récente
  parcAuto.data._lastSaved = Date.now();

  // Sauvegarder en local (immédiat)
  try {
  ...
  // Re-render IMMÉDIATEMENT avec les données fraîches
  parcAuto.renderUserVehicleDetail();

  // 🔧 FIX: Bloquer l'auto-sync cloud pendant 5 secondes pour éviter écrasement
  parcAuto._justSavedLocally = Date.now();

  // Sync cloud en arrière-plan (après le render)
  if (parcAuto.isCloudConfigured()) {
    parcAuto.showSyncStatus('syncing');
    parcAuto.writeToCloud()
      .then(success => parcAuto.showSyncStatus(success ? 'synced' : 'error'))
      .catch(() => parcAuto.showSyncStatus('error'));
  }

  parcAuto.showToast(`✅ Kilométrage mis à jour : ${oldKm.toLocaleString()} → ${newKm.toLocaleString()} km`, 'success');
}
// ============================================
...
async function saveUserVisite(matricule) {
  ...
  // 🔧 FIX: Bloquer l'auto-sync cloud pendant 5 secondes
  parcAuto._justSavedLocally = Date.now();
